...
class Cache:
    ...
    def __init__(self, domain=None, db=None):
        ...
        if db:
            self.db = db
            self.db.define(cache="""url TEXT UNIQUE, html TEXT,
                                    headers JSON, data JSON""")
    ...
    def add(self, *resource_urls):
        for resource_url in resource_urls:
            ...
            try:
                self.db.insert("cache", url=url, html=resource.text,
    ...
    def __getitem__(self, resource_url):
        try:
            ...
            try:
                url = self.format_url(resource_url)
                resource_data = self.db.select("cache", where="url = ?",
                                               vals=[url])[0]
...
class Transaction:
    ...
    def __init__(self, url, method="get", fetch=True, **kwargs):
        ...
        if fetch:
            ...
            self.response = request(method, self.url, **kwargs)
