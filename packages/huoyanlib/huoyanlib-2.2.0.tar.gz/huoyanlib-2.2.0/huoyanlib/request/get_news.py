...
def get_news():
    def search():
        kw = e1.get()
        if len(kw.replace(" ", "")) == 0:
            messagebox.showwarning("火焰新闻 - 警告", "搜索内容不能为空！", parent=win)
            return
    ...
    l1 = tk.Label(win, text="火焰新闻", font=(None, 40), fg="red")
    l1.pack()
    ...
    def copytext(item):
        ...
        if selection == "":
            messagebox.showwarning("火焰新闻 - 警告", "请选中要复制的那行！", parent=win)
            return
    ...
    def openurl():
        selection = searchList.item(searchList.focus())["values"]
        if selection == "":
            messagebox.showwarning("火焰新闻 - 警告", "请选中要打开链接的那行！", parent=win)
            return
