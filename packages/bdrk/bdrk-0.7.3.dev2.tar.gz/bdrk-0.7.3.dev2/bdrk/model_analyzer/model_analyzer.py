...
class ModelAnalyzer:
    ...
    def fairness_config(self, config: Dict[str, Dict[str, List]]):
        ...
        for feature in config:
            ...
            self.fconfig[feature]["group_a"] = (
                config[feature].get("group_a")
                or config[feature].get("unprivileged_attribute_values")
            )
            self.fconfig[feature].pop("unprivileged_attribute_values", None)
            self.fconfig[feature]["group_b"] = (
                config[feature].get("group_b")
                or config[feature].get("privileged_attribute_values")
            )
    ...
    def analyze(self):
        ...
        if self.fconfig:
            if self.test_feat is None or self.test_lbs is None or self.test_inf is None:
                ...
                fairness = (
                    regression_fairness.RegressionFairness(self.fconfig, self.test_feat, self.test_inf, self.test_lbs)
                    if self.model_task.value == ModelTask.REGRESSION.value
                    else classifier_fairness.ClassifierFairness(self.fconfig, self.test_feat, self.test_inf, self.test_lbs)
                )
