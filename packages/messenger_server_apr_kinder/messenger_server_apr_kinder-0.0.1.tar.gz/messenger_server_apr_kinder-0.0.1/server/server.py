...
class Server(threading.Thread, metaclass=ServerMaker):
    ...
    def process_message(self, message, listen_socks):
        if message[DESTINATION] in self.names and self.names[message[DESTINATION]
            ...
            logger.error(
                f'Пользователь {message[DESTINATION]} оффлайн, отправка сообщения невозможна.')
    ...
    def process_client_message(self, message, client):
        ...
        if ACTION in message and message[ACTION] == PRESENCE and TIME in message and USER in message:
            ...
            if message[USER][ACCOUNT_NAME] not in self.names.keys():
                ...
                if self.database.check_password(
                        message[USER][ACCOUNT_NAME],
                        message[USER][ACCOUNT_PASSWORD]):
...
def main():
    ...
    def save_user():
        ...
        if passwd == passwd_again and username and passwd and passwd_again:
            ...
            mes = database.add_user(username, password_hash_string)
            message = QMessageBox()
    ...
    main_window.show_registration_button.triggered.connect(
