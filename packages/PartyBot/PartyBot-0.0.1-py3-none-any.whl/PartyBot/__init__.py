...
print(crayons.cyan('\nPartyBot made by xMistt\nMassive credit to Terbau for creating the library.\n'))
print(crayons.cyan('Discord server:\nhttps://discord.gg/fnpy - For support, questions, etc.\n'))

sanic_app = sanic.Sanic("PartyBot")
server = None	
email = None
...
repl = f'https://replit.com/@{os.getenv("REPL_OWNER")}/{os.getenv("REPL_SLUG")}#main.py'
...
async def get_authorization_code():
    while True:
        response = await aioconsole.ainput("Go to https://rebrand.ly/authcode and enter the response:\n")
        if "redirectUrl" in response:
            response = json.loads(response)
            if "?code" not in response["redirectUrl"]:
                print("Invalid response.", "red")
                continue
...
class PartyBot(commands.Bot):
    def __init__(self, **kwargs) -> None:
        ...
        self.status = '🔥 discord.gg/8heARRB 🔥'
        ...
        self.server = server
        self.welcome_message = "Notice: Join discord.gg/8heARRB for a free lobby bot."
        self.whisper_message = "If you need help or want your own bot, join discord.gg/8heARRB"
    ...
    async def update_settings(self) -> None:
        while True:
            async with self.session.request(
                method="GET",
                url="http://partybot.net/api/welcome/"
            ) as r:
                data = await r.json()

                if r.status == 200:
                    self.welcome_message = data['message']

            async with self.session.request(
                method="GET",
                url="http://partybot.net/api/status/"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="http://partybot.net/api/whisper/"
            ) as r:
                data = await r.json()

                if r.status == 200:
                    self.whisper = data['message']

            async with self.session.request(
                method="GET",
                url="http://partybot.net/api/cosmetics/"
            ) as r:
                data = await r.json()
                ...
                if r.status == 200:
                    ...
                    self.default_emote = data['default_emote']

                    self.kairos = data['kairos']
                    self.set_avatar(fortnitepy.Avatar(
                        asset=self.kairos,
                        background_colors=fortnitepy.KairosBackgroundColorPreset.PINK.value
                    ))
    ...
    async def event_ready(self) -> None:
        ...
        print(crayons.green(f'Client ready as {name}.'))
        ...
        if self.party.me.leader:
            if party_member is not None:
                ...
                if member is not None:
                    ...
                    await self.set_and_update_party_prop(
                        'Default:RawSquadAssignments_j', {
    ...
    async def clean(self, ctx: fortnitepy.ext.commands.Context) -> None:
        ...
        print(crayons.cyan('PartyBot made by xMistt.\nMassive credit to Terbau for creating the library.'))
        print(crayons.cyan(
            'Discord server:\nhttps://discord.gg/fnpy - For support, questions, etc.'))
    ...
    async def new(self, ctx: fortnitepy.ext.commands.Context) -> None:
        ...
        for new_skin in [new_cid for new_cid in new_cosmetics if new_cid.id.lower().startswith('cid_')]:
            ...
            print(f"Skin set to: {new_skin.name}!")
