...
def establish_link(dirname,roots,roots_videos):
    ...
    for vfile in p.glob('*'):
        ...
        tags = set([i[1:-1] for i in yxsfile.yxsFile(vfile).decode_filename().stem.split('.') if i.startswith('[') and i.endswith(']') and i[1:-1].find(']') == -1])
        tags.add('ALL')
        for t in tags:
            if t not in roots:
...
def generate_root_index_html(dirname,encrypt):
    ...
    fp = StringIO()
    fp.write(poster_html.replace('{title}',p.name))
    albums = [(yxsfile.yxsFile(i).decode_filename().stem,i) for i in p.glob('*') if i.is_dir()]
    ...
    for alb in albums:
        ...
        if user_cover.exists():
            ...
            jpgs = list((alb[1] / poster_dir_name).glob('*'))
            if jpgs:
                infojpg = f'{real_name}/{poster_dir_name}/{jpgs[0].name}'
            else: 
                infojpg = ''
...
if __name__=='__main__':
    sys.argv.extend(['./video','--no_encrypt'])
    main(None,None)