...
from .base import BaseObject, BaseLayer, BasePage
...
class Axis:
    ...
    def sfrom(self):
        ...
        return self.layer.GetNumProp('{0}.from'.format(self.ax))
    @sfrom.setter
    def sfrom(self, value):
        """set scale limit from value"""
        self.layer.SetNumProp('{0}.from'.format(self.ax), value)

    @property
    def sto(self):
        """get scale limit to value"""
        return self.layer.GetNumProp('{0}.to'.format(self.ax))
    @sto.setter
    def sto(self, value):
        """set scale limit to value"""
        self.layer.SetNumProp('{0}.to'.format(self.ax), value)

    @property
    def sstep(self):
        """get scale limit increment"""
        return self.layer.GetNumProp('{0}.inc'.format(self.ax))
    @sstep.setter
    def sstep(self, value):
        """set scale limit increment"""
        self.layer.SetNumProp('{0}.inc'.format(self.ax), value)
...
class GLayer(BaseLayer):
    ...
    def group(self, group=True, begin=-1, end=-1):
        ...
        script = (f'layer -g{"" if group else "u"} '
                f'{"" if begin < 0 else begin} '
                f'{"" if not group or end < 0 else end}')
