...
def _tree_node_get_attributes_key_prefix():
    return '___'    # something unlikely to be a real node's tag name prefix'

def _tree_node_get_attributes_key_for_root():
    return 'root'

def _tree_dict_is_key_attributes(key):
    strNeededPrefix = _tree_node_get_attributes_key_prefix()
    lenNeededPrefixLength = len(strNeededPrefix)
    if len(key) <= lenNeededPrefixLength:
        return False
    strPrefix = key[:lenNeededPrefixLength]
    if strNeededPrefix == strPrefix:
        return True
    return False
...
def _dict_to_xml_element(dd, parent, bAttributes):
    for k, v in dd.items():
        if _tree_dict_is_key_attributes(k):
            pass        # will do it later when requested or already done,
                        # see the call to _dict_to_xml_element_atts(dd, child, k) below
        else:
...
def dict_to_xml(dd, bAttributes = False):
    """
...
def lt_dict_to_tree(dd, treevarname, bAddTreeVar = True, bAttributes = False):
    """
    ...
    """
    strxml = dict_to_xml(dd, bAttributes)
    ...
    if bAddTreeVar:
        po.LT_execute(f'tree {treevarname}')
    strLTexecute = (f'doc.settreevar({treevarname};'
...
def lt_tree_to_dict(name, bAttributes=False):
    """
    ...
    with LTTMPOUTSTR(name) as pp:
        ...
        dd = {}
        for node in tr:
            _tree_to_dict(dd, node, bAttributes)
