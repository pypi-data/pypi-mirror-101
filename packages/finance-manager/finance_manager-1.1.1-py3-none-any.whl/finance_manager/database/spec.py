...
class curr_nonp(Base):
    """
    Nonpay costs generated by the curriculum model

    Table exists as a buffer between the CM and FM, so that the FM login 
    doesn't need full permissions for selecting on the CM. 

    Attributes
    ----------
    set_id : int
        ID of the set to which the nonpay belongs.
    account : str
        4 character account ID. 
    amount : float
        Curriculum nonpay cost for this set & account.
    """
    __tablename__ = "curriculum_nonp"

    set_id = Column(INTEGER(), ForeignKey("f_set.set_id"),
                    primary_key=True)
    account = Column(CHAR(4), ForeignKey("fs_account.account"),
                     primary_key=True)
    amount = Column(_FDec, nullable=False)
