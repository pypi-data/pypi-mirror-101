...
class DerivedDataApi(metaclass=Singleton):
    ...
    def get_index_pair(self):
        index_val = {'PE':'pe_ttm','PB':'pb_mrq','ROE':'roe'}
        return index_val
    ...
    def get_fof_nav_public(self, manager_id: str, fof_id_list: Tuple[str] = ()):
        with DerivedDatabaseConnector().managed_session() as session:
            try:
                query = session.query(
                    FOFNavPublic,
                ).filter(
                    FOFNavPublic.manager_id == manager_id,
                )
                if fof_id_list:
                    query = query.filter(
    ...
    def delete_hedge_fund_custodian_data(self, id_to_delete: int) -> bool:
        with DerivedDatabaseConnector().managed_session() as session:
            try:
                session.query(
                    HedgeFundCustodianData
                ).filter(
                    HedgeFundCustodianData.id == id_to_delete,
                ).delete(synchronize_session=False)
                session.commit()
                return True
            except Exception as e:
    ...
    def get_all_industry_val(self, begin_date, end_date, time_para):
        try:
            raw_api = RawDataApi()
            begin_date, end_date = raw_api.get_date_range(time_para, begin_date, end_date)
            dic = {
        ...
        try:
            ...
            for col in df:
                if col != '均值':
                    ...
                    index_list = df_i[df_i['con'] != df_i['last_con']].index.tolist()
                    _result = []
                    for _dt in index_list:
                        idx = index_list.index(_dt) + 1
                        if idx < len(index_list):
                            next_dt = index_list[idx]
                        else:
                            next_dt = df_i.index[-1]
                        df_part = df_i.loc[_dt:next_dt].dropna()#.iloc[:-1].dropna()
                        if df_part.empty:
                            continue
                        df_last_day = df_part.index[-1]
                        if df_last_day == index_risk.index[-1]:
                            pass
                        else:
                            next_day = index_risk.loc[df_last_day:].index[1]
                            next_value = index_risk.loc[next_day,'risk_value']
                            df_part.loc[next_day,col] = next_value
                            _result.append(df_part)
                    if len(_result) > 0:
                        _df_result = pd.concat(_result)[[col]].sort_index()
                        result.append(_df_result)
                    else:
                        empty_col.append(col)
                else:
                    result.append(df[[col]].copy())
            df = pd.concat(result)
            for i in empty_col:
                df.loc[:,i] = None
