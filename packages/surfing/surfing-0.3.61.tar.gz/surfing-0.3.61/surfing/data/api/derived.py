...
class DerivedDataApi(metaclass=Singleton):
    ...
    def delete_fund_indicator_with_dt(self, date_list):
        with DerivedDatabaseConnector().managed_session() as session:
            try:
                session.query(
                    FundIndicator
                ).filter(
                    FundIndicator.datetime.in_(date_list),
    ...
    def get_industry_stock_ret_and_size(self, begin_date, end_date, time_para):
        try:
            def process_x(x):
                x.loc[:,'weight'] = x.total_share / x.total_share.sum()
                x = x[x.weight >= 0.015]
                return x
            # 建议只用短时间，上时间慢
            raw_api = RawDataApi()
            ...
            groupby_df = stock_df.join(total_share).copy()
            stock_df = groupby_df.groupby('industry_2').apply(lambda x: process_x(x))
