...
class BasicDataApi(metaclass=Singleton):
    ...
    def get_rzrqye(self, begin_date, end_date, time_para):
        try:
            raw = RawDataApi()
            begin_date, end_date = raw.get_date_range(time_para, begin_date, end_date)
            df = raw.get_em_macroeconomic_daily(codes=['rzrqye'],start_date=begin_date,end_date=end_date)
            df = df.set_index('datetime').rename(columns={'value':'融资融券余额(亿元)'})[['融资融券余额(亿元)']]
