...
class DerivedDataApi(metaclass=Singleton):
    ...
    def get_index_val_info(self):
        dic = {
            'A股指数PE估值':{
                'index_ids':['sse50','hs300','csi500','gem_50'],
                'val':'pe_ttm',
            },
            'A股指数PB估值':{
                'index_ids':['sse50','hs300','csi500','gem_50'],
            ...
            '港股指数PE估值':{
                'index_ids':['hsi','hsi_stateown','hsi_ita','hsi_china25','hsi_mssize','hsi_shkscne','hsi_intertec'],
                'val':'pe_ttm',
            },
            '港股指数PB估值':{
                'index_ids':['hsi','hsi_stateown','hsi_ita','hsi_china25','hsi_mssize','hsi_shkscne','hsi_intertec'],
            ...
            '风格指数PE估值':{
                'index_ids':['csi300_growth','csi300_value','csi800_value','csi800_growth'],
                'val':'pe_ttm',
            },
            '风格指数PB估值':{
                'index_ids':['csi300_growth','csi300_value','csi800_value','csi800_growth'],
            ...
            '行业指数PE估值':{
                'index_ids':BasicDataApi().get_asset_info_by_type(['行业指数']).real_id.tolist(),
                'val':'pe_ttm',
            },
            '行业指数PB估值':{
                'index_ids':BasicDataApi().get_asset_info_by_type(['行业指数']).real_id.tolist(),
            ...
            '主题指数PE估值':{
                'index_ids':BasicDataApi().get_asset_info_by_type(['主题指数']).real_id.tolist(),
                'val':'pe_ttm',
            },
            '主题指数PB估值':{
                'index_ids':BasicDataApi().get_asset_info_by_type(['主题指数']).real_id.tolist(),
    ...
    def get_index_val_detail(self):
        index_group = ['A股指数','港股指数','风格指数','行业指数','主题指数']
        index_val = ['PE','PB']
        data = {'index_group':index_group,'index_val':index_val}
        return data

    def get_index_valuation_with_period(self, begin_date, end_date, time_para, index_group='A股指数', index_val='PE'):
        data_type = f'{index_group}{index_val}估值'
        inputs_dic = self.get_index_val_info()
        index_ids = inputs_dic[data_type]['index_ids']
        columns = [inputs_dic[data_type]['val']]
        ...
        with DerivedDatabaseConnector().managed_session() as mn_session:
            try:
                ...
                index_info = BasicDataApi().get_index_info(index_ids)
                ...
                df.loc['中位数',:] = df.median()
                df.loc['最大值',:] = df.max()
                df.loc['最小值',:] = df.min()
                df.loc['现值',:] = df.ffill().iloc[-1]
    ...
    def get_index_valuation_last_date(self, index_ids=[]):
    ...
    def delete_fof_nav(self, date_to_delete: datetime.date, manager_id: str, fof_id_list: List[str]) -> bool:
        with DerivedDatabaseConnector().managed_session() as session:
            try:
                session.query(
                    FOFNav
    ...
    def get_fof_nav_calc(self, fof_id_list: Tuple[str] = ()):
        with DerivedDatabaseConnector().managed_session() as session:
            try:
                query = session.query(
                    FOFNavCalc,
    ...
    def delete_fof_nav_calc(self, date_to_delete: datetime.date, fof_id_list: List[str]) -> bool:
        with DerivedDatabaseConnector().managed_session() as session:
            try:
                session.query(
                    FOFNavCalc
                    ...
                    FOFNavCalc.fof_id.in_(fof_id_list),
                    FOFNavCalc.datetime == date_to_delete,
                ).delete(synchronize_session=False)
                session.commit()
                return True
            except Exception as e:
    ...
    def get_industry_stock_ret_and_size(self, begin_date, end_date, time_para):
        try:
            # 建议只用短时间，上时间慢
            raw_api = RawDataApi()
            ...
            industry_dic = industry_df.set_index('em_id').to_dict()['ind_name']
            stock_df.loc[:,'industry_1_name'] = stock_df.industry_1.map(industry_dic)
            stock_df.loc[:,'industry_2_name'] = stock_df.industry_2.map(industry_dic)
            ...
            stock_price = raw_api.get_em_stock_price(begin_date=begin_date, end_date=end_date,stock_list=stock_list,columns=['close'])
            ...
            stock_df = stock_df.join(size_df[['size']]).dropna()
    ...
    def get_all_industry_val(self):
        try:
            ...
            asset_name = index_info.set_index('real_id').to_dict()['asset_name']
            df = self.get_index_valuation_last_date(index_ids=index_ids)
    ...
    def index_val_history(self, begin_date, end_date, time_para, index_id, valuation_type):
        dic = {
            'pb_mrq':'PB',
            'pe_ttm':'PE',
        }
        try:
            index_ids = [index_id]
            begin_date, end_date = RawDataApi().get_date_range(time_para, begin_date, end_date)
            index_val = self.get_index_valuation_develop(index_ids, begin_date, end_date)
            ...
            df.loc[:,'均值'] = df[valuation_type].rolling(window=df.shape[0], min_periods=10).mean()
    ...
    def stock_debt_val_info(self):
        ...
        index_info = [[r.index_id, r.desc_name] for r in index_info.itertuples()]
        valuation_type = [['PE','风险溢价'],['DY','股息率溢价']]
        data = {
    ...
    def stock_debt_val_detail(self, index_id, valuation_type):
        try:
            dic = {
                'pe_ttm':'PE',
                'dy':'DY',
            ...
            bond_yield = RawDataApi().get_em_macroeconomic_daily(codes=['TB_10Y'])
    ...
    def _industry_beta(self):
        try:
            index_info = BasicDataApi().get_asset_info_by_type(['行业指数'])
            index_ids = index_info.real_id.tolist()
            df = DerivedDataApi().get_index_valuation_develop_without_date(index_ids=index_ids)
            ...
            _df = _df.join(index_price).reset_index()
            return _df
        except Exception as e:
            print(f'failed to get data <err_msg> {e} from derived.stock_debt_val_detail')
            return False