...
def set_motion_limits(data, logfile, threshold=False, verbose=False):
    ...
    logfile['start_frame'] = np.nonzero(moving)[0][0] - 1
    logfile['end_frame']   = np.nonzero(moving)[0][-1]
...
def set_angles_from_limits(logfile, max_angle=360):
    ...
    angles[logfile['start_frame']:logfile['end_frame']] = np.linspace(0,max_angle,logfile['end_frame']-logfile['start_frame'])
    logfile['angles'] = angles

    return logfile


def normalise_rotation(fg_data, fg_logfile, bg_data, bg_logfile):
    """
    ...
    bg_angles = bg_logfile['detector']['angles']

    for i in range(num_fg_frames):
        fg_angle = fg_logfile['detector']['angles'][i]
        j = np.argmin( np.abs ( fg_angle - bg_angles ) )

        normalised_data[i] = fg_data[i]/bg_data[j]
