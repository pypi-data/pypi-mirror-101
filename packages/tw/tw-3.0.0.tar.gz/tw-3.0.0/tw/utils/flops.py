...
def _deconv_counter_hook(m, inputs, outputs):
  ...
  conv_per_position_flops = kernel_height * kernel_width * in_channels * filters_per_channel  # nopep8
  active_elements_count = batch_size * input_height * input_width
  m.__insight__.FLOPs += conv_per_position_flops * active_elements_count
  m.__insight__.Params = filters_per_channel * in_channels * kernel_height * kernel_width  # nopep8
...
def _conv_counter_hook(m, inputs, outputs):
  ...
  filters_per_channel = out_channels // groups
  conv_per_position_flops = np.prod(kernel_dims) * in_channels * filters_per_channel  # nopep8
  active_elements_count = batch_size * np.prod(output_dims)
  m.__insight__.FLOPs += conv_per_position_flops * active_elements_count
  m.__insight__.Params = filters_per_channel * in_channels * np.prod(kernel_dims)  # nopep8
