...
class DynasaurDefinitions:
    ...
    def get_ids_from_name(self, name, data_container, binout_data_type):
        """get ids from given name

        Args:
          name: param data_container:
          binout_data_type: return dict(name):
          data_container: 
        ...
        if name not in d.keys() or len(d) == 0:
            self._logger.emit(LOGConstants.ERROR[0], "\"" + name + "\" is not defined for " + plugin_name)
            time.sleep(.5)
            return []
