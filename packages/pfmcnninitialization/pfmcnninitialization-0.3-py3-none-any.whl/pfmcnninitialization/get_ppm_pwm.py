...
#获取pfm
def get_pfm(taxonomic_groups=str('plants')):
    pre_pfm = pd.read_csv(str('./data/' + taxonomic_groups + '.txt'),
                          sep='\t',
                          header=None)
    pfm = []
    ...
    print('There are '+str(len(pfm))+ ' PFMs in '+str(taxonomic_groups)+str('!'))
...
def filter_initialization_matrix(taxonomic_groups='plants',
                                 filters=64,
                                 L_=8,
                                 pattern='ppm_rp25',
