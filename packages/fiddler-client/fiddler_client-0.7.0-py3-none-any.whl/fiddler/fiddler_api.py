...
from .aws_utils import validate_s3_uri_access
...
class FiddlerApi:
    ...
    def __getattr__(self, function_name):
        ...
        def method(*args, **kwargs):
            ...
            error_msg = (
                ...
                f'Please consult Fiddler documentation at `https://docs.fiddler.ai/api-reference/python-package/`'
            )
    ...
    def _abort_dataset_upload(
        ...
        if warn_and_query:
            LOG.warning(
                ...
                f'https://docs.fiddler.ai/components/#dataset'
            )
    ...
    def get_slice(
        ...
        """
        payload = dict(sql=sql_query, project=project_id)
    ...
    def upload_dataset(
        ...
        if info:
            ...
            for column in info.columns:
                ...
                if info_outdated:
                    raise ValueError(
                        'Dataset info is outdated. Please update using '
                        'fdl.DatasetInfo.update_stats_for_existing_schema() '
                        'and upload datasets with the updated dataset info.'
                    )
    ...
    def get_mutual_information(self, project_id: str, dataset_id: str, features):
        """
    ...
    def publish_events_batch(  # noqa: C901
        self,
        project_id: str,
        model_id: str,
        ...
        :param project_id:    The project to which the model whose events are being published belongs
        :param model_id:      The model whose events are being published
        :param batch_source:  Batch object to be published. Can be one of: Pandas DataFrame, CSV file, or Parquet file
        ...
        :param timestamp_field:     Column to extract timestamp value from.
                              Timestamp must be UTC in format: `%Y-%m-%d %H:%M:%S.%f` (e.g. 2020-01-01 00:00:00.000000)
                              or as the timestamp of the event in epoch milliseconds (e.g. 1609462800000)
        :param default_timestamp:    Default timestamp to use if ts_column not specified.
                              Timestamp must be UTC in format: `%Y-%m-%d %H:%M:%S.%f` (e.g. 2020-01-01 00:00:00.000000)
                              or as the timestamp of the event in epoch milliseconds (e.g. 1609462800000)
        ...
        if timestamp_field is not None:
            ...
            if default_timestamp is None:
                default_timestamp = int(round(time.time() * 1000))
            payload['default_timestamp'] = default_timestamp
        ...
        if data_source == BatchPublishType.DATAFRAME:
            try:
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    # Overwrite CSV with current batch
                    df.to_csv(log_path, index=include_index)
                ...
                print(
                    'ERROR: Something went wrong; please double check `data_source` to ensure that the `batch_source` '
                    'object matches what is specified by the `data_source`'
                )
        ...
        if data_source == BatchPublishType.AWS_S3:
            ...
            # Validate S3 creds
            validate_s3_uri_access(s3_sri_path, auth_context, throw_error=True)
    ...
    def register_model(
        ...
        if outputs_available:
            ...
            try:
                ...
                self.trigger_pre_computation(project_id, model_id, dataset_id)
            except Exception as e:
                print(
