...
LastEditTime: 2021-04-09 23:03:54
LastEditors: SongJ
'''
...
def show_result_3d(ori_nc,extent,label,path=''):
    '''
    ...
    times = []
    for i in range(len(data)):
        lons.append(float(ori_nc.lon[int(data[i,1])].values))
        lats.append(float(ori_nc.lat[int(data[i,0])].values))
    ...
    draw_geo3dscatter(lons,lats,times,colors[i%len(colors)],extent,path)
    pass

def draw_geo3dscatter(lons,lats,times,label_color,extent,path=''):
