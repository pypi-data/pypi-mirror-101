...
class Basebot:
    ...
    def setup_jqe(self):
        ...
        id=random.randint(1,100000000000000000)
        while self._jobqueue.has_key(id)==True:
            id=random.randint(1,100000000000000000)
        self._jobqueue.update({id,self.updater.job_queue})
        return id
    def jqe_run_repeating(self,function,date,id):
        ...
        if self._jobqueue.has_key(id):
            self._jobqueue[id].run_repeating(function,date)
        else:
            self.log.error('Invalid Job id')
    def jqe_kill(self,id):
        if self._jobqueue.has_key(id):
            self._jobqueue[id].stop()
            del(self._jobqueue[id])
        else:
            self.log.error('Invalid Job id')
