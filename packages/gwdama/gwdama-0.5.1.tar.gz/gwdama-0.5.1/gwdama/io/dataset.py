...
def data(self):
    ...
    if 'unit' in self.attrs:
        if isinstance(self.attrs['unit'], apu.Quantity):
            return self[...] * self.attrs['unit']
        elif isinstance(self.attrs['unit'], str):
            return self[...] * apu.Quantity(1,self.attrs['unit'])
        else:
            return self[...]
    else:
        return self[...]
...
def plot(self, figsize=(10,3), ax=None, epoch=None, scale=None, closefig=True, **axkwgs):
    ...
    **axkwgs : dict, optional
    ...
    # Define x axis
    xt = self.times - self.attrs.get('t0', (epoch or 0))*apu.s
    ...
    axh.set(xlabel=f"Time [{scale}] form {from_gps(self.attrs.get('t0', (epoch or 0))).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]} ({self.attrs.get('t0', (epoch or 0))})",
            label=self.attrs.get('unit',''), xlim=(xt[0].value,xt[-1].value), title=self.attrs.get('channel',''))
    axh.set(**axkwgs)
    axh.grid(True)
...
def psd(self, fftlength=None, overlap=None, fs=None, return_type='dataset', dts_key=None, **psdkwgs):
    ...
    **psdkwargs : dict, optional
    ...
    if return_type == 'array':
        # The output array has units
        return freq *apu.Hz, Pxx * (self.attrs.get('unit', '1')/apu.Hz)
