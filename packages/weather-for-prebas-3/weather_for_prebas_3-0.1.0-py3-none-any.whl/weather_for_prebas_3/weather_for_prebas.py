...
sys.path.append(os.path.join(sys.path[-1], "weather_for_prebas_3"))
try:
    from sites import create_sites_file
except ModuleNotFoundError:
    for path in sys.path:
        if "weather_for_prebas_3" in os.listdir(path):
            sys.path.append(os.path.join(path, "weather_for_prebas_3"))
            from sites import create_sites_file
            break
