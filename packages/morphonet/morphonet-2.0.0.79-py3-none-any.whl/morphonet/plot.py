...
class MorphoInfo():
    ...
    def inactivate_using_daughter(self,a_daughter):
        if a_daughter.mother in self.data:
            for val in self.data[a_daughter.mother]:
                if type(val.value) == list or type(val.value) == dict:
                    for item in val.value:
                        if item == a_daughter:
                            val.value.remove(item)
                else :
                    if val.value == a_daughter:
                        self.data[a_daughter.mother].remove(val)
...
class Dataset():
        ...
        if filename is not None:
            ...
            for info_name in self.infos:
                ...
                if info_name == "cell_lineage":
                    line_infos = inf.get_dict()
                    f2 = open("new_info.txt", "w")
                    f2.write(inf.get_txt())
                    f2.close()
                    f3 = open("new_info_dict.txt", "w")
    ...
    def del_link(self,o): #We remove all links correspond to a cells
        ...
        f.write(inf.get_txt())
        f.close()
        f1 = open("prev_info_dict.txt","w")
        for line in line_infos:
            f1.write(str(line)+"\n")
        f1.close()
        #remove currations
        for mc in inf.get_curations(o.mother):
            if mc.value==o:
                print("deleted link : "+str(o.get_name())+" -> "+o.mother.get_name())
                mc.active=False

        for d in o.daughters:
            for mc in inf.get_curations(d):
                if mc.value == o:
                    print("deleted link : " + str(o.get_name()) + " -> " + d.get_name())
                    mc.active = False

        inf.inactivate_using_daughter(o)
        #remove hard links (originals)
        o.del_links()
        #
        inf.inactivate_curations(o)

        #inf is updated, so write
        #line_infos = inf.get_dict()
        #f2 = open("new_info.txt","w")
        #f2.write(inf.get_txt())
        #f2.close()
