...
class Kubedev:
  ...
  def _deploy(
    ...
    if docker_network is not None:
      ...
      print()
      print(f'  docker run -it --rm --network {docker_network} --volume "{normalized_kubeconfig}:/tmp/kube_config" bitnami/kubectl --kubeconfig /tmp/kube_config <your-commands>')
      print()
      command = [
        "/bin/sh",
        "-c",
  ...
  def system_test_from_config(self, kubedev, appName: str, reuseCluster: str, file_accessor, env_accessor, shell_executor, tag_generator, sleeper) -> bool:
      ...
      try:
          ...
          if reuseCluster is not None:
            ...
            print(f'  docker run -it --rm --network {network} --volume "{wslClusterConfig}:/tmp/kube_config" bitnami/kubectl --kubeconfig /tmp/kube_config <your-commands>')
