...
#        self.cfg["redis"]["ipaddress"] = self.handler.cfg["ipaddress"]
    ...
    def _generic_config(self):
        ...
        if not self.cfg["redis"]["is_deploy"]:
            self.cfg["redis"]["ipaddress"] = self.cfg["redis"]["ipaddress"]
        else:
            self.cfg["redis"]["ipaddress"] = self.handler.cfg["ipaddress"]
    ...
    def init_mysql(self):
        if not self.cfg["mysql"]["is_deploy"]:
            ...
            print(self.master_host,"1")
        print(self.master_host,self.port,self.user,self.password)
