...
class HoloWAN:
    ...
    @checkParameter()
    def upload_recorder(self, holowan_ip: str, holowan_port: str, filePath: str):
        '''
        :param holowan_ip:
        :param holowan_port:
        :param filePath: 文件路径
        :return: xml
        '''
        requestURL = "http://{0}:{1}/recorder_upload".format(holowan_ip, holowan_port)
        fileType = mt.getFileType(filePath)
        self.remove_recorder_information(holowan_ip, holowan_port)       # 清空recorder文件数据
        if fileType == "txt":
            fileName = ntpath.basename(filePath)
            with open(filePath, "rb") as file:
                request_file = {str(len(open(filePath, "rb").read())): (fileName, file, "text/plain")}
                return requests.post(requestURL, files=request_file).text
        elif fileType == "db":
            fileName = ntpath.basename(filePath)
            with open(filePath, "rb") as file:
                request_file = {str(len(open(filePath, "rb").read())): (fileName, file, "application/octet-stream")}
                return requests.post(requestURL, files=request_file).text
        else:
            return r'{"errCode":"-400","errMsg":"error","errReason":"上传文件必须为db格式或txt格式"}'

    # 获取上传Recorder文件信息
    @checkParameter()
    def get_recorder_information(self, holowan_ip: str, holowan_port: str):
        '''
        :param holowan_ip: holowan IP地址
        :param holowan_port: holowan 端口号
        :return: xml
    ...
    # 配置recorder指定引擎Engine的虚拟链路Path
    @checkParameter()
    def set_recorder_config(self, holowan_ip: str, holowan_port: str, server_id: int, engine_id: int, path_id: int, loss_base: int):
        # loss_base取值错误
        if str(loss_base) not in self.packetLossCycleList:
            return r'{"errCode":"-500","errMsg":"ERROR","errReason":"Error parameter values(loss_base)"}'

        # 判断是否存在这个server_id
        serverIDList = []   # recorder能用的server_id
        recorderInfoString = self.get_recorder_information(holowan_ip, holowan_port)
        if recorderInfoString == r'{"errCode":"-3","errMsg":"Process Recoder File Error","errReason":"Device Not Exists Recoder File"}':
            return recorderInfoString
        infoRoot = xt.xmlString_to_Object(recorderInfoString)
        serverNodes = xt.get_nodes(infoRoot, "./server")
        for serverNode in serverNodes:
            serverIDList.append(int(xt.get_node(serverNode, "index").text)+1)
        if server_id not in serverIDList:
            return r'{"errCode":"-500","errMsg":"ERROR","errReason":"Error parameter values(server_id)"}'

        # 判断Engine id
        if self.has_engine(holowan_ip, holowan_port, engine_id) is False:
            return r'{"errCode":"-5","errMsg":"ERROR","errReason":"range of engine param"}'

        # 判断Path id是否合理
        if self.right_path(holowan_ip, holowan_port, engine_id, path_id) is False:
            return r'{"errCode":"-6","errMsg":"ERROR","errReason":"range of path param"}'

        # 判断Path id是否存在
        if self.has_path(holowan_ip, holowan_port, engine_id, path_id) is False:
            return r'{"errCode":"-7","errMsg":"ERROR","errReason":"remove config from xml"}'

        # 查看配置情况
        recorderConfString = self.get_recorder_config(holowan_ip, holowan_port)
        # 无任何配置
        if recorderConfString == r'{"errCode":"-3","errMsg":"Process Recoder File Error","errReason":"Device Not Exists Recoder File"}':
            # 读取配置文件，获取xml格式，并生成XMLObject对象
            confRoot = xt.get_node(xt.xmlFile_to_Object(self.recorder_config_fileName), ".")
            node = xt.get_node(confRoot, "./config")
            xt.set_node_text(xt.get_node(node, "server_id"), server_id)
            xt.set_node_text(xt.get_node(node, "engine_id"), engine_id)
            xt.set_node_text(xt.get_node(node, "path_id"), path_id)
            xt.set_node_text(xt.get_node(node, "loss_base"), loss_base)
            return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.recorder_config_api),
                                        xt.xmlObject_to_string(confRoot))
        # 有配置
        else:
            confRoot = xt.xmlString_to_Object(recorderConfString)
            # 配置有重复，删除重复配置
            for configNode in xt.get_nodes(confRoot, "./config"):
                serverNode = xt.get_node(configNode, "server_id")
                engineNode = xt.get_node(configNode, "engine_id")
                pathNode = xt.get_node(configNode, "path_id")
                if server_id == int(serverNode.text) or (engine_id == int(engineNode.text) and path_id == int(pathNode.text)):
                    self.remove_recorder_config(holowan_ip, holowan_port, engineNode.text, pathNode.text)
                    self.remove_recorder_config(holowan_ip, holowan_port, engine_id, path_id)
                    return self.set_recorder_config(holowan_ip, holowan_port, server_id, engine_id, path_id, loss_base)
            # 配置无重复，新增配置
            confNode = xt.create_node("config", {}, {})
            children_node_Map = {"server_id": server_id, "engine_id": engine_id, "path_id": path_id, "loss_base": loss_base}
            xt.add_children(confNode, children_node_Map)
            confRoot.append(confNode)
            return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.recorder_config_api),
                                        xt.xmlObject_to_string(confRoot))

    # 获取最有一次Recorder配置参数
    @checkParameter()
    def get_recorder_config(self, holowan_ip: str, holowan_port: str):
        '''
        :param holowan_ip: holowan IP地址
        :param holowan_port: holowan 端口号
        ...
        try:
            ...
            return r'{"errCode":"-3","errMsg":"Process Recoder File Error","errReason":"Device Not Exists Recoder File"}'

    # 删除上传的Recorder文件信息
    @checkParameter()
    def remove_recorder_information(self, holowan_ip: str, holowan_port: str):
        '''
        :param holowan_ip: holowan IP地址
        :param holowan_port: holowan 端口号
        :return:
        '''
        requestURL = "http://{0}:{1}/cancel_recorder?type=1".format(holowan_ip, holowan_port)
        return requests.get(requestURL).text

    # 删除Recorder配置信息
    def remove_recorder_config(self, holowan_ip:str, holowan_port: str, engineID: int, pathID: int):
        '''
        :param holowan_ip: holowan IP地址
        :param holowan_port: holowan 端口号
        :param engineID: 引擎ID
        ...
        :return:
        '''
        requestURL = "http://{0}:{1}/cancel_recorder?type=2&engine={2}&path={3}".format(holowan_ip, holowan_port, engineID, pathID)
        return requests.get(requestURL).text
...
class SubHoloWAN(HoloWAN):
    ...
    def set_BER(self, holowan_ip: str = "default", holowan_port: str = "default", engineID: int = 1,
                pathID: int = 1, pathDirection: int = 1,
