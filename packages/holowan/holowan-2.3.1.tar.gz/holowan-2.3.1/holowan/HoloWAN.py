            ...
            for arg, param in zip(args, va):
                if param.name == "matchType ":
                    if str(arg) not in self.matchTypeList:
                        return '{"errCode":"-500","errMsg":"ERROR","errReason":"Error parameter values ' + param.name + '"}'
            ...
            for k, v in kwargs.items():
                if k == "matchType":
                    if str(v) not in self.matchTypeList:
                        return '{"errCode":"-500","errMsg":"ERROR","errReason":"Error parameter values ' + k + '"}'
                if k == "modifyType":
...
class HoloWAN:
    ...
    def __init__(self):
        ...
        self.normalProbabilityDecimal = self.ini.get("reordering", "normalProbabilityDecimal")
        self.normalDelayMinDecimal = self.ini.get("reordering", "normalDelayMinDecimal")
        self.normalDelayMaxDecimal = self.ini.get("reordering", "normalDelayMaxDecimal")
    ...
    # ===============================================身份验证相关接口====================================================#
    # # 身份验证
    # def login(self, holowan_ip:str, holowan_port:str, admin):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    #     :param admin: 用户名
    #     :return: 正确：{“login”: “YES”}，并在HTTP Header中增加标准的Cookie，错误：{“login”: “ERROR”}
    #     '''
    #     url = "http://{0}:{1}/login".format(holowan_ip, holowan_port)
    #     return mt.post_original_api(url, admin)
    ...
    def hold_engine(self, holowan_ip: str, holowan_port: str, engineID: int, password: int, new_password: int):
        ...
        try:
            ...
            if (isCheck or isOpen or isSet or isClean) is False:
                raise RuntimeError(r'{"errCode":"-1","errMsg":"error","errReason":"password error"}')
            ...
            return r'{"errCode":"-1","errMsg":"ConnectionError","errReason":"Failed to establish a new connection"}'
    ...
    def close_path(self, holowan_ip:str, holowan_port:str, engineID:int, pathID:int, pathName: str):
        ...
        :return: json 正确：errCode 等于 0，错误：errCode小于0（详见附录二（1.1修改虚拟链路配置错误编号））
        '''
        children_node_Map = {"modify_switch": "3", "engine_id": engineID, "path_id": pathID,
                             "path_name": pathName,
                             "if_enable": "1"}
        return self.remove_or_open_or_close_path(holowan_ip, holowan_port, engineID, pathID, pathName,
                                                 children_node_Map)
    ...
    def set_path_Bandwidth_Fixed(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
                                 pathDirection: int, rateValue: int, rateUnit: int):
        '''
        :param holowan_ip: holowan IP地址
        :param holowan_port: holowan 端口号
        :param engineID: 引擎ID
        ...
        '''
        parent_node_ID = "bd"
        children_node_Map = {"s": 1, "r": rateValue, "t": rateUnit}
        properties_Map = {}
        return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
                                                  parent_node_ID,
                                                  children_node_Map, properties_Map)

    # # 设置path带宽，Jitter抖动模式_正弦图像
    # def set_path_Bandwidth_Jitter_sine(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                    pathDirection, rateUnit, max,
    #                                    min, phase, cycle):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     '''
    #     parent_node_ID = "bd"
    #     children_node_Map = {"s": 2, "t": rateUnit,
    #                          "shake": {"max": max, "min": min, "phase": phase, "cycle": cycle}}
    #     properties_Map = {"/shake": {"type_id": "1"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置path带宽，Jitter抖动模式_斜坡图像
    # def set_path_Bandwidth_Jitter_slope(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                     pathDirection, rateUnit, max,
    #                                     min, cycle, proportion_up, proportion_down):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    #     :param engineID: 引擎ID
    ...
    #     :param min: 抖动最小值
    #     :param cycle: 抖动周期
    #     :param proportion_up: 上升曲线第一部分占比值
    #     :param proportion_down: 下降去向第一部分占比值
    #     :return: :return: Json [正确：erroCode等于0， 错误：erroCode小于0（详见附录二（1.2修改虚拟链路损伤错误编号））]
    #     '''
    #     parent_node_ID = "bd"
    #     children_node_Map = {"s": 2, "t": rateUnit,
    #                          "shake": {"max": max, "min": min, "cycle": cycle, "proportion_up": proportion_up,
    #                                    "proportion_down": proportion_down}}
    #     properties_Map = {"/shake": {"type_id": "2"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置path带宽，Jitter抖动模式_抛物线图像
    # def set_path_Bandwidth_Jitter_parabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                        pathDirection,
    #                                        rateUnit, max, min, cycle):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    #     :param engineID: 引擎ID
    ...
    #     '''
    #     parent_node_ID = "bd"
    #     children_node_Map = {"s": 2, "t": rateUnit, "shake": {"max": max, "min": min, "cycle": cycle}}
    #     properties_Map = {"type_id": "3"}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置path带宽，Jitter抖动模式_逆抛物线图像
    # def set_path_Bandwidth_Jitter_InverseParabola(self, holowan_ip: str, holowan_port: str, engineID: int,
    #                                               pathID: int, pathDirection,
    #                                               rateUnit, max, min, cycle):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     parent_node_ID = "bd"
    #     children_node_Map = {"s": 2, "t": rateUnit, "shake": {"max": max, "min": min, "cycle": cycle}}
    #     properties_Map = {"type_id": "4"}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置path带宽，Jitter抖动模式_上升折线图像
    # def set_path_Bandwidth_Jitter_ascLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                            pathDirection,
    #                                            rateUnit, max, min, cycle, proportion):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    #     :param engineID: 引擎ID
    ...
    #     :param min: 抖动最小值
    #     :param cycle: 抖动周期
    #     :param proportion: 上升或下降曲线，第一部分占比值
    #     :return: Json [正确：erroCode等于0， 错误：erroCode小于0（详见附录二（1.2修改虚拟链路损伤错误编号））]
    #     '''
    #     parent_node_ID = "bd"
    #     children_node_Map = {"s": 2, "t": rateUnit,
    #                          "shake": {"max": max, "min": min, "cycle": cycle, "proportion": proportion}}
    #     properties_Map = {"type_id": "5"}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置path带宽，Jitter抖动模式_下降折线图像
    # def set_path_Bandwidth_Jitter_descLineChart(self, holowan_ip: str, holowan_port: str, engineID: int,
    #                                             pathID: int, pathDirection,
    #                                             rateUnit, max, min, cycle, proportion):
    #     '''
    #     :param holowan_ip: holowan IP地址
    ...
    def close_Background_Utilization(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
        ...
        try:
            if pathDirection == 1:
                ...
                raise RuntimeError("Error parameter values")
            tagText = 1
            return self.set_one_tag_text(holowan_ip, holowan_port, engineID, pathID, tagPath, tagText)
    ...
    def close_message_Modify(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection: int):
        ...
        if pathDirection == 1:
            ...
            return r'{"errCode":"-500","errMsg":"ERROR","errReason":"Error parameter values"}'
        tagText = 0
        return self.set_one_tag_text(holowan_ip, holowan_port, engineID, pathID, tagPath, tagText)
    ...
    def set_message_Modify_Random(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
        ...
        if len(str(modifyRandomRate).split('.')[1]) > int(self.modifyRandomRateDecimal):
            return r'{"errCode":"-500","errMsg":"ERROR","errReason":"Error parameter values modifyRandomRate"}'
    ...
    def set_Delay_Constant(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection: int,
        ...
        :return: Json [正确：erroCode等于0， 错误：erroCode小于0（详见附录二（1.2修改虚拟链路损伤错误编号））]
        '''
        if len(str(delay).split(".")[1]) > int(self.constantDelayDecimal) or delay < 0.1:
    ...
    # # 设置时延配置  平均分布时延(Uniform)  高级设置：正弦图像
    # def set_Delay_Uniform_sine(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection,
    #                            minimum, maximum,
    #                            enableReordering, max, min, phase, cycle):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     :param pathID: 链路ID
    #     :param pathDirection: 链路损伤方向 [1.仅损伤下行，2.仅损伤上行]
    #     :param minimum: 平均分布最小值
    #     :param maximum: 平均分布最大值
    #     :param enableReordering: 是否允许时延乱序 [0.关闭, 1.开启]
    #     :param max: 抖动最大值
    #     :param min: 抖动最小值
    #     :param phase: 相位偏移值
    ...
    #                                               children_node_Map, properties_Map)
    #
    # # 设置时延配置  平均分布时延(Uniform)  高级设置：斜坡图像
    # def set_Delay_Uniform_slope(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection,
    #                             minimum, maximum,
    #                             enableReordering, max, min, cycle, proportion_up, proportion_down):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     parent_node_ID = "d"
    #     children_node_Map = {"s": "2", "un": {"dmi": minimum, "dma": maximum, "reo": enableReordering,
    #                                           "shake": {"max": max, "min": min, "cycle": cycle,
    ...
    #                                               children_node_Map, properties_Map)
    #
    # # 设置时延配置  平均分布时延(Uniform)  高级设置：抛物线图像
    # def set_Delay_Uniform_parabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                pathDirection, minimum,
    #                                maximum, enableReordering, max, min, cycle):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     children_node_Map = {"s": "2", "un": {"dmi": minimum, "dma": maximum, "reo": enableReordering,
    #                                           "shake": {"max": max, "min": min, "cycle": cycle}}}
    #     properties_Map = {"shake": {"type_id": "3"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置时延配置  平均分布时延(Uniform)  高级设置：逆抛物线图像
    # def set_Delay_Uniform_InverseParabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                       pathDirection, minimum,
    #                                       maximum, enableReordering, max, min, cycle):
    #     '''
    #
    ...
    #     children_node_Map = {"s": "2", "un": {"dmi": minimum, "dma": maximum, "reo": enableReordering,
    #                                           "shake": {"max": max, "min": min, "cycle": cycle}}}
    #     properties_Map = {"shake": {"type_id": "4"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置时延配置  平均分布时延(Uniform)  高级设置：上升折线图像
    # def set_Delay_Uniform_ascLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                    pathDirection, minimum,
    #                                    maximum, enableReordering, max, min, cycle, proportion):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     properties_Map = {"shake": {"type_id": "5"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置时延配置  平均分布时延(Uniform)  高级设置：下降折线图像
    # def set_Delay_Uniform_descLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                     pathDirection, minimum,
    #                                     maximum, enableReordering, max, min, cycle, proportion):
    #     '''
    #
    ...
    #                                           "shake": {"max": max, "min": min, "cycle": cycle,
    #                                                     "proportion": proportion}}}
    #     properties_Map = {"shake": {"type_id": "6"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    ...
    # # 设置丢包配置  Jitter抖动丢包模式 正弦图像
    # def set_Loss_Jitter_sine(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection,
    #                          max, min, phase,
    #                          cycle):
    #     '''
    #
    ...
    #     '''
    #     parent_node_ID = "l"
    #     children_node_Map = {"s": "5", "shake": {"max": max, "min": min, "phase": phase, "cycle": cycle}}
    #     properties_Map = {"shake": {"type_id": "1"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置丢包配置  Jitter抖动丢包模式 斜坡图像
    # def set_Loss_Jitter_slope(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection,
    #                           max, min, phase,
    #                           cycle, proportion_up, proportion_down):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #                                               children_node_Map, properties_Map)
    #
    # # 设置丢包配置  Jitter抖动丢包模式 抛物线图像
    # def set_Loss_Jitter_parabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                              pathDirection, max, min, cycle):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     '''
    #     parent_node_ID = "l"
    #     children_node_Map = {"s": "5", "shake": {"max": max, "min": min, "cycle": cycle}}
    #     properties_Map = {"shake": {"type_id": "3"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置丢包配置  Jitter抖动丢包模式 逆抛物线图像
    # def set_Loss_Jitter_InverseParabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                     pathDirection, max, min,
    #                                     cycle):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    ...
    #                                               children_node_Map, properties_Map)
    #
    # # 设置丢包配置  Jitter抖动丢包模式 上升折线图像
    # def set_Loss_Jitter_ascLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                  pathDirection, max, min,
    #                                  cycle, proportion):
    #     '''
    #
    ...
    #     '''
    #     parent_node_ID = "l"
    #     children_node_Map = {"s": "5", "shake": {"max": max, "min": min, "cycle": cycle, "proportion": proportion}}
    #     properties_Map = {"shake": {"type_id": "5"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置丢包配置  Jitter抖动丢包模式 下降折线图像
    # def set_Loss_Jitter_descLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                   pathDirection, max, min,
    #                                   cycle, proportion):
    #     '''
    #
    ...
    # 设置BER配置
    @checkParameter()
    def set_BER(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection: int,
        ...
        parent_node_ID = "cor"
        children_node_Map = {"ber": bitErrorValue, "beri": bitErrorIndex}
        properties_Map = {}
        return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
                                                  parent_node_ID,
                                                  children_node_Map, properties_Map)
    ...
    def set_Reordering_Normal(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection: int,
        ...
        parent_node_ID = "reo"
        children_node_Map = {"s": "1", "p": probability, "dmi": delayMin, "dma": delayMax}
        properties_Map = {}
        return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
                                                  parent_node_ID,
                                                  children_node_Map, properties_Map)
    ...
    # # 设置报文乱序配置  抖动类型  正弦图像
    # def set_Reordering_Jitter_sine(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                pathDirection, delayMin,
    #                                delayMax, max, min, phase, cycle):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     '''
    #     parent_node_ID = "reo"
    #     children_node_Map = {"s": "2", "dmi": delayMin, "dma": delayMax,
    #                          "shake": {"max": max, "min": min, "phase": phase, "cycle": cycle}}
    #     properties_Map = {"shake": {"type_id": "1"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    ...
    #
    # # 设置报文乱序配置  抖动类型  斜坡图像
    # def set_Reordering_Jitter_slope(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                 pathDirection, delayMin,
    #                                 delayMax, max, min, phase, cycle, proportion_up, proportion_down):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #     children_node_Map = {"s": "2", "dmi": delayMin, "dma": delayMax,
    #                          "shake": {"max": max, "min": min, "phase": phase, "cycle": cycle,
    #                                    "proportion_up": proportion_up, "proportion_down": proportion_down}}
    #     properties_Map = {"shake": {"type_id": "2"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置报文乱序配置  抖动类型  抛物线图像
    # def set_Reordering_Jitter_parabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                    pathDirection, delayMin,
    #                                    delayMax, max, min, cycle):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #                                               children_node_Map, properties_Map)
    #
    # # 设置报文乱序配置  抖动类型  逆抛物线图像
    # def set_Reordering_Jitter_InverseParabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                           pathDirection,
    #                                           delayMin, delayMax, max, min, cycle):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #                                               children_node_Map, properties_Map)
    #
    # # 设置报文乱序配置  抖动类型  上升折线图像
    # def set_Reordering_Jitter_ascLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                        pathDirection,
    #                                        delayMin, delayMax, max, min, phase, cycle, proportion):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #                                               children_node_Map, properties_Map)
    #
    # # 设置报文乱序配置  抖动类型  下降折线图像
    # def set_Reordering_Jitter_descLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                         pathDirection,
    #                                         delayMin, delayMax, max, min, phase, cycle, proportion):
    #     '''
    #
    ...
    # # 设置报文乱序配置  抖动类型  正弦图像
    # def set_Duplication_Jitter_sine(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                 pathDirection, max, min,
    #                                 phase, cycle):
    #     '''
    #
    ...
    #     '''
    #     parent_node_ID = "du"
    #     children_node_Map = {"s": "2", "shake": {"max": max, "min": min, "phase": phase, "cycle": cycle}}
    #     properties_Map = {"shake": {"type_id": "1"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置报文乱序配置  抖动类型  斜坡图像
    # def set_Duplication_slope(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection,
    #                           max, min, phase,
    #                           cycle, proportion_up, proportion_down):
    #     '''
    #
    ...
    #
    # # 设置报文乱序配置  抖动类型  抛物线图像
    # def set_Duplication_Jitter_parabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                     pathDirection, max, min,
    #                                     cycle):
    #     '''
    #
    ...
    #     '''
    #     parent_node_ID = "du"
    #     children_node_Map = {"s": "2", "shake": {"max": max, "min": min, "cycle": cycle}}
    #     properties_Map = {"shake": {"type_id": "3"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    #
    # # 设置报文乱序配置  抖动类型  逆抛物线图像
    # def set_Duplication_Jitter_InverseParabola(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                            pathDirection, max,
    #                                            min, cycle):
    #     '''
    #
    ...
    #
    # # 设置报文乱序配置  抖动类型  上升折线图像
    # def set_Duplication_Jitter_ascLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                         pathDirection, max,
    #                                         min, phase, cycle, proportion):
    #     '''
    #
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    ...
    #
    # # 设置报文乱序配置  抖动类型  下降折线图像
    # def set_Duplication_Jitter_descLineChart(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int,
    #                                          pathDirection, max,
    #                                          min, cycle, proportion):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    #     :param engineID: 引擎ID
    ...
    #     '''
    #     parent_node_ID = "du"
    #     children_node_Map = {"s": "2", "shake": {"max": max, "min": min, "cycle": cycle, "proportion": proportion}}
    #     properties_Map = {"shake": {"type_id": "6"}}
    #     return self.set_tag_children(holowan_ip, holowan_port, engineID, pathID, pathDirection,
    #                                               parent_node_ID,
    #                                               children_node_Map, properties_Map)
    ...
    def get_path_config_information(self, holowan_ip:str, holowan_port:str, engineID:int, pathID: int):
        ...
        '''
        try:
            requestURL = "http://{0}:{1}/path_config_info_{2}_{3}.xml".format(holowan_ip, holowan_port, engineID,
                                                                              pathID)
            return requests.get(requestURL).text
        except Exception:
            return r'{"errCode":"-400","errMsg":"ERROR","errReason":"PATH has no found"}'
    ...
    def add_IPV4_to_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int, sourceIP: str,
        ...
        :param holowan_port: holowan 端口号
        :param engineID: 引擎ID
        :param portID: 设定Port配置节点，若多个则包含多个此节点
        :param sourceIP: 匹配的IPv4的源始地址，若为任意值，则属性any值为1
        :param sourceMask: 源始地址的匹配掩码，默认32
        :param destinationIP: 匹配的IPv4的目标地址，若为任意值，则属性any值为1
        :param destinationMask: 目标地址的匹配掩码，默认32
        :param TOS: 匹配的IPv4的TOS值，若为任意值，则属性any值为1
        :param action: 命中分类后进入的虚拟链路ID编号path_id [-1:Bypass, -2:Drop, 1:PATH1, 2:PATH2, 3:PATH3, 4:PATH4, 5:PATH5, 6:PATH6]
        :return:
        '''
        try:
        ...
        if port_node is None:
            return RuntimeError("The engine {0} has no port{1}".format(engineID, portID))
        # 创建ipv4节点
        ipv4_node = self.create_IPV4(holowan_ip, holowan_port, engineID, sourceIP, sourceMask, destinationIP,
                                     destinationMask, TOS, action)
        if type(ipv4_node) == RuntimeError:
            return ipv4_node
        # 把ipv4节点添加到父节点中
        port_node.append(ipv4_node)
        pathXMLStr = xt.xmlObject_to_string(root)
    ...
    def remove_IPV4_from_Classifier(self, holowan_ip: str, holowan_port: str, engineID:int, portID: int, sourceIP: str,
        ...
        # 遍历所有的IPV4节点
        remove_flags = False
        ipv4_nodes = xt.get_nodes(port_node, "./ipv4")
        for node in ipv4_nodes:
            if xt.classifier_equal(ipv4_node, node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
            return RuntimeError("Engine{0} port{1} has not such ipv4".format(engineID, portID))
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api),
                                    pathXMLStr)
    ...
    def create_IPV4(self, holowan_ip: str, holowan_port: str, engineID:int, sourceIP: str, sourceMask: int,
        ...
        # ===============action================== #
        paths_dic = self.get_paths_from_engine(holowan_ip, holowan_port, engineID)
    ...
    def add_IPV6_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int,
        ...
        :param engineID: 引擎ID
        :param portID: 设定Port配置节点，若多个则包含多个此节点
        :param sourceIP: 匹配的IPv6的源始地址，若为任意值，则属性any值为1
        :param destinationIP: 匹配的IPv6的目标地址，若为任意值，则属性any值为1
        :param action: 命中分类后进入的虚拟链路ID编号path_id [-1:Bypass, -2:Drop, 1:PATH1, 2:PATH2, 3:PATH3, 4:PATH4, 5:PATH5, 6:PATH6]
        :return:
        '''
        try:
        ...
        # 创建ipv6节点
        ipv6_node = self.create_IPV6(holowan_ip, holowan_port, engineID, sourceIP, destinationIP, action)
        if type(ipv6_node) == RuntimeError:
            return ipv6_node
        # 把ipv6节点添加到父节点中
        port_node.append(ipv6_node)
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api),
                                    pathXMLStr)
    ...
    def remove_IPV6_from_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int,
        ...
        # 遍历所有的IPV6节点
        remove_flags = False
        ipv6_nodes = xt.get_nodes(port_node, "./ipv6")
        for node in ipv6_nodes:
            if xt.classifier_equal(ipv6_node, node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
            return RuntimeError("Engine{0} port{1} has not such ipv6".format(engineID, portID))
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api),
                                    pathXMLStr)
    ...
    def create_IPV6(self, holowan_ip:str, holowan_port:str, engineID:int, sourceIP: str, destinationIP: str, action: int):
        ...
        if sourceIP == "any":
            ...
            return RuntimeError(r"sourceIP必须输入'any'或ipv6地址")
        # ===============dst================== #
        if destinationIP == "any":
            properties["dst"] = {"any": 1}
        elif mt.isIPV6(sourceIP):
            ...
            return RuntimeError(r"destinationIP必须输入'any'或ipv6地址")
        # ===============Action================== #
        paths_dic = self.get_paths_from_engine(holowan_ip, holowan_port, engineID)
        if action not in [-1, -2] and action not in paths_dic.keys():
            return RuntimeError("action值输入错误")
    ...
    def add_MAC_to_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int, sourceMAC: str,
        ...
        :param engineID: 引擎ID
        :param portID: 设定Port配置节点，若多个则包含多个此节点
        :param sourceMAC: 匹配源MAC地址，若为任意值，则属性any值为1
        :param destinationMAC: 匹配目标MAC地址，若为任意值，则属性any值为1
        :param EtherType: 报文类型
        :param action: 命中分类后进入的虚拟链路ID编号path_id [-1:Bypass, -2:Drop, 1:PATH1, 2:PATH2, 3:PATH3, 4:PATH4, 5:PATH5, 6:PATH6]
        :return:
        '''
        try:
        ...
        if port_node is None:
            return RuntimeError("The engine {0} has no port{1}".format(engineID, portID))
        # 创建MAC节点
        MAC_node = self.create_MAC(holowan_ip, holowan_port, engineID, sourceMAC, destinationMAC, EtherType, action)
        if type(MAC_node) == RuntimeError:
            return MAC_node
        # 把MAC节点添加到父节点中
        port_node.append(MAC_node)
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api),
                                    pathXMLStr)
    ...
    def remove_MAC_from_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int, sourceMAC: str,
        ...
        '''
                ...
                :param engineID: 引擎ID
                :param portID: 设定Port配置节点，若多个则包含多个此节点
                :param sourceIP: 匹配的MAC的源始地址，若为任意值，则属性any值为1
                :param destinationIP: 匹配的MAC的目标地址，若为任意值，则属性any值为1
                :param action: 命中分类后进入的虚拟链路ID编号path_id [-1:Bypass, -2:Drop, 1:PATH1, 2:PATH2, 3:PATH3, 4:PATH4, 5:PATH5, 6:PATH6]
                :return:
                '''
        try:
        ...
        # 遍历所有的MAC节点
        remove_flags = False
        MAC_nodes = xt.get_nodes(port_node, "./mac")
        for node in MAC_nodes:
            if xt.classifier_equal(MAC_node, node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
            return RuntimeError("Engine{0} port{1} has not such MAC".format(engineID, portID))
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api),
                                    pathXMLStr)
    ...
    def create_MAC(self, holowan_ip:str, holowan_port:str, engineID:int, sourceMAC: str, destinationMAC: str, EtherType: str, action: int):
        ...
        if sourceMAC == "any":
            ...
            return RuntimeError(r"sourceIP必须输入'any'或MAC地址")
        # ===============dst================== #
        if destinationMAC == "any":
            ...
            return RuntimeError(r"destinationIP必须输入'any'或MAC地址")
        # ===============EtherType================== #
        if EtherType == "any":
            ...
            return RuntimeError(r"MAC的EtherType输入值不正确")
        # ===============Action================== #
        paths_dic = self.get_paths_from_engine(holowan_ip, holowan_port, engineID)
        if action not in [-1, -2] and action not in paths_dic.keys():
            return RuntimeError("action值输入错误")
    ...
    def add_TCP_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourcePort: str, destPort: str,
        ...
        :param holowan_port: holowan 端口号
        :param engineID: 引擎ID
        :param portID: 设定Port设置节点，若多个则包含多个此节点
        :param sourcePort: 源端口号
        :param destPort: 目标端口号
        :param checkVersion: IPV4或IPV6
        :param action: 命中分类后进入的虚拟链路ID编号path_id [-1:Bypass, -2:Drop, 1:PATH1, 2:PATH2, 3:PATH3, 4:PATH4, 5:PATH5, 6:PATH6]
        '''
        try:
            parent_node_path = "./port"
        ...
        # 创建TCP节点
        tcp_node = self.create_TCP_or_UDP(holowan_ip, holowan_port, engineID, 1, sourcePort, destPort, checkVersion, action)
        if type(tcp_node) == RuntimeError:
            return tcp_node
        # 把tcp节点添加到父节点中
        port_node.append(tcp_node)
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api), pathXMLStr)
    ...
    def remove_TCP_from_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourcePort: str, destPort: str,
        ...
        # 遍历所有TCP节点
        remove_flags = False
        tcp_nodes = xt.get_nodes(port_node, "./tcp_udp")
        for node in tcp_nodes:
            if xt.classifier_equal(tcp_node, node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
            return RuntimeError("Engine{0} port{1} has not such TCP".format(engineID, portID))
    ...
    def add_UDP_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourcePort: str, destPort: str,
        ...
        if port_node is None:
            return RuntimeError("The engine {0} has no port{1}".format(engineID, portID))
        # 创建UDP节点
        udp_node = self.create_TCP_or_UDP(holowan_ip, holowan_port, engineID, 2, sourcePort, destPort, checkVersion, action)
        if type(udp_node) == RuntimeError:
            return udp_node
        # 把tcp节点添加到父节点中
        port_node.append(udp_node)
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api),
                                    pathXMLStr)
    ...
    def remove_UDP_from_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int,
        ...
        tcp_nodes = xt.get_nodes(port_node, "./tcp_udp")
        for node in tcp_nodes:
            if xt.classifier_equal(udp_node, node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
            return RuntimeError("Engine{0} port{1} has not such UDP".format(engineID, portID))
    ...
    def create_TCP_or_UDP(self, holowan_ip: str, holowan_port: str, engineID: int, TCPUDP: int, sourcePort: str, destPort: str, checkVersion: int, action: int):
        ...
        if sourcePort == "any":
            ...
            return RuntimeError("sourcePort必须为1-65535之间")
        # ===============dst================== #
        if destPort == "any":
            properties["dst"] = {"any": 1}
            ...
            return RuntimeError("dstPort必须为1-65535之间")
        # ===============checkVersion================== #
        if checkVersion not in [0, 4, 6]:
            return RuntimeError("checkVersion必须为0、4、6中的一个")
        # ===============Action================== #
        paths_dic = self.get_paths_from_engine(holowan_ip, holowan_port, engineID)
        if action not in [-1, -2] and action not in paths_dic.keys():
            return RuntimeError("action值输入错误")
    ...
    def add_IPV4_TCP_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourceIP: str,
                               sourceMask: int, destinationIP: str, destinationMask: int, TOS: str, sourcePort: str, destPort: str,
                              checkVersion: int, action: int):
        try:
            parent_node_path = "./port"
            classifierXMLStr = self.get_Packet_Classifier_config_information(holowan_ip, holowan_port, engineID)
        ...
        # 创建comb混合节点
        comb_node = self.create_IPV4_and_TCP_or_UDP(holowan_ip, holowan_port, engineID, 1, sourceIP, sourceMask,
                                                    destinationIP, destinationMask, TOS, sourcePort,
                                                    destPort, checkVersion, action)
        if type(comb_node) == RuntimeError:
            return comb_node
        # 把combination节点添加到父节点中
        port_node.append(comb_node)
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api),
                                    pathXMLStr)
    ...
    def remove_IPV4_TCP_from_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourceIP: str,
        ...
        # 遍历所有的comb节点
        remove_flags = False
        comb_nodes = xt.get_nodes(port_node, "./comb")
        for node in comb_nodes:
            if xt.classifier_equal(node, comb_node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
            return RuntimeError("Engine{0} port{1} has not such comb".format(engineID, portID))
        pathXMLStr = xt.xmlObject_to_string(root)
        return mt.post_original_api("http://{0}:{1}/{2}".format(holowan_ip, holowan_port, self.emulator_config_api),
                                    pathXMLStr)
    ...
    def add_IPV4_UDP_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourceIP: str,
        ...
        # 创建comb混合节点
        comb_node = self.create_IPV4_and_TCP_or_UDP(holowan_ip, holowan_port, engineID, 2, sourceIP, sourceMask,
                                                    destinationIP, destinationMask, TOS, sourcePort, destPort,
                                                    checkVersion, action)
        if type(comb_node) == RuntimeError:
            return comb_node
    ...
    def get_Packet_Classifier_config_information(self, holowan_ip: str, holowan_port: str, engineID: int):
        ...
        try:
            ...
            return RuntimeError("Engine is not Found")
    ...
    def set_one_tag_text(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, tagPath, tagText):
        ...
        try:
            ...
            if pathXMLStr == r'{"errCode":"-400","errMsg":"ERROR","errReason":"PATH has no found"}':
                return pathXMLStr
    ...
    def set_tag_children(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection,
        ...
        try:
            if pathDirection == 1:
                parent_node_path = "pltr/" + parent_node_ID
            elif pathDirection == 2:
                parent_node_path = "prtl/" + parent_node_ID
            else:
                return r'{"errCode":"-400","errMsg":"ERROR","errReason":"Error pathDirection"}'
            pathXMLStr = self.get_path_config_information(holowan_ip, holowan_port, engineID, pathID)
            if pathXMLStr == r'{"errCode":"-400","errMsg":"ERROR","errReason":"PATH has no found"}':
                return pathXMLStr
            root = xt.xmlString_to_Object(pathXMLStr)
    ...
    def get_HoloWAN_history_entire_data(self, holowan_ip:str, holowan_port:str, engineID:int, pathID:int, last: int, count: int):
        ...
        :param count: 每次获取的数量，最多支持每次获取30个数据点
        :return:
        '''
    ...
    # ===============================================Recorder相关接口====================================================#
    # 上传HoloWAN Recorder 录制文件或Apposite Recorder APP 录制文件
    # def upload_recorder(self, holowan_ip:str, holowan_port:str, filePath):
    #     pass


    # 获取上传Recorder文件信息
    # def get_upload_recorder_information(self, holowan_ip, holowan_port):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    #     :return: xml 详见附录一（1.10Recorder文件概述XML格式）
    #     '''
    #     requestURL = "http://{0}:{1}/recorder_info".format(holowan_ip, holowan_port)
    #     return requests.get(requestURL).text


    # 获取最有一次Recorder配置参数
    # def get_recorder_config_information(cself, holowan_ip, holowan_port):
    #     '''
    #     :param holowan_ip: holowan IP地址
    #     :param holowan_port: holowan 端口号
    #     :return: xml 详见附录一（1.11Recorder文件概述XML格式）
    #     '''
    #     requestURL = "http://{0}:{1}/recorder.xml".format(holowan_ip, holowan_port)
    #     return requests.get(requestURL).text
    ...
    # 设置偏好
    @checkParameter()
    def set_preferences(self, holowan_ip:str, holowan_port:str, clean_buffer: str, zero_line: str):
        '''
        :param holowan_ip: holowan IP地址
        :param holowan_port: holowan 端口号
        ...
        if clean_buffer not in ["true", "false"]:
            return r'{"errCode":"-500","errMsg":"ERROR","errReason":"Error parameter values(clean_buffer)"}'

        parent_node = xt.create_node("holowan_admin_preferences", {}, "")
        parent_node.append(xt.create_node("clean_buffer", {}, clean_buffer))
        parent_node.append(xt.create_node("zero_line", {}, zero_line))
        xmlStr = xt.xmlObject_to_string(parent_node)
    ...
    # 获取PATH链路Name
    @checkParameter()
    def get_path_Name(self, holowan_ip:str, holowan_port:str, engineID:int, pathID: int):
    ...
    @checkParameter()
    def get_paths_from_engine(self, holowan_ip:str, holowan_port:str, engineID:int):
        path_dic = {}
    ...
    def resetEngine(self, holowan_ip: str, holowan_port: str, engineID: int):
        ...
        for pathId in paths_dic.keys():
            ...
            remove_path_return = self.remove_path(holowan_ip, holowan_port, engineID, pathId, pathName)
...
if __name__ == '__main__':
    ...
    action = -2
    holowan = HoloWAN()
    response = holowan.remove_IPV4_UDP_from_Classifier(holowan_ip=holowan_ip,
