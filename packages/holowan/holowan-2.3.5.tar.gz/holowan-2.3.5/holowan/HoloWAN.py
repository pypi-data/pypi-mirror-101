...
class HoloWAN:
    ...
    def remove_path(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathName: str):
        ...
        if self.has_engine(holowan_ip, holowan_port, engineID) is False:
            return r'{"errCode":"-5","errMsg":"ERROR","errReason":"ENGINE is not found"}'
        elif self.has_path(holowan_ip, holowan_port, engineID, pathID) is False:
            return r'{"errCode":"-12","errMsg":"ERROR","errReason":"PATH is not found"}'
        if self.path_is_open(holowan_ip, holowan_port, engineID, pathID):
            return r'{"errCode":"-500","errMsg":"ERROR","errReason":"PATH is enabled, can not be deleted"}'
    ...
    def get_path_config_information(self, holowan_ip:str, holowan_port:str, engineID:int, pathID: int):
        ...
        try:
            ...
            if self.has_path(holowan_ip, holowan_port, engineID, pathID) is False:
                return r'{"errCode":"-6","errMsg":"ERROR","errReason":"PATH has no found"}'
    ...
    def set_one_tag_text(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, tagPath: str, tagText):
        ...
        try:
            ...
            if pathXMLStr == r'{"errCode":"-3","errMsg":"ERROR","errReason":"ENGINE has no found"}' or \
                    pathXMLStr == r'{"errCode":"-6","errMsg":"ERROR","errReason":"PATH has no found"}':
                return pathXMLStr
    ...
    # 获取当前设备支持的引擎数
    def get_engine_count(self, holowan_ip: str, holowan_port: str):
        response = self.get_HoloWAN_information(holowan_ip=holowan_ip, holowan_port=holowan_port)
        return xt.get_node(xt.xmlString_to_Object(response), "./eeq").text

    # 判断引擎是否存在
    def has_engine(self, holowan_ip: str, holowan_port: str, engineID: int):
        if engineID > int(self.get_engine_count(holowan_ip, holowan_port)):
            return False
        return True

    # 判断链路是否存在
    def has_path(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int):
        if pathID not in self.get_paths_from_engine(holowan_ip, holowan_port, engineID).keys():
            return False
        return True
...
class SubHoloWAN(HoloWAN):
    ...
    def get_paths_from_engine(self, holowan_ip: str = "default", holowan_port: str = "default", engineID: int = 1):
        holowan_ip, holowan_port = self.setDefaultParameter(holowan_ip, holowan_port)
...
if __name__ == '__main__':
    ...
    pathName = "PATH 2"
    holowan = HoloWAN()
    response = holowan.remove_path(holowan_ip=holowan_ip,
