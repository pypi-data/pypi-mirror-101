...
class HoloWAN:
    ...
    '''===============================================Playback相关接口===================================================='''
    # 上传HoloWAN Recorder 录制文件
    @checkParameter()
    def __upload_playback(self, holowan_ip: str, holowan_port: str, filePath: str):
        '''
        :param holowan_ip:
        :param holowan_port:
        :param filePath: 文件路径
        :return: xml
        '''
        requestURL = "http://{0}:{1}/playback_upload".format(holowan_ip, holowan_port)
        ...
        if fileType == "txt":
            ...
            return r'{"errCode":"-400","errMsg":"error","errReason":"上传文件必须为txt格式"}'

    # 应用回放文件到指定链路
    def __apply_playback(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, playback_id: int, playback_file_name: str):
        requestURL = "http://{0}:{1}/playback_apply?playback_id={2}&filename={3}&engine={4}&path={5}"\
            .format(holowan_ip, holowan_port, playback_id, playback_file_name, engineID, pathID)
        return requests.get(requestURL).text

    # 上传HoloWAN Recorder录制文件，并回放到指定链路
    def upload_and_apply_playback(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, filePath: str):
        upload_response = self.__upload_playback(holowan_ip, holowan_port, filePath)
        playback_id = -1
        for key, value in (dict(json.loads(upload_response))).items():
            if key == "code" and value != 0:
                return upload_response
            if key == "data":
                playback_id = dict(value).get("playback_id")
                playback_file_name = filePath.split("\\")[-1]
        return self.__apply_playback(holowan_ip, holowan_port, engineID, pathID, playback_id, playback_file_name)

    # 删除回放文件及其回放配置
    def remove_playback(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int):
        requestURL = "http://{0}:{1}/playback_remove?engine={2}&path={3}"\
            .format(holowan_ip, holowan_port, engineID, pathID)
        return requests.get(requestURL).text

    # 获取回放文件内容
    def get_playback_data(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, type: int):
        '''
        :param type: 0：头部信息；1：延时数据；2：丢包数据；3：带宽数据
        :return:
        '''
        requestURL = "http://{0}:{1}/get_playback_data?engine={2}&path={3}&type={4}"\
            .format(holowan_ip, holowan_port, engineID, pathID, type)
        return requests.get(requestURL).text

    # 获取当前回放状态
    def get_playback_status(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int):
        requestURL = "http://{0}:{1}/get_playback_status?engine={2}&path={3}" \
            .format(holowan_ip, holowan_port, engineID, pathID)
        return requests.get(requestURL).text

    # 改变回放状态
    def set_playback_status(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, action: int, **kwargs):
        if len(kwargs) > 1:
            return r'{"errCode":"-500","errMsg":"error","errReason":"参数错误"}'
        if len(kwargs) == 1:
            cursor = -1
            for key, value in kwargs.items():
                if key != "cursor":
                    ...
                    cursor = value
            requestURL = "http://{0}:{1}/set_playback_status?engine={2}&path={3}&action={4}&cursor={5}" \
                .format(holowan_ip, holowan_port, engineID, pathID, action, cursor)
        else:
            requestURL = "http://{0}:{1}/set_playback_status?engine={2}&path={3}&action={4}" \
                .format(holowan_ip, holowan_port, engineID, pathID, action)
        return requests.get(requestURL).text
