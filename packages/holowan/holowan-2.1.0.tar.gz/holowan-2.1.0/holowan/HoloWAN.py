...
class HoloWAN:
    ...
    # 设置丢包配置  Dual双通道丢包模式
    @checkParameter()
    def set_Loss_Dual(self, holowan_ip: str, holowan_port: str, engineID: int, pathID: int, pathDirection,
    ...
    def add_IPV4_to_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int, sourceIP: str,
        ...
        children_node_Map = {"src": "", "smask": sourceMask, "dst": "", "dmask": destinationMask, "tos": "", "path_id": action}
        properties = {}
        # ===============src================== #
        if sourceIP == "any":
        ...
        # ===============Action================== #
        if action not in [-1, -2, 2*(engineID-1)+1]:
    ...
    def remove_IPV4_from_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int, sourceIP: str,
        ...
        if TOS == "any":
            ...
            return RuntimeError(r"TOS必须输入'any'或2位十六进制")
        # ===============Action================== #
        if action not in [-1, -2, 2 * (engineID - 1) + 1, 2 * (engineID - 1) + 2]:
            return RuntimeError("action值输入错误")
        ...
        for node in ipv4_nodes:
            if xt.xmlObject_to_string(ipv4_node) == xt.xmlObject_to_string(node):
    ...
    def remove_IPV6_from_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int,
        ...
        for node in ipv6_nodes:
            if xt.xmlObject_to_string(ipv6_node) == xt.xmlObject_to_string(node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
    ...
    # 添加MAC分类器  无隧道
    @checkParameter()
    def add_MAC_to_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int, sourceIP: str,
                              destinationIP: str, EtherType: str, action: int):
        '''
        :param holowan_ip: holowan IP地址
        :param holowan_port: holowan 端口号
        :param engineID: 引擎ID
        ...
        if EtherType == "any":
            ...
            return RuntimeError(r"MAC的Ether Type输入值不正确")
        # ===============Action================== #
        if action not in [-1, -2, 2 * (engineID - 1) + 1, 2 * (engineID - 1) + 2]:
            return RuntimeError("action值输入错误")
    ...
    # 删除MAC分类器  无隧道
    @checkParameter()
    def remove_MAC_from_Classifier(self, holowan_ip:str, holowan_port:str, engineID:int, portID: int, sourceIP: str,
                              destinationIP: str, EtherType: str, action: int):
        '''
                :param holowan_ip: holowan IP地址
                :param holowan_port: holowan 端口号
        ...
        for node in MAC_nodes:
            if xt.xmlObject_to_string(MAC_node) == xt.xmlObject_to_string(node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
    ...
    def add_TCP_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourcePort: str, destPort: str,
        ...
        # 创建TCP节点
        tcp_node = xt.create_node("tcp_udp", {}, {})
        children_node_map = {"type": 1, "src": "", "dst": "", "check": checkVersion, "path_id": action}
        properties = {}
        # ===============src================== #
        if sourcePort == "any":
            properties["src"] = {"any": 1}
        ...
        xt.add_children(tcp_node, children_node_map)
        xt.add_properties(tcp_node, properties)
    ...
    def remove_TCP_from_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourcePort: str, destPort: str,
        ...
        for node in tcp_nodes:
            if xt.xmlObject_to_string(tcp_node) == xt.xmlObject_to_string(node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
    ...
    def add_UDP_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourcePort: str, destPort: str,
        ...
        # 创建TCP节点
        tcp_node = xt.create_node("tcp_udp", {}, {})
        children_node_map = {"type": 2, "src": "", "dst": "", "check": checkVersion, "path_id": action}
        properties = {}
        # ===============src================== #
        if sourcePort == "any":
            properties["src"] = {"any": 1}
    ...
    def add_IPV4_TCP_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourceIP: str,
        ...
        # 创建TCP节点
        tcp_node = xt.create_node("tcp_udp", {}, {})
        children_node_map = {"type": 1, "src": "", "dst": "", "check": checkVersion}
        properties = {}
        # ===============src================== #
        if sourcePort == "any":
            properties["src"] = {"any": 1}
    ...
    def remove_IPV4_TCP_from_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourceIP: str,
        ...
        for node in comb_nodes:
            if xt.xmlObject_to_string(comb_node) == xt.xmlObject_to_string(node):
                port_node.remove(node)
                remove_flags = True
                break
        if remove_flags is False:
    ...
    def add_IPV4_UDP_to_Classifier(self, holowan_ip: str, holowan_port: str, engineID: int, portID: int, sourceIP: str,
        ...
        # 创建UDP节点
        tcp_node = xt.create_node("tcp_udp", {}, {})
        children_node_map = {"type": 2, "src": "", "dst": "", "check": checkVersion}
        properties = {}
        # ===============src================== #
        if sourcePort == "any":
            properties["src"] = {"any": 1}
