"""Base transformers apps to train an embedding"""
...
class Transformers:
    """Class provide embedding, logelikelihood function"""
    ...
    def __init__(
        ...
        if device == "cpu" and model_type != "esm1_t6_43M_UR50S":
            ...
            sleep(2)
        try:
            model_fn = getattr(pretrained, model_type)
        except:
            msg = (
        ...
        self.model, self.token = model_fn()
        self.model = self.model.to(self.device)
        self.emb_version = int(re.findall(PATTERN, model_type)[0])

    @classmethod
    def list_esm_backend(cls) -> None:
        """Get all possible backend for the ESM model"""

        log.info("Use ESM backend in this list %s", " / ".join(BACKEND))

    def _generate_chunks(self, sequences: List[str], batch_size: int) -> List[str]:
        """Build a generator to yield protein sequence"""
        for i in range(0, len(sequences), batch_size):
            yield sequences[i : (i + batch_size)]

    def _predict(
        self, sequences: List[str], type: str = "logits", batch_size=1
    ) -> np.array:
        """Return the model prediction of a sequence of Am.Ac."""

        batch_converter = self.token.get_batch_converter()
        convert_sequences = self.convert_batch_format(sequences)
        batch_labels, batch_strs, batch_tokens = batch_converter(convert_sequences)
        result = []

        for batch in tqdm(self._generate_chunks(batch_tokens, batch_size)):
            with torch.no_grad():
                output = self.model(
                    batch.to(self.device), repr_layers=[self.emb_version]
                )
            ...
            if type == "logits":
                ...
                output = (
                    output["representations"][self.emb_version]
                    .detach()[:, :, :]
                    .mean(1)
                )
    ...
    def predict_embedding(self, sequences: List[str], batch_size=1) -> np.ndarray:
        pred = self._predict(sequences, type="representations", batch_size=batch_size)
        return pred

    def predict_logits(self, sequences: List[str], batch_size=1) -> np.ndarray:
        pred = self._predict(sequences, type="logits", batch_size=batch_size)
        return pred
