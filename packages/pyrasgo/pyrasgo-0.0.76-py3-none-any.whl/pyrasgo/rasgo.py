...
# ---------
    ...
    # Alias
    @track_usage
    def get_collections(self, include_shared: bool=False) -> List[Model]:
        """
        ...
        """
        return self.get_models(include_shared=include_shared)
    ...
    def get_data_source_columns(self, data_source_id: int) -> List[api.DataSourceColumn]:
        ...
        """
        try:
            response = self._get(f"/data-source/columns/{data_source_id}", api_version=1).json()
    ...
    @track_usage
    def get_models(self, include_shared: bool=False) -> List[Model]:
        """
        Returns all Rasgo Collections (set of joined Features) that I have author access to. Add an include_shared 
        parameter to return all Rasgo Collections that I have any access to (author or shared access)
        :param include_shared: Boolean value indicating if the return should include all accessible collections
        """
        try:
            return [Model(api_object=entry) for entry in self._get(f"/models", {"include_shared": include_shared}, api_version=1).json()]
        except:
    ...
    @track_usage
    def match_feature_set(self, table_name: str, fs_name: Optional[str] = None) -> Optional[api.v1.FeatureSet]:
        """
        ...
        try:
            ...
            if fs_name and len(fs) > 1:
                for f in fs:
                    if f.name == fs_name:
                        print("f", f)
                        return api.v1.FeatureSet(**f)
                return None
            else:
                return api.v1.FeatureSet(**fs[0])
        except:
            return None
...
# ------------------    
    ...
    @track_usage
    def publish_features(self, features_dict: dict) -> api.v1.FeatureSet:
        f"""
    ...
    def publish_features_from_df(self, df: pd.DataFrame, dimensions: List[str], features: List[str],
                                 ...
                                 sandbox: bool = True) -> api.v1.FeatureSet:
        """
        ...
        return self.get_feature_set(featureset.id)
    ...
    def publish_features_from_source(self, data_source_id: int,
                                     ...
                                     sandbox: bool = True, 
                                     if_exists: str = 'fail') -> api.v1.FeatureSet:
        """
        ...
        return self.get_feature_set(feature_set.id)

    @track_usage
    def publish_features_from_yml(self, yml_file: str, sandbox: Optional[bool] = True, 
                                  git_repo: Optional[str] = None) -> api.v1.FeatureSet:
        """
...
# -------------------------------
    ...
    def _save_feature_set(self, name: str, data_source_id: int, table_name: str, organization_id: int, granularity: Optional[str] = None, file_path: Optional[str] = None, if_exists: str = 'return') -> api.v1.FeatureSet:
        """
    ...
    def _save_feature_set_dict(self, feature_set_dict: dict) -> api.v1.FeatureSet:
        """
