...
from cntkx.layers import LayerNormalization, Dropout
from cntk.default_options import default_override_or
from cntk.layers.blocks import _inject_name


def LinearAttention(hidden_dim: int, model_dim: int, dropout_rate: float = 0,
    ...
    Katharopoulos et al. (https://arxiv.org/abs/2006.16236) (uses elu kernel trick)

    For softmax kernel trick see "Efficient Attention: Attention with Linear Complexities"
    ...
    Arguments:
        ...
        kernel_type (str): there are many kinds of linear attention each using a different method. This defines the kernel trick to be used here.
          Only 2 types are allow 'elu' or 'softmax'.
    ...
    dropout = Dropout(dropout_rate) if dropout_rate else None
    ...
    def model(query, key, value):
        ...
        if kernel_type == 'elu':
            ...
            q = C.softmax(query_linear(query))
            k = C.sequence.softmax(key_linear(key))
        else:
            raise ValueError(f"kernel type cannot be {kernel_type}")
        ...
        kv_broadcasted = dropout(kv_broadcasted) if dropout is not None else kv_broadcasted
        # kv [#, *] [model_dim, hidden_dim]
...
def LinearAttentionModel(hidden_dim: int, model_dim: int,
    ...
    attention = LinearAttention(hidden_dim=hidden_dim, model_dim=model_dim,
                                ...
                                kernel_type=kernel_type, name=name)
