...
from utilitys.widgets import PopupLineEditor
...
class ParamEditor(QtWidgets.QDockWidget):
  ...
  def saveParamValues(self, saveName: str=None, paramState: dict=None, allowOverwriteDefault=False,
    ...
    * Returns None if no save name was given
    ...
    # TODO: Decide whether states created outside saveDir should be emitted here
    return paramState
  ...
  def loadParamValues(self, stateName: t.Union[str, Path],
    ...
    if useDefaults and defaultFile and defaultFile.exists():
      base = fns.attemptFileLoad(defaultFile)
    ...
    if applyChanges:
      self.applyChanges()
    self.lastAppliedName = Path(stateName).stem
    self.stateSelector.setText(self.lastAppliedName)
  ...
  def _buildGui(self, **kwargs):
    ...
    self.stateSelector = PopupLineEditor(clearOnComplete=False, parent=self)
    """Shows current state or load a new one"""
    def onReturn():
      val = self.stateSelector.text()
      if val in self.savedStates and val != self.lastAppliedName:
        self.loadParamValues(val)
    self.stateSelector.returnPressed.connect(onReturn)

    # -----------
    # Widget layout
    # -----------
    children = [
      [QtWidgets.QLabel('Load state: '), self.stateSelector],
    ...
    if self.saveDir is None:
      self.saveAsBtn.hide()
      self.dockContentsWidget.easyChild.children_[0].widget_.hide()
