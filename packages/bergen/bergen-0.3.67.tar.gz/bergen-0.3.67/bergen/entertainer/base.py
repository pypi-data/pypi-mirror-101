...
from bergen.messages.postman.unprovide import UnprovideDoneMessage, BouncedUnprovideMessage
...
class BaseEntertainer(Hookable):
    ...
    async def on_message(self, message: MessageModel):
        ...
        if isinstance(message, UnprovideDoneMessage): 
            ...
            self.unprovide_futures[message.meta.reference].set_result(message.data.reference)

        if isinstance(message, BouncedUnprovideMessage): 
            #  We are responsible To Shut ourselfves down when Arkitekt Tells us t
            assert message.data.reference is not None, "Received Cancellation with no reference to kill" 
            logger.info("Arkitekt demands our Actor to die")
        ...
        if isinstance(message, BouncedAssignMessage):
            assert message.data.pod is not None, "Received assignation that had no Provision???"
            assert message.data.pod in self.provision_actor_queue_map, f"Provision not entertained {message.data.pod} {self.provision_actor_queue_map}"
            await self.provision_actor_queue_map[message.data.pod].put(message)
    ...
    async def unentertain(self, bounced_unprovide: BouncedUnprovideMessage, from_arkitekt=False):
        ...
        actor = self.provision_actor_map[provision_reference]
        
        # Try to unreserve all reserve resources
        ok = await actor._on_unprovide() 
    ...
    async def entertain(self, bounced_provide: BouncedProvideMessage, from_arkitekt=False):
        ...
        actorClass = await self.get_actorclass_for_template(bounced_provide.data.template)
        ...
        try:
            # Try to reserve resources in its dependencies if there is any, if we fail we cancel everything
            ok = await actor._on_provide()
