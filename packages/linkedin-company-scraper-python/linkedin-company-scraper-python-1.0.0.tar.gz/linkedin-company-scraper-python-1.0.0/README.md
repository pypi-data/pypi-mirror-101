Linkedin-Company-Scraper-Python is a python library to scrape company profile data on linkedin using browser automation. 
It currently runs only on windows.

### Example
In this example we first import library, then we login with cookies and then scraped profile data of company.
```sh
from linkedin_company_scraper_python import *
...
response=linkedin.get_company_profile(company_link='https://www.linkedin.com/company/microsoft/about/')
data=response["body"]
```
...
```sh
pip install linkedin-company-scraper-python
```
...
```sh
response=linkedin.get_company_profile(company_link='company link')
```
