...
class kDataFrame(object):
    ...
    def getTwin(self) -> "kDataFrame *":
        r""" creates a new kDataframe using the same parameters as the current kDataFrame. It is like clone but without copying the data"""
        return _kProcessor.kDataFrame_getTwin(self)

    def reserve(self, n: "uint64_t") -> "void":
        r""" request a capacity change so that the kDataFrame can approximately at least n kmers"""
        return _kProcessor.kDataFrame_reserve(self, n)

    def insert(self, *args) -> "bool":
        r"""
        *Overload 1:*
        insert the kmer one time in the kDataFrame, or increment the kmer count if it is already exists. Returns bool value indicating whether the kmer is inserted or not

        |

        *Overload 2:*
        insert the kmer N time in the kDataFrame, or increment the kmer count with N if it is already exists. Returns bool value indicating whether the kmer is inserted or not

        |

        *Overload 3:*
        insert the kmer in the kmer row time in the kDataFrame, or increment the kmer count with the count in the row if it is already exists. Returns bool value indicating whether the kmer is inserted or not
        """
        return _kProcessor.kDataFrame_insert(self, *args)

    def setCount(self, kmer: "std::string", N: "uint64_t") -> "bool":
        r"""
         set the kmer's count to N time in the kDataFrame Returns bool value indicating whether the kmer is inserted or not.
