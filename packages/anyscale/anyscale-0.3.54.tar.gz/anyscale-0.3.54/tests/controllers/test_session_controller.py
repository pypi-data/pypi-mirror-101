...
from anyscale.sdk.anyscale_client import (  # type: ignore
    CreateSession,
    StartSessionOptions,
    UpdateSession,
)
...
def test_stop_multiple_sessions(
...
def test_create_or_update_session_data(
    ...
    if update_session:
        base_mock_anyscale_api_client.update_session.assert_called_once_with(
        ...
        base_mock_anyscale_api_client.create_session.assert_called_once_with(
            create_session_data
        )
...
def test_list_app_configs(
    ...
    (mock_api_client, project_test_data, _, _) = mock_up_tuple
    base_mock_anyscale_api_client.list_app_configs = Mock(
        return_value=Mock(results=[], metadata=Mock(next_paging_token=None))
    )
    session_controller = SessionController(
        api_client=mock_api_client, anyscale_api_client=base_mock_anyscale_api_client
    )
    session_controller._list_app_configs(project_test_data.id)
    base_mock_anyscale_api_client.list_app_configs.assert_called_once_with(
        project_test_data.id, count=50, paging_token=None
    )
...
def test_list_builds(
    ...
    base_mock_anyscale_api_client.list_builds = Mock(
        return_value=Mock(results=[], metadata=Mock(next_paging_token=None))
    )
    session_controller = SessionController(
        api_client=mock_api_client, anyscale_api_client=base_mock_anyscale_api_client
    ...
    base_mock_anyscale_api_client.list_builds.assert_called_once_with(
...
def test_anyscale_start(
    ...
    (mock_api_client, project_test_data, _, mock_wait_for_start,) = mock_up_tuple
    session_controller = SessionController(
        api_client=mock_api_client, anyscale_api_client=base_mock_anyscale_api_client
    )
    session_controller.push = Mock()  # type: ignore
    ...
    base_mock_anyscale_api_client.start_session.assert_called_once_with(
        "session-11-id",
        StartSessionOptions(
    ...
    if sync_files:
        session_controller.push.assert_called_once_with(
            all_nodes=False,
            config=None,
            session_name="session-11",
            source="/some/directory",
            target="mock_working_dir",
        )
...
def test_ssh_session(
    session_test_data: Session,
    mock_api_client_multiple_sessions: Mock,
    base_mock_anyscale_api_client: Mock,
) -> None:
    session_controller = SessionController(
