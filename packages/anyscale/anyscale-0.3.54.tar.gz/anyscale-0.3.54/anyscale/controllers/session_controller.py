...
class SessionController:
    ...
    def start(
        ...
        if sync_files:
            # Push only the project directory to the working directory
            working_dir = get_working_dir(
                cluster_config={}, project_id=project_id, api_client=self.api_client
            )
            self.push(
    ...
    def _list_app_configs(self, project_id: str) -> List[AppConfig]:
        ...
        while has_more:
            resp = self.anyscale_api_client.list_app_configs(
                project_id, count=50, paging_token=None
            )
