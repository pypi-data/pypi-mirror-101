...
def count_n_atoms(form_str:str):
    ...
    Positional arguments:
        form_str (str) : chemical formula string

    Keyword arguments:
        None

    Returns:
        n_atoms (int) : number of atoms in chemical formula
    """
...
def index_elements(labels:list,shift=0:int):
    """
    Return dictionary of element (keys) and indices (values) from list of labels

    Positional arguments:
        labels (list) : atomic labels

    Keyword arguments:
        shift (int)   : additive shift to apply to all indices

    Returns:
        ele_index (dict) : dictionary of element (keys) and indices (values)
    """
...
def formdict_to_formstr(form_dict:dict, include_one:bool=False):
    ...
    Keyword arguments:
        include_one (bool) : Include 1 in final chemical formula e.g. C1H4
