...
class NacosClient:
    ...
    def get_config(self, data_id, group, timeout=None, no_snapshot=None):
        ...
        #     logger.error("[get-config] error HTTP code [:%s] for data_id:%s, group:%s, namespace:%s" % (
        #             resp.status_code, data_id, group, self.namespace))
        #     return None
    ...
    def _do_sync_req(self, url, headers=None, params=None, data=None, timeout=None, method="GET"):
        ...
        all_headers = self._get_common_headers(self.current_server, params, data)
        if headers:
    ...
    def _get_common_headers(self, server, params, data):
        # import ipdb;ipdb.set_trace()
        if '://' in server[0]:
            schema, host = server[0].split('://')
        else:
            host = server[0]
        return {'Host': host}
