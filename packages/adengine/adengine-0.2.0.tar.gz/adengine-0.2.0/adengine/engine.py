...
class SessionPool(Startable):
    ...
    def _put(self, session: PacketTracerSession):
        ...
        if self._tasks_counter == self._tasks_before_session_restart:
            self._run_new_thread(threading.Thread(name='RestartSession', target=restart_session, daemon=True))
            self._tasks_counter = 0
        else:
            self._pool.put(session)
    ...
    def increase_pool(self):
        ...
        if self.size < self._max_size:
            self._run_new_thread(threading.Thread(name='IncreasePool', target=_increase_pool, daemon=True))
        else:
    ...
    def decrease_pool(self):
        def _decrease_pool():
            ...
            delay = 1
            try:
                session = self._pool.get(True, delay)
            except queue.Empty:
        ...
        if self.size > self._min_size:
            self._run_new_thread(threading.Thread(name='DecreasePool', target=_decrease_pool, daemon=True))
        else:
    ...
    def _clear_old_threads(self):
        self._threads = [thread for thread in self._threads if thread.is_alive()]

    def _run_new_thread(self, thread: threading.Thread):
        self._clear_old_threads()
...
class ADEngine(Startable):
    ...
    def _control_session_pool(self):
        ...
        while not self._event.wait(check_loading_delay):
            ...
            tasks_per_session = not_completed_tasks // session_pool_size
            ...
            if tasks_per_session > max_tasks_per_session:
                ...
                try:
                    ...
                    logger.debug('Failed to increase pool')
                else:
                    logger.debug('Session pool increased successfully')
                ...
                try:
                    ...
                    logger.debug('Failed to decrease pool')
                else:
                    logger.debug('Session pool decreased successfully')
