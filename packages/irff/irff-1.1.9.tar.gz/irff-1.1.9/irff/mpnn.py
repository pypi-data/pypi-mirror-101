...
class MPNN(ReaxFF):
  ...
  def get_bondorder(self,t,bd,atomi,atomj):
      ...
      if self.MessageFunction==1:
         Fi      = self.f_nn(flabel,bd, self.nbd[bd],[Dbi,Dbj,h],layer=self.mf_layer[1])
         Fj      = self.f_nn(flabel,bdr,self.nbd[bd],[Dbj,Dbi,h],layer=self.mf_layer[1])
         F       = 2.0*Fi*Fj
         ...
         Fi      = self.fmassage(flabel,bd, self.nbd[bd],[Dbi,Dbj,h],layer=self.mf_layer[1])
         Fj      = self.fmassage(flabel,bdr,self.nbd[bd],[Dbj,Dbi,h],layer=self.mf_layer[1])
         F       = 2.0*Fi*Fj
  ...
  def set_m(self):
      ...
      for t in range(1,self.messages+1):
          ...
          self.set_wb(pref='f'+str(t),reuse_m=reuse_m,nin=3,nout=nout_,layer=self.mf_layer,
