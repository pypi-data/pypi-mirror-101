...
from typing import Any, Callable, Generator, NamedTuple, Optional, Tuple, TypeVar, Literal, Union
...
class DispatcherContextManager(BaseDispatcher):
    ...
    def afterDispatch(self, interface: "DispatcherInterface", exception: Optional[Exception], tb: Optional[TracebackType]):
        try:
            if not tb:
                self.generator.send((StatusCodeEnum.DISPATCH_COMPLETED, None))
            else:
                self.generator.send((StatusCodeEnum.DISPATCH_EXCEPTION, ExcInfo(exception, tb)))
    ...
    def afterExecution(self, interface: "DispatcherInterface", exception: Optional[Exception], tb: Optional[TracebackType]):
        try:
            if not tb:
                ...
                self.generator.send((StatusCodeEnum.EXECUTION_EXCEPTION, ExcInfo(exception, tb)))
        except StopIteration:
            pass