...
def write_files(
    ...
    for template in collection.templates:
        ...
        if template.output_path in protected and os.path.exists(
            output_path
        ):
    ...
    for static_file in collection.static_files:
        ...
        if static_file in protected and os.path.exists(output_path):
...
def rename_folders(
    ...
    for (
        ...
        rename = replace_head(folder, render_context[rename_token])
...
def run_initialization(
    ...
    for cmd in collection.initialization:
        reporter.show_run_initialization(cmd)

        if not dry_run:
            pipe_call(cmd.split(" "), cwd=output_dir)
