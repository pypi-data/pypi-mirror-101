...
def _run_command_host(section, path, cmd_to_run, mount, extra_args, debug):
    """
...
def _run_command(section, path, cmd_to_run, mount, extra_args, debug):
    ...
    if ci_env == CIEnvironment.HOST:
        return _run_command_host(section, path, cmd_to_run, mount, extra_args, debug)
...
def exec_(ctx, cmd_name, extra_args, command, save_as, warn, mount):
    """
    ...
    returncode = _run_command(section, path, cmd_to_run, mount, extra_args, ctx.obj.debug)
    ctx.exit(returncode)
