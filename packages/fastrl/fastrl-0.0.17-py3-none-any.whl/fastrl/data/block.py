...
__all__ = ['source_events', 'Src', 'SrcCallback', 'GymSrc', 'FirstLast', 'SourceDataset', 'IterableTfmdLists',
           'vstack_collate', 'fr_collate', 'IterableTfmdDL', 'IterableDataBlock']
...
class GymSrc(SrcCallback):
    def initialize(self):
        ...
        if self.source.agent is None:
            test_cb=TstCallback(action_space=self.pool[0].action_space,constant=0)
    ...
    def after_history(self):
        ...
        for k in self.all_exp: self.all_exp[k][self.imask]=torch.clone(active_exp[k])
