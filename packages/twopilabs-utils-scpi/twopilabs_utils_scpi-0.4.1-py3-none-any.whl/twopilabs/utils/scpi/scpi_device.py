...
class ScpiDevice(object):
    ...
    def __init__(self, resource: ScpiResource, **kwargs) -> None:
        ...
        logger.info(f'open [{resource.transport._transport_name}]: {resource.address}')
    ...
    def close(self) -> None:
        if self.transport is not None:
            logger.info(f'close [{self.resource.transport._transport_name}]: {self.resource.address}')
