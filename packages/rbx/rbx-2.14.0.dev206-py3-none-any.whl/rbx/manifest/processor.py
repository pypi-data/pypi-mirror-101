...
CreativeVariant = namedtuple('CreativeVariant', ['filename', 'name', 'creative'])
...
class HttpAssetFetcher(AssetFetcher):
    ...
    def get_video(self, url, base_url=None):
        """Retrieve Video from http, decode it using opencv and return the decoded video.

        Parameters:
            url (str):
                The location of the video to retrieve.
            base_url (str):
                The base location of the video to retrieve, if overriding the manifest.

        Returns:
            VideoFileClip:
                A VideoFileClip representation of the retrieved video.
        """
        if self.base_url is None and base_url is None:
            raise FatalException(
                f"A Base URL must be provided: {url}"
            ...
            base_url = base_url if base_url is not None else self.base_url
            try:
                video = VideoFileClip(f'{base_url}{url}')
            except IndexError:
                raise FatalException(
                    f"Video not found at specified location: {base_url}{url}"
                )
        return video
...
class S3AssetFetcher(AssetFetcher):
    ...
    def get_video(self, url):
        aws_url = self.client.generate_url(url)
        try:
            video = VideoFileClip(aws_url)
        except IndexError:
            raise FatalException(
                f"Video not found at specified location: "
                f"{self.client.name}/{url}"
            )
...
class Creative:
    ...
    def render(self):
        """Builds an opencv image or video representation of the manifest and return.
        ...
        ordered_components = self.root_component.compile()
        flattened_components = list(flatten(ordered_components))
        for position, entry in enumerate(flattened_components):
            ...
            elif isinstance(entry, VideoFileClip):
                root_video = merge_images_to_video(
                    video=entry,
                    background_image=root_image,
                    overlay_images=flattened_components[position+1:]
                )
                break

        return root_video if root_video else root_image
    ...
    def compile(self):
        ...
        for entry in sorted(self.components, key=operator.attrgetter("z_index")):
            ...
            for flat_entry in flattened_entry:
                if isinstance(flat_entry, np.ndarray):
                    components.append(merge_images(
                ...
                elif isinstance(flat_entry, VideoFileClip):
                    components.append(flat_entry)
...
class VideoComponent(Component):
    """A component that describes a video."""

    def __init__(self, component, client, variant):
        """Initialises the VideoComponent, storing the location of the Video file.
        ...
        Parameters:
            ...
            variant (Variant): The variant details to apply to the component.
        """
        super(VideoComponent, self).__init__(
            component=component,
            client=client,
            variant=variant
        )
        self.url = component.get('mp4_url')

    def compile(self):
        """Return the rendered video in an iterable format.

        Returns:
            list:
                Returns the video in an iterable, for easier handling in the base render function.
        """
        return [self.render()]

    def render(self):
        """Retrieve Video from http or S3, decode it using opencv and return the decoded image.

        Returns:
            VideoFileClip:
                A VideoFileClip representation of the retrieved video.
        """
        return self.client.get_video(f'{self.url}')
...
def make_component(component, client, variant):
    ...
    if component_type == 'layout':
        ...
        return VideoComponent(component, client, variant)
...
def merge_images_to_video(video, background_image, overlay_images=None):
    """Applies a video to a background image, then adds overlay images on top.

        As moviepy needs images in a particular format, we convert each ndarray image to an
        ImageClip. This also gives us an opportunity to add extra data, such as how long the image
        should show up in the video.

        Parameters:
            video (VideoFileClip):
                The video to apply images to.
            background_image (ndarray):
                The image to place behind the video.
            overlay_images (list):
                The list of ndarray images to apply over the top of the video.

        Returns:
            CompositeVideoClip:
                Returns a CompositeVideoClip that has the video, audio with all images applied.

    """
    if overlay_images is None:
        overlay_images = []

    background_image = ImageClip(background_image, duration=video.duration)

    overlay_images = [ImageClip(x, duration=video.duration) for x in overlay_images]

    final_video = CompositeVideoClip([background_image, video, *overlay_images], use_bgclip=True)

    return final_video


def zip_creatives(creatives, filename=None):
    """Combine all creatives in the iterable to a ZIP format.

        Parameters:
            creatives (list):
                A list of CreativeVariants with the filename, human name and creative data.
            filename (str):
    ...
    for data in creatives:
        if isinstance(data.creative, np.ndarray):
            retval, buf = cv2.imencode('.png', data.creative)
            zipf.writestr(data.filename, buf)
        else:
            # since moviepy does not provide a way to output to buffer, we generate a uuid to
            # use as a temporary filename. We then zip this file and delete it afterwards.
            uuid = f'{uuid4()}.mp4'
            try:
                data.creative.write_videofile(uuid)
                zipf.write(uuid)
            finally:
                os.remove(uuid)
