...
class Task:
    ...
    def __init__(self, name, settings, argv, stdout, stderr):
        ...
        if not self.using_system_out:
            self.stdout.reconfigure(write_through=True)
        
        if not self.using_system_err:
            self.stderr.reconfigure(write_through=True)
    ...
    def cli(self, cmd, capture=False):
        ...
        # Flush output buffers before executing the command. This is important
        # if output streams are redirected to files, to ensure any output
        # generated by the task itself is written before any output from the
