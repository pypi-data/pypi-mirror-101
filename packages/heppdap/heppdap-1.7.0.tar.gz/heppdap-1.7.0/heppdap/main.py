...
class MainFrame(wx.Frame):
    ...
    def InitFrame(self):
        ...
        #Export Options        
        csvexportitem = fileMenu.Append(wx.ID_ANY, "Export CSV", "Export CSV")
        self.Bind(wx.EVT_MENU, self.panel.csv_export, csvexportitem)
        self.Bind(wx.EVT_MENU, self.panel.csv_export, id=ceid)
        excelexportitem = fileMenu.Append(wx.ID_ANY, "Export Excel", "Export Excel")
        self.Bind(wx.EVT_MENU, self.panel.excel_export, excelexportitem)
        self.Bind(wx.EVT_MENU, self.panel.excel_export, id=eeid)
...
#        fileSizer.Add(browser, 0, wx.ALL|wx.CENTER, 0)
        ...
        self.tsetting = wx.SpinCtrl(self, wx.ID_ANY, max = 500)
...
#          """
            ...
            try:
                    for event in self.data.events:
                        ...
                        for channel in event:
                                event_data.append(channel)
                        with open(pathname, 'a') as f:
                            fullevent = pd.concat(event_data, axis = 1)
                            fullevent.to_csv(f,index=True, header=False)
                            f.write("\n")
    ...
    def excel_export(self, event):
        with wx.FileDialog(self, "Save XYZ file", wildcard="XLSX files (*.xlsx)|*.xlsx",
                       style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT) as fileDialog:
        ...
        # save the current contents in the file
            ...
            pathname = fileDialog.GetPath()
            columnnames = []
            for b, board in enumerate(self.data.loadedchannels):
                for c, channel in enumerate(board):
                    columnnames.append(channelnames[b][c])
            ...
            try:
                ...
                for event in self.data.events:
                    ...
                    for channel in event:
                        event_data.append(channel)
                    fullevent = pd.concat(event_data, axis = 1)
                    fullevent.index.name = "Time (ns)"
                    fullevent.columns = columnnames
                    complete_events.append(fullevent)

                event_num = 1
                with pd.ExcelWriter(pathname, mode = 'wb') as writer:
                    for fullevent in complete_events:
                        fullevent.to_excel(writer,index=True, header=True, startcol = column)
                        event_num += 1 
                        column += self.numchannels + 2                   
                        if event_num == 10:
                           break 
