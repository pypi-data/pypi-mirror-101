...
from .utils import assert_valid_name, raise_resp_exception_error, requests_retry, _upload_local_files
import json
...
class TrojClient:
    def __init__(
        ...
        **kwargs,
        ...
        requests_retry.hooks['response'].append(self.reauth) # https://github.com/psf/requests/issues/4747 - Important for Retry vs urllib3
    ...
    def _get_creds_headers(self):
        ...
        if self._creds_id_token:
            return {"Authorization": f"Bearer {self._creds_id_token}", "x-api-key": f"{self._creds_api_key}"}
        else:
    ...
    def upload_dataset_files(self, image_filepaths: List, project_name: str, dataset_name: str):
        download_urls = []
        get_upload_path = (
            f"{self.api_endpoint}/project/{project_name}/dataset/{dataset_name}/get_s3_upload_url"
        )
    ...
    def upload_df_results(self, project_name: str, dataset_name: str, dataframe: dict):
        ...
        try:
            ...
            if dataset is not False:
                ...
                #if 403 call refresh_tokens func

                #if still 403 continue
                ...
                upload_response = requests.post(s3_payload_dict['url'], data=s3_payload_dict['fields'], files=files)
