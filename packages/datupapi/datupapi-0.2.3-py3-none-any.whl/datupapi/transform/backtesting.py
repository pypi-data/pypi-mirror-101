...
class Backtesting(Config):
    ...
    frr = Errors(config_file='config.yml', logfile='data_evaluation', log_path='output/logs')
    ...
    def format_backtests_export(self, df_backtests, intervals_fill=None):
        """
        ...
        >>> df = format_backtests_export(df_backtests, intervals_fill=['Lo95', 'Up95'])
        >>> df =
                        var1    var2    var3
                idx0     1       2       3
        """
    ...
    def create_backtest_dataset(self, df_backtests, backtest_name=None):
        ...
        :param backtest_name: Backtests name to distinguish oldest to newest backtest dataset. Choose from alpha, beta, gamma, delta & eps
        ...
        if backtest_name == 'alpha':
            ...
            date_ix = 1
        elif backtest_name == 'eps':
            date_ix = 0
        else:
            self.logger.exception(f'No valid backtest name. Choose from alpha, beta, gamma, delta or eps.')
    ...
    def compute_tracked_bias_error(self, df_back, error_types=['WMAPE', 'MASE']):
        ...
        try:
            for item in df_back['Item']:
                mask = df_back['Item'] == item
                forecast_col = 'Forecast ' + df_back.loc[mask, 'Suggested Interval'].values[0]
    ...
    def concat_backtest_datasets(self, backtests_names=None, datalake_path_=None):
        """
        Return a dataframe including all formatted backtest datasets

        :param backtests_names: List of backtests names to concat, e.g. alpha, beta, gamma, delta and eps
        ...
        >>> df = concat_backtest_datasets(backtests_names=['alpha', 'beta'], datalake_path_='path/to/backtests/folder')
        >>> df =
                        var1    var2    var3
                idx0     1       2       3
        """
        ...
        try:
            for backtest in backtests_names:
                df_back = self.io.download_csv(q_name='Qback-' + backtest,
                                               datalake_path=datalake_path_,
                                               )
                df_backtest = pd.concat([df_backtest, df_back], axis='rows')
