...
class Note:
    ...
    def __init__(self, name: str = None, semitones_above_middle_c: int = None):
        ...
        if name is not None:
            ...
            self.degree = interval_from_names[name]
        elif semitones_above_middle_c is not None:
            self.degree = semitones_above_middle_c
        else:
            self.degree = 0
    ...
    def name(self):
        ...
        local_degree = self.degree % 12
        octave = int((self.degree - local_degree) / 12) + 3
        return names_from_interval[local_degree] + str(octave)

    def __add__(self, shift: int):
        """The note found by shifting this note's degree upwards."""
        return Note(semitones_above_middle_c=self.degree + shift)

    def __sub__(self, shift: int):
        """The note found by shifting this note's degree downwards."""
        return self + (-shift)
