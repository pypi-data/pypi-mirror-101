...
try:
    ...
    print("You need to install hugging face transformers library")
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        # So we can broadcast to [batch_size, num_heads, —seq_length, to_seq_length]
...
class VilBERT(VILBertForVLTasks):
    def forward(
        self,   
        batch    
    ):
        question = batch["question"]
        input_imgs = batch["features"]
        image_loc = batch["boxes"]

        tokens = []
        for q in question:
            tokens = self._tokenizer.encode(q)
