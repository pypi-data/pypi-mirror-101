...
def time_before_request():
    ...
    if project_config and request.endpoint not in getattr(project_config, 'timer_logger_skip_endpoints', set()):
...
def time_after_request(res):
    ...
    if project_config and request.endpoint not in getattr(project_config, 'timer_logger_skip_endpoints', set()) \
        ...
        logger_over_secs = getattr(project_config, 'timer_logger_over_secs', False)
        response_time = time.time()
        wasted_time = response_time - g.request_time

        if logger_over_secs is None or isinstance(logger_over_secs, (int, float)) and wasted_time >= logger_over_secs:
            current_app.loggers(getattr(project_config, 'timer_logger_skip_endpoints', 'timer'))\
