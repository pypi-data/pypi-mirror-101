...
class ProjectConfig:
    ...
    time_logger_over_secs = 0  # disabled if False or negative nums; must logger if None or 0; logger when over positive nums
    time_logger_skip_endpoints = {'doc', 'specs', 'restx_doc.static', 'static'}  # set-typed
    time_logger_type = 'timer'


class LoggerConfig:
    app_id = ProjectConfig.id
    app_name = ProjectConfig.name
    log_path = '/app/log/'  # log_path = '/app/' + app_name + '/log/'
