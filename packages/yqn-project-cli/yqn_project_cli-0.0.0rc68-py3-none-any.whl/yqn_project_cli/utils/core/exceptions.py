...
class _Exception(HTTPException):
    ...
    def __init__(self, data=None, bizExtMap=None, code=200, header=None,
                 headers=None, msg="", msgCode="", msgDetail="", success=True, **kwargs):
        ...
        if header is None:
            ...
            if isinstance(req_data, dict) and 'header' in req_data:
                self.header = {
                    "xAppId": getattr(req_data['header'], "xAppId", ""),
                    "xTraceId": getattr(req_data['header'], "xTraceId", "")
                }
        else:
            self.header = header
