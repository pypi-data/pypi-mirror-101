...
class NFEnv(gym.Env):
    ...
    def __init__(self,show_gui=False,fall_rew=-10,motion_rew=0,const_rew=2,height_rew=-0.5,rand_force=None,mp4_path=None,foot_friction_lat=1,foot_friction_spin=1,pitch_rew=-1,roll_rew=-1,distance_rew=0,max_distance=0,power_rew=-0.1,min_height=0.5,max_pitch=0,max_roll=0,target_height=1.7,rand_force_freq=1000,speed_x_rew=-0.1,speed_y_rew=-0.3,speed_z_rew=0,speed_pitch_rew=0,speed_roll_rew=0,speed_yaw_rew=0,max_steps=5000,debug=False,base_mass=4,min_actions=[-0.15,-0.4,-0.15,-0.4],max_actions=[0.3,0,0.3,0],target_speed_x=0,target_speed_y=1,action_sim_steps=10):
        self.step_counter = 0
        if show_gui:
            mode=p.GUI
        else:
        ...
        self.min_actions=min_actions or [-1,-1,-1,-1]
        self.max_actions=max_actions or [1,1,1,1]
    ...
    def step(self, action):
        ...
        for i in range(self.action_sim_steps):
            p.stepSimulation()
        ...
        r_speed_x=abs(self.target_speed_x-lin_vel[0])*self.speed_x_rew
        reward+=r_speed_x
        r_speed_y=abs(self.target_speed_y-lin_vel[1])*self.speed_y_rew
        reward+=r_speed_y
