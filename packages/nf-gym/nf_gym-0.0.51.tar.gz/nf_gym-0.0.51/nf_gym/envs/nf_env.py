...
class NFEnv(gym.Env):
    ...
    def __init__(self,show_gui=False,fall_rew=-1000.0,motion_rew=0,const_rew=0,height_rew=1.0,rand_force=None,mp4_path=None,foot_friction_lat=1,foot_friction_spin=1,pitch_rew=-1,roll_rew=-1,distance_rew=0,max_distance=0,power_rew=-0.2,min_height=0.8,max_pitch=0.8,max_roll=0.8):
        self.step_counter = 0
        if show_gui:
            mode=p.GUI
        else:
    ...
    def step(self, action):
        ...
        #print("rewards: h=%s p=%s r=%s pow=%s => tot=%s"%(pos[2]*self.height_rew,abs(rot[0])*self.pitch_rew,abs(rot[1])*self.roll_rew,power*self.power_rew,reward))
