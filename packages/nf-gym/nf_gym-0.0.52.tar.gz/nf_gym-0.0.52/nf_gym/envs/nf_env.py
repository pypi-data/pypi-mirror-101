...
class NFEnv(gym.Env):
    ...
    def __init__(self,show_gui=False,fall_rew=-1000.0,motion_rew=0,const_rew=1,height_rew=-1.0,rand_force=None,mp4_path=None,foot_friction_lat=1,foot_friction_spin=1,pitch_rew=-1,roll_rew=-1,distance_rew=0,max_distance=0,power_rew=-0.2,min_height=0.8,max_pitch=0.8,max_roll=0.8,target_height=1.6,rand_force_freq=1000):
        self.step_counter = 0
        if show_gui:
            mode=p.GUI
        else:
    ...
    def step(self, action):
        ...
        reward=self.const_rew
        if self.prev_pos:
            reward+=(pos[1]-self.prev_pos[1])*self.motion_rew

        height=pos[2]
        print("height=%s"%height)
        reward+=abs(self.target_height-height)*self.height_rew

        if height<self.min_height:
            reward=self.fall_rew
            done=True

        reward+=abs(rot[0])*self.pitch_rew
        reward+=abs(rot[1])*self.roll_rew
        
        #print("power",power)
        reward+=power*self.power_rew

        print("rewards: h=%s p=%s r=%s pow=%s => tot=%s"%(abs(self.target_height-height)*self.height_rew,abs(rot[0])*self.pitch_rew,abs(rot[1])*self.roll_rew,power*self.power_rew,reward))

        dist=(pos[0]**2+pos[1]**2)**0.5
        if self.distance_rew:
            #print("dist",dist)
            reward+=dist*self.distance_rew

        #if pos[2]>2:
        #    reward=self.fall_rew
        #    done=True
        
        if abs(rot[0])>=self.max_pitch:
            reward=self.fall_rew
            done=True

        if abs(rot[1])>=self.max_roll:
            reward=self.fall_rew
            done=True
