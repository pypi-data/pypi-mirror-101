...
def load_data(star, lc_data=False, ps_data=False):
    ...
    if glob.glob(star.params['inpdir']+'%d_*' % star.name) != []:
        if star.verbose:
            print('')
            print('-------------------------------------------------')
            ...
            print('-------------------------------------------------')
        # Load light curve
        if not os.path.exists(star.params['inpdir']+'%d_LC.txt' % star.name):
            lc_data=False
            if star.verbose:
                print('Error: %s%d_LC.txt not found' % (star.params['inpdir'], star.name))
        else:
            lc_data=True
            star.time, star.flux = get_file(star.params['inpdir'] + '%d_LC.txt' % star.name)
        ...
        # Load power spectrum
        if not os.path.exists(star.params['inpdir'] + '%d_PS.txt' % star.name):
            ps_data=False
            if star.verbose:
                print('Error: %s%d_PS.txt not found' % (star.params['inpdir'], star.name))
        else:
            ps_data=True
            star.frequency, star.power = get_file(star.params['inpdir'] + '%d_PS.txt' % star.name)
