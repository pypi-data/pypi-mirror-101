...
public:
    ...
    static_assert(std::is_same<Type, float>::value || std::is_same<Type, double>::value  ||
                      std::is_same<Type, std::int32_t>::value || std::is_same<Type, std::uint32_t>::value,
    ...
    uniform() : uniform(static_cast<Type>(0.0),
                    std::is_integral<Type>::value ? (std::is_same<Method,
                    uniform_method::standard>::value ? (1 << 23) :
                    std::numeric_limits<Type>::max()): static_cast<Type>(1.0)) {}
...
public:
    ...
    struct param_type {
        ...
        dimen_(dimen), mean_(mean), matrix_(matrix) {}
        std::uint32_t dimen_;
    ...
    explicit gaussian_mv(std::uint32_t dimen, std::vector<RealType> mean,
        ...
        if ((matrix.size() != dimen * dimen) && (matrix.size() != dimen) && (matrix.size() != dimen * (dimen + 1) / 2)) {
...
public:
    ...
    struct param_type {
        param_type(RealType p, RealType q, RealType a, RealType b) : p_(p),
                q_(q), a_(a), b_(b) {}
        RealType p_;
        RealType q_;
        RealType a_;
...
public:
    ...
    struct param_type {
        param_type(RealType alpha, RealType a, RealType beta) : alpha_(alpha),
                a_(a), beta_(beta) {}
        RealType alpha_;
        RealType a_;
        RealType beta_;
