...
def _get_must_be_coded_on_msg(name, param, bit_number):
    """Returns a warning message if the given parameter is not in the
    expected bits range.
    ...
    if param not in range(limit_low, limit_high):
        return [
            f"- {name} must be in [{limit_low},{limit_high - 1}], "
            f"currently at {param}"
        ]
    return []
...
def _get_fully_connected_hw_incompatibilities(model, layer_index, nsoc_version):
    ...
    # Check threshold_fire parameter
    hw_msg += _get_must_be_coded_on_msg('threshold_fire', params.threshold_fire,
                                        20)
...
def _get_input_conv_hw_incompatibilities(layer, nsoc_version):
    ...
    # Check threshold_fire parameter
    hw_msg += _get_must_be_coded_on_msg('threshold_fire', p.threshold_fire, 24)
...
def _get_conv_hw_incompatibilities(model, layer_index, nsoc_version):
    ...
    # Check threshold_fire parameter
    hw_msg += _get_must_be_coded_on_msg('threshold_fire', p.threshold_fire, 20)
