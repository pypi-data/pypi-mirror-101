...
def touch_gui(H5_file_name, label_key):
    global im_ind
    global h
    global img
    global LABELS
    ...
    with h5py.File(H5_file_name, 'r') as h:
        def move(add_to):
            ...
            img = np.zeros(np.asarray(h['images'][0].shape) + np.asarray([10, 2, 0])).astype('int16')
            img = np.repeat(img, xx * 2 + 1, axis=1)
            img = np.concatenate((img, np.expand_dims(img[:, 0, :], axis=[1])), axis=1)
            img = img[:, 1:, :]
            W = h['images'][0].shape[1]
            for i, k in enumerate(index_out(h['images'], im_ind - xx, im_ind + xx + 1)):
                b = h['images'][k]
                l = LABELS[k]
                if l == -1:
                    pass
        ...
        root = Tk()
        im = None
        LABELS = copy.deepcopy(h[label_key][:])
        ...
        im_ind = -1
        image_tk = ImageTk.PhotoImage(image=Image.fromarray(h['images'][im_ind]))
        ...
        right_btn.grid(column=1, row=0, sticky=N, pady=5, padx=2)
        root.bind('<Right>', move_right)

        left_btn = ttk.Button(c, text="left", command=move_left)
        left_btn.grid(column=0, row=0, sticky=S, pady=5, padx=2)
        ...
        quit_btn = ttk.Button(c, text="QUIT-n-SAVE", command=root.destroy)
        quit_btn.grid(column=3, row=0, sticky=N, pady=5, padx=2)

        label_1 = ttk.Label(c)
        label_1.grid(column=0, row=10, sticky=N, pady=5, padx=5, columnspan=2, rowspan=1)
