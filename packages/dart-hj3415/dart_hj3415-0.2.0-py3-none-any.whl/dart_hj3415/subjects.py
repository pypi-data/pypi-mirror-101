...
class DartSubject:
    ...
    def scoring_유통주식(self):
        ...
        if 유통주식 <= DartSubject.최소유통주식기준:
            # 유통주식이 너무 작으면...
            self.point -= 1
            self.text += f"유통주식이 너무 작음 : {to_만(유통주식)}주\n"
    ...
    def correcting(self):
        ...
        if price <= low52 * 0.8:  # 최근 주가가 바닥이면....
            self.text += f'최근 주가({price}원)이 52주최저({low52}원)보다 20% 저렴.'
            correct_rate = 2
        elif low52 * 0.8 <= price <= low52 * 1.2:
            self.text += f'최근 주가({price}원)이 52주최저({low52}원)근처.'
            correct_rate = 1.5
        else:
            correct_rate = 1
...
class 특정증권등소유상황보고서(DartSubject):
    ...
    def process(self):
        """
        {'단가': 951.0, '임원': '등기임원', '주요주주': '10%이상주주', '증감': 630914, '직위명': '대표이사'}
        """
        super().process()
        ...
        self.data['증감'] = to_float(self.data['증감'])
        ...
        if math.isnan(self.data['단가']):
            ...
            self.data['취득처분총액'] = int(self.data['증감'] * self.data['단가'])
    ...
    def scoring(self):
        ...
        def check_past_dart() -> list:
            ...
            return_list = []
            for i, namedtuple in enumerate(df.itertuples()):
                past_subject = 특정증권등소유상황보고서(make_intro(dart_tuple=namedtuple), echo=False)
                past_subject.extract()
                past_subject.process()
                if self.echo:
                    print((f"\t- {i + 1}. Date : {namedtuple.rcept_dt}\t"
        ...
        if self.data['취득처분총액'] >= MIN_BUYING_COST and self.data['임원'] == '등기임원':
            ...
            if noticeable_case > 0:
                ...
                point += 1
                text += f'등기임원이 {to_억(MIN_BUYING_COST)}이상 취득했으나 최근 {DAYS_AGO}일내 유사 케이스 없음.'
        else:
            text += f"등기임원이 {to_억(MIN_BUYING_COST)}억원 이상 구매하지 않음."
        self.point, self.text = point, text
        return
