...
def custom_print(*args, sep=" ", end="", inline=False, il=False, inline_end=False, ile=False, new_line=True, nl=True,
                 return_string=False, wait=0):
    """
    costum print
    ...
    :param bool return_string: print or return the string
    :param wait: wait time after print in seconds
    :return string: if return_string true returns string
    """
    pre = ""
    suf = ""
    string = ""
    ...
    for i, arg in enumerate(args, start=0):
        ...
        if arg.find("\033") == -1:
            if i != length - 1:
                string += sep
    string = f"{pre}{string}{PT.RESET}{suf}"
    if return_string:
        return string
    else:
