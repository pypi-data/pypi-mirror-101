...
class SpellingErrorCorrection(object):
    ...
    def build_DAE(cls, n_features, window_size, noise_rate, n_hidden1, learning_rate, watch=WatchUtil()):
        if len(cls.graph) == 0:
            ...
            # layers = 2
            W1 = tf.Variable(tf.random.normal([n_features, n_hidden1]), name='W1', dtype=tf.float32)
            b1 = tf.Variable(tf.random.normal([n_hidden1]), name='b1', dtype=tf.float32)
            layer1 = tf.nn.sigmoid(tf.compat.v1.matmul(X, W1) + b1, name='layer1')
            layer1_dropout = tf.nn.dropout(layer1, 1 - (dropout_keep_prob), name='layer1_dropout')

            W2 = tf.Variable(tf.random.normal([n_hidden1, n_features]), name='W2')
            b2 = tf.Variable(tf.random.normal([n_features]), name='b2')
            ...
            cost = tf.reduce_mean(input_tensor=tf.compat.v1.nn.softmax_cross_entropy_with_logits(logits=labels_hat, labels=labels), name='cost')
            train_step = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)

            accuracy = tf.reduce_mean(input_tensor=tf.compat.v1.cast(tf.abs(tf.nn.softmax(y_hat) - Y) < 0.1, dtype=tf.float32), name='accuracy')
            # log.debug('X:', X)
            # log.debug('Y:', Y)
