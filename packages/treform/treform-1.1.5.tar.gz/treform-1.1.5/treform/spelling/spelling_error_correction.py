...
class SpellingErrorCorrection(object):
    ...
    def build_DAE(cls, n_features, window_size, noise_rate, n_hidden1, learning_rate, watch=WatchUtil()):
        if len(cls.graph) == 0:
            ...
            X = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, n_features], name='X')  # shape=(batch_size, window_size * feature_vector.size)
            Y = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, n_features], name='Y')  # shape=(batch_size, window_size * feature_vector.size)
            dropout_keep_prob = tf.compat.v1.placeholder(tf.compat.v1.float32)

            # layers = 2
            W1 = tf.compat.v1.Variable(tf.compat.v1.random_normal([n_features, n_hidden1]), name='W1')
            b1 = tf.compat.v1.Variable(tf.compat.v1.random_normal([n_hidden1]), name='b1')
            ...
            W2 = tf.compat.v1.Variable(tf.compat.v1.random_normal([n_hidden1, n_features]), name='W2')
            b2 = tf.compat.v1.Variable(tf.compat.v1.random_normal([n_features]), name='b2')
            y_hat = tf.compat.v1.add(tf.compat.v1.matmul(layer1_dropout, W2), b2, name='y_hat')  # shape=(batch_size, window_size * feature_vector.size)

            labels_hat = tf.compat.v1.reshape(y_hat, shape=(-1, window_size, features_vector_size))  # shape=(batch_size, window_size, feature_vector.size)
            labels = tf.compat.v1.reshape(Y, shape=(-1, window_size, features_vector_size))  # shape=(batch_size, window_size, feature_vector.size)

            cost = tf.compat.v1.reduce_mean(tf.compat.v1.nn.softmax_cross_entropy_with_logits(logits=labels_hat, labels=labels), name='cost')
            train_step = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)

            accuracy = tf.compat.v1.reduce_mean(tf.compat.v1.cast(tf.compat.v1.abs(tf.compat.v1.nn.softmax(y_hat) - Y) < 0.1, dtype=tf.compat.v1.float32), name='accuracy')
            # log.debug('X:', X)
            # log.debug('Y:', Y)
...
if __name__ == '__main__':
    ...
    try:
        ...
        total_epoch = max(5, 20 // window_size)  # 10 ~ 100 # window_size 가 클 수록 total_epoch는 작아도 됨.
