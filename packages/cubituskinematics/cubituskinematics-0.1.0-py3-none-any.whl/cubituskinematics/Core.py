...
class Core(object):
    ...
    def __init__(self, endEffector=[0.0, 0.0, 0.0]):
        ...
        self.__gripper = 0.0
        self.__xmlPath = f"{os.path.dirname(os.path.abspath(__file__))}/cubitus.xml"
    ...
    def __PlotData(self, alpha, beta, gamma, delta, epsilon, phi, inv=False, geometry=None, isolatedPoints=None, threePoints=None):
        ...
        fk.SetDefaultParameters(self.__xmlPath)
    ...
    def LoadXML(self, fileName, name):
        self.__xmlPath = f"{os.path.dirname(os.path.abspath(fileName))}/{name}.xml"
