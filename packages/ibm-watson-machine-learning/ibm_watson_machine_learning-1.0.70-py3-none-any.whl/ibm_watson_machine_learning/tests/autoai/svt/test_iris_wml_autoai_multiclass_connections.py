...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_02_save_remote_data_and_DataConNection_setup(self):
        if self.wml_client.ICP:
            ...
            #     connection=ConnectionAsset(id='66d1d6c9-aa49-4583-b020-f1d7eaf416c1'),
            #     location=ConnectionAssetLocation(bucket='somebucket2', file_name='bank.csv'))

            TestAutoAIRemote.data_connection = DataConnection(
                location=AssetLocation(asset_id='e4f316b9-115e-4926-83ff-e56e3233271e'))
    ...
    def test_09_predict_using_fitted_pipeline(self):
        predictions = self.remote_auto_pipelines.predict(X=self.train_data.drop(['y'], axis=1).values[:5])
        print(predictions)
