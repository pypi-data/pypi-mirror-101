...
from ..flight.utils import get_data_asset_attachment
...
class DataConnection(BaseDataConnection):
    ...
    def __init__(self,
        ...
        elif connection_asset_id is not None and isinstance(location, S3Location):
    ...
    def read(self,
        ...
        if self.type == DataConnectionTypes.DS and self._wml_client is None:
        ...
        if self.type == DataConnectionTypes.S3:
            ...
            if self._check_if_connection_asset_is_s3():
                ...
                try:
                    ...
                    attachment = get_data_asset_attachment(self.location.to_dict(), self._wml_client)

                    flight_service = FlightService(
                        wml_client=self._wml_client,
                        entity=self._wml_client.connections.get_details(attachment['connection_id'])['entity'],
                        attachment=get_data_asset_attachment(self.location.to_dict(), self._wml_client),
...
class AssetLocation(BaseLocation):
    ...
    def _get_connection_id(self, client) -> str:
        ...
        attachment_url = client.data_assets._href_definitions.get_data_asset_href(self.id)
    ...
    def _get_file_size(self, workspace: 'WorkSpace', *args) -> 'int':
        if self._wsd:
            ...
            asset_info_response = requests.get(
                workspace.wml_client.data_assets._href_definitions.get_data_asset_href(self.id),
