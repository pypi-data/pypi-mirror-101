...
def get_data_asset_attachment(data_location: dict, wml_client: 'APIClient') -> dict:
    ...
    if wml_client.ICP:
        attachment = requests.get(attachment_url, params=wml_client._params(), headers=wml_client._get_headers(), verify=False)
    ...
    if 'secret_key' in connection_details['entity']['properties']:
        ...
        if out:
            raise UnsupportedOutputConnection(
                connection_id=attachment_json['connection_id'] if'connection_id' in attachment_json else data_location['connection']['id'],
