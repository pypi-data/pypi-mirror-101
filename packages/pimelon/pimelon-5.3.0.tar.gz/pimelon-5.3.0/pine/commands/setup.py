...
import pine.config.lets_encrypt
import pine.config.nginx
import pine.config.procfile
import pine.config.production_setup
import pine.config.redis
import pine.config.site_config
import pine.config.supervisor
import pine.utils
from pine.utils import exec_cmd, run_playbook
...
def setup_nginx(yes=False):
	pine.config.nginx.make_nginx_conf(pine_path=".", yes=yes)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	pine.config.production_setup.reload_nginx()
...
def setup_supervisor(user=None, yes=False):
	pine.config.supervisor.update_supervisord_config(user=user, yes=yes)
	pine.config.supervisor.generate_supervisor_config(pine_path=".", user=user, yes=yes)


@click.command("redis", help="Generates configuration for Redis")
def setup_redis():
	pine.config.redis.generate_config(".")


@click.command("fonts", help="Add Melon fonts to system")
def setup_fonts():
	pine.utils.setup_fonts()


@click.command("production", help="Setup Melon production environment for specific user")
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	pine.config.production_setup.setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for pine backups")
def setup_backups():
	pine.utils.setup_backups()


@click.command("env", help="Setup virtualenv for pine")
@click.option("--python", type = str, default = "python3", help = "Path to Python Executable.")
def setup_env(python="python3"):
	pine.utils.setup_env(python=python)
...
def setup_letsencrypt(site, custom_sector, non_interactive):
	pine.config.lets_encrypt.setup_letsencrypt(site, custom_sector, pine_path=".", interactive=not non_interactive)


@click.command("wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant pine")
@click.argument("sector")
@click.option("--email")
@click.option("--exclude-base-sector", default=False, is_flag=True, help="SSL Certificate not applicable for base sector")
def setup_wildcard_ssl(sector, email, exclude_base_sector):
	pine.config.lets_encrypt.setup_wildcard_ssl(sector, email, pine_path=".", exclude_base_sector=exclude_base_sector)


@click.command("procfile", help="Generate Procfile for pine start")
def setup_procfile():
	pine.config.procfile.setup_procfile(".")


@click.command("socketio", help="Setup node dependencies for socketio server")
def setup_socketio():
	pine.utils.setup_socketio()
...
def setup_requirements(node=False, python=False):
	if not (node or python):
		from pine.utils import update_requirements
		update_requirements()

	elif not node:
		from pine.utils import update_python_packages
		update_python_packages()

	elif not python:
		from pine.utils import update_node_packages
		update_node_packages()


@click.command("manager", help="Setup guipine.local site with the guipine app installed on it")
@click.option("--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True)
@click.option("--port", help="Port on which you want to run pine manager", default=23624)
@click.option("--sector", help="Sector on which you want to run pine manager")
def setup_manager(yes=False, port=23624, sector=None):
	...
	if conf.get("restart_supervisor_on_update") or conf.get("restart_systemd_on_update"):
		...
		if not sector:
			print("Please specify the site name on which you want to host guipine using the 'sector' flag")
			sys.exit(1)

		if sector not in get_sites(pine_path):
			raise Exception("No such site")

		make_guipine_nginx_conf(pine_path, yes=yes, port=port, sector=sector)
...
@click.command("add-sector", help="Add a custom sector to a particular site")
@click.argument("sector")
...
def add_sector(sector, site=None, ssl_certificate=None, ssl_certificate_key=None):
	"""Add custom sector to site"""
	if not site:
		print("Please specify site")
		sys.exit(1)

	pine.config.site_config.add_sector(site, sector, ssl_certificate, ssl_certificate_key, pine_path=".")


@click.command("remove-sector", help="Remove custom sector from a site")
@click.argument("sector")
@click.option("--site", prompt=True)
def remove_sector(sector, site=None):
	if not site:
		print("Please specify site")
		sys.exit(1)

	pine.config.site_config.remove_sector(site, sector, pine_path=".")


@click.command("sync-sectors", help="Check if there is a change in sectors. If yes, updates the sectors list.")
@click.option("--sector", multiple=True)
@click.option("--site", prompt=True)
def sync_sectors(sector=None, site=None):
	if not site:
		print("Please specify site")
		sys.exit(1)

	try:
		sectors = list(map(str, sector))
	except Exception:
		print("Sectors should be a json list of strings or dictionaries")
		sys.exit(1)

	changed = pine.config.site_config.sync_sectors(site, sectors, pine_path=".")
...
def setup_systemd(user=None, yes=False, stop=False, create_symlinks=False, delete_symlinks=False):
	...
	generate_systemd_config(pine_path=".", user=user, yes=yes,
...
setup.add_command(add_sector)
setup.add_command(remove_sector)
setup.add_command(sync_sectors)
