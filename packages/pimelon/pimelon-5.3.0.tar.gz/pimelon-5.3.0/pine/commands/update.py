...
from pine.utils import post_upgrade, patch_sites, build_assets


@click.command('update', help="Performs an update operation on current pine. Without any flags will backup, pull, setup requirements, build, run patches and restart pine. Using specific flags will only do certain tasks instead of all")
...
@click.option('--requirements', is_flag=True, help="Update requirements. If run alone, equivalent to `pine setup requirements`")
...
def switch_to_branch(branch, apps, upgrade=False):
	from pine.app import switch_to_branch
	switch_to_branch(branch=branch, apps=list(apps), upgrade=upgrade)


@click.command('switch-to-master', help="[DEPRECATED]: Switch melon and monak to master branch")
def switch_to_master():
	from pine.utils import log
	log("`switch-to-master` has been deprecated as master branches were renamed to version-4")


@click.command('switch-to-develop')
def switch_to_develop(upgrade=False):
	"Switch melon and monak to develop branch"
	from pine.app import switch_to_develop
