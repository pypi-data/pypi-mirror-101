...
import pine
from pine.utils import get_pine_name, get_sites


def make_nginx_conf(pine_path, yes=False):
	...
	config = pine.config.common_site_config.get_config(pine_path)
...
def make_guipine_nginx_conf(pine_path, yes=False, port=23624, sector=None):
	...
	site_config = get_site_config(sector, pine_path=pine_path)
	pine_name = get_pine_name(pine_path)
	...
	if not yes and os.path.exists(conf_path):
		click.confirm('nginx.conf already exists and guipine configuration will be appended to it. Do you want to continue?',
			abort=True)
	...
	for site in sites_configs:
		if dns_multitenant:
			...
			if sector:
				...
				sector_map[sector] = site['name']

			site_name = sector or site['name']
...
def get_sites_with_config(pine_path):
	...
	sites = get_sites(pine_path=pine_path)
	dns_multitenant = get_config(pine_path).get('dns_multitenant')
	...
	for site in sites:
		try:
			...
			strict_nginx = get_config(pine_path).get('strict_nginx')
			if strict_nginx:
		...
		if dns_multitenant and site_config.get('sectors'):
			for sector in site_config.get('sectors'):
				# sector can be a string or a dict with 'sector', 'ssl_certificate', 'ssl_certificate_key'
				if isinstance(sector, string_types):
...
def use_wildcard_certificate(pine_path, ret):
	...
	for site in ret:
		...
		if (site.get('sector') or site['name']).endswith(sector):
