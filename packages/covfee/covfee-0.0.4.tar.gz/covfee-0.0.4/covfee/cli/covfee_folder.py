...
class CovfeeFolder:
    ...
    def launch_in_browser(self):
        if which('xdg-open') is not None:
            ...
            print(Fore.GREEN +
                  f' * covfee is available at {app.config["ADMIN_URL"]}')
    ...
    def launch_prod(self, unsafe=False, build=False, launch_browser=False):
        ...
        if launch_browser:
            self.launch_in_browser()

        if unsafe:
            os.environ['UNSAFE_MODE_ON'] = 'enable'
        os.environ['FLASK_ENV'] = 'production'
        os.environ['FLASK_APP'] = 'covfee.server.start:create_app'
        os.system(f'gunicorn --workers 8 -b {app.config["SERVER_SOCKET"]} \'covfee.server.start:create_app()\'')
