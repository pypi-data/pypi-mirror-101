...
# -*- coding:utf-8 -*-
import numpy as np
from natf.utils import mcnp_style_str_append
...
class Surface(object):
    ...
    def __init__(self, name=None, surf_id=None, flag=None, mnemoic=None, card_entries=None):
        if name is not None:
            self._name = name
        if surf_id is not None:
            self._surf_id = surf_id # id
        if flag is not None:
    ...
    @property
    def surf_id(self):
        return self._surf_id

    @surf_id.setter
    def surf_id(self, value):
    ...
    def __str__(self):
        ...
        s = ''.join([self.flag, str(self.surf_id), '     ', self.mnemoic])
        indent_length = len(s)
        indent_str = ' '*indent_length
        for i, value in enumerate(self.card_entries):
            s = mcnp_style_str_append(s, value, indent_length)
        s = ''.join([s, '\n'])
        return s
