...
class EmbeddedMultisigAccountModificationTransactionBuilder(EmbeddedTransactionBuilder):
    ...
    type_hints = {
        ...
        'type' : 'EntityTypeDto',
        'address_additions' : 'array',
        'address_deletions' : 'array',
    }
    ...
    @classmethod
    def load_from_binary(cls, payload: bytes) -> EmbeddedMultisigAccountModificationTransactionBuilder:
        ...
        body = MultisigAccountModificationTransactionBodyBuilder.load_from_binary(bytes_)  # kind:CUSTOM1_nonbyte
        bytes_ = bytes_[body.get_size():]

        # create object and call
        result = EmbeddedMultisigAccountModificationTransactionBuilder(superObject.signer_public_key, superObject.network)
    ...
    def min_removal_delta(self):
        return self.body.min_removal_delta

    @min_removal_delta.setter
    def min_removal_delta(self, min_removal_delta): # MARKER1 AttributeKind.SIMPLE
        self.body.min_removal_delta = min_removal_delta

    @property
    def min_approval_delta(self):
        return self.body.min_approval_delta

    @min_approval_delta.setter
    def min_approval_delta(self, min_approval_delta): # MARKER1 AttributeKind.SIMPLE
        self.body.min_approval_delta = min_approval_delta

    @property
    def address_additions(self):
        return self.body.address_additions

    @property
    def address_deletions(self):
        return self.body.address_deletions
