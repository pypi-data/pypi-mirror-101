...
class MosaicAliasTransactionBodyBuilder:
    ...
    def loadFromBinary(cls, payload: bytes) -> MosaicAliasTransactionBodyBuilder:
        ...
        namespace_id_ = NamespaceIdDto.loadFromBinary(bytes_)  # kind:CUSTOM1_byte
        namespace_id = namespace_id_.namespaceId
        bytes_ = bytes_[namespace_id_.getSize():]
        mosaic_id_ = MosaicIdDto.loadFromBinary(bytes_)  # kind:CUSTOM1_byte
        mosaic_id = mosaic_id_.mosaicId
        bytes_ = bytes_[mosaic_id_.getSize():]
        alias_action_ = AliasActionDto.loadFromBinary(bytes_)  # kind:CUSTOM2
        alias_action = alias_action_.value
        bytes_ = bytes_[alias_action_.getSize():]
    ...
    def getSize(self) -> int:
        ...
        """
        size = 0
        size += NamespaceIdDto(self.namespace_id).getSize()
        size += MosaicIdDto(self.mosaic_id).getSize()
        size += AliasActionDto(self.alias_action).getSize()
        return size
    ...
    def serialize(self) -> bytes:
        ...
        """
        bytes_ = bytes()
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, NamespaceIdDto(self.namespace_id).serialize())  # kind:CUSTOM
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, MosaicIdDto(self.mosaic_id).serialize())  # kind:CUSTOM
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, AliasActionDto(self.alias_action).serialize())  # kind:CUSTOM
        return bytes_
    ...
    def __str__(self):
        ...
        """
        result = ''
        result += '{:24s} : {}\n'.format('namespaceId', to_hex_string(NamespaceIdDto(self.namespace_id).serialize()))
        result += '{:24s} : {}\n'.format('mosaicId', to_hex_string(MosaicIdDto(self.mosaic_id).serialize()))
        result += '{:24s} : {}\n'.format('aliasAction', to_hex_string(AliasActionDto(self.alias_action).serialize()))
        return result
