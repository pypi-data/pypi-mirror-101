...
class MultisigAccountModificationTransactionBodyBuilder:
    ...
    def loadFromBinary(cls, payload: bytes) -> MultisigAccountModificationTransactionBodyBuilder:
        ...
        min_removal_delta = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 1))  # kind:SIMPLE
        bytes_ = bytes_[1:]
        min_approval_delta = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 1))  # kind:SIMPLE
        bytes_ = bytes_[1:]
        address_additions_count = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 1))  # kind:SIZE_FIELD
        bytes_ = bytes_[1:]
        address_deletions_count = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 1))  # kind:SIZE_FIELD
        bytes_ = bytes_[1:]
        multisig_account_modification_transaction_body__reserved1 = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 4))  # kind:SIMPLE
        bytes_ = bytes_[4:]
    ...
    def serialize(self) -> bytes:
        ...
        """
        bytes_ = bytes()
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, GeneratorUtils.uintToBuffer(self.min_removal_delta, 1))  # serial_kind:SIMPLE
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, GeneratorUtils.uintToBuffer(self.min_approval_delta, 1))  # serial_kind:SIMPLE
    ...
    def __str__(self):
        ...
        """
        result = ''
        result += '{:24s} : {}\n'.format('minRemovalDelta', to_hex_string(GeneratorUtils.uintToBuffer(self.min_removal_delta, 1)))
        result += '{:24s} : {}\n'.format('minApprovalDelta', to_hex_string(GeneratorUtils.uintToBuffer(self.min_approval_delta, 1)))
        size_value = len(self.address_additions)
        result += '{:24s} : {}\n'.format('addressAdditionsCount', to_hex_string(GeneratorUtils.uintToBuffer(size_value, 1)))
        size_value = len(self.address_deletions)
        result += '{:24s} : {}\n'.format('addressDeletionsCount', to_hex_string(GeneratorUtils.uintToBuffer(size_value, 1)))
