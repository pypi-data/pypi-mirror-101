...
class MessageProcessor:
    ...
    def create_metadata(
        ...
        if metadata and metadata.get("message_type"):
            metadata["message_type"] = metadata.get("message_type")
        else:
    ...
    async def _handle_message_with_tracker(
        ...
        if message.metadata['message_type']:
            isDTMF = message.metadata['message_type'] == 'DTMF'
        
        redacted_text = message.text
        redacted_entities =  copy.deepcopy(parse_data['entities'])

        if '/greet' not in redacted_text and isDTMF != True:
