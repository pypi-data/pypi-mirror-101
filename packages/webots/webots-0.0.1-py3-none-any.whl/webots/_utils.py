...
def get_tmp_dir(simulation_id=None):
    if simulation_id is None:
        simulation_id = get_random_string()
    return os.path.join('/tmp', 'webots_' + get_random_string())
...
def is_webots_in_lib_path():
    webots_home = get_webots_home()
    path_to_prepend = os.path.join(webots_home, 'lib', 'controller')
    if sys.platform == 'linux':
        if 'LD_LIBRARY_PATH' not in os.environ:
            return False
        if path_to_prepend not in os.environ['LD_LIBRARY_PATH']:
            return False
    return True
...
def append_webots_lib_to_path():
    ...
    path_to_prepend = os.path.join(webots_home, 'lib', 'controller')
    if sys.platform == 'linux':
        current_path = os.environ.get('LD_LIBRARY_PATH') if 'LD_LIBRARY_PATH' in os.environ else ''
        if path_to_prepend not in current_path:
            os.environ['LD_LIBRARY_PATH'] = ':'.join([path_to_prepend, current_path])
            print(f'Please call `export LD_LIBRARY_PATH={path_to_prepend}:$LD_LIBRARY_PATH`')
        return True
    elif sys.platform == 'darwin':
        os.environ['DYLD_LIBRARY_PATH'] = os.path.join(webots_home, 'lib', 'controller') + ':' + os.environ.get('DYLD_LIBRARY_PATH')
        return True
