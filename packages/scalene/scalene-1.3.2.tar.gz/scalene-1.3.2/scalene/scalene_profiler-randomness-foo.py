...
class Scalene:
    ...
    __last_cpu_sampling_rate: float = random.random() * 0.01
    ...
    def enable_signals() -> None:
        ...
        # Turn on the CPU profiling timer to run on average every cpu_sampling_rate seconds.
        signal.setitimer(
            ScaleneSignals.cpu_timer_signal,
            #Scalene.__args.cpu_sampling_rate,
            #Scalene.__args.cpu_sampling_rate,
            Scalene.__last_cpu_sampling_rate,
            Scalene.__last_cpu_sampling_rate,
        )
    ...
    def cpu_signal_handler_helper(
        ...
        if use_low_discrepancy:
            # next_interval = ((0.6180339887498949 + Scalene.__last_cpu_sampling_rate / Scalene.__args.cpu_sampling_rate) % 1.0) * (2.0 * Scalene.__args.cpu_sampling_rate)
            # generate a geometric distribution
            q = Scalene.__args.cpu_sampling_rate
            # rnd = random.random()
            rnd = ((0.6180339887498949 * Scalene.iteration + Scalene.alpha) % 1.0)
            Scalene.iteration += 1
            # print(rnd)
            next_interval = (rnd + 0.5) * q
            #next_interval = 1.0 / (math.log(1.0 - rnd) / math.log(1 - q))
            #print(next_interval)
        else:
            # Pick a new random interval, distributed around the mean.
            next_interval = 0.0
            while next_interval > 1.0: # FIXME <= 0.0:
