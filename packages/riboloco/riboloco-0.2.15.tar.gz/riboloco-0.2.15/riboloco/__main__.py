        ...
        if E_site_test + 6 in these_ratios.keys():
            ...
            for ref in ref_APEs.keys():
                if not ref == this_type + "_added":  # ensure no self referencing
                    cors.append(round(find_cor(ref_APEs[ref], test_d, args.use_kl_not_r), 4))
...
def find_cor(ref, test_d, use_kl_not_r):
    """
    ...
    if not use_kl_not_r:  # use r
        ...
        for site in sites:
            for codon in codons:
                if not (codon == "TGA" or codon == "TAA" or codon == "TAG"):
                    refs.append(ref[site][codon])
                    tests.append(test_d[site][codon])

        print(refs)
        print(tests)

        refs = [a/sum(refs) for a in refs]
        tests = [a/sum(tests) for a in tests]
        score = -sum([a*np.log2(a/b) for a, b in zip(tests, refs) if b > 0])
...
def main():
    ...
    parser.add_argument("--use_kl_not_r", default=False, action="store_true",
                        help="Use KL divergence instead of correlation")
