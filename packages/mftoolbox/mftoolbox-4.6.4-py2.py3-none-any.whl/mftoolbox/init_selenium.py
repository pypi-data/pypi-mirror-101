...
class Browser(webdriver.Chrome):
    ...
    def clear_cache(self):
        ...
        try:
            self.__root1 = self.find_element_by_tag_name("settings-ui")
            self.__shadow_root1 = self.__expand_shadow_element(self, self.__root1)
            self.__root2 = self.__shadow_root1.find_element_by_css_selector("settings-main#main")
            self.__shadow_root2 = self.__expand_shadow_element(self, self.__root2)
            self.__root3 = self.__shadow_root2.find_element_by_css_selector("settings-basic-page[role='main']")
            self.__shadow_root3 = self.__expand_shadow_element(self, self.__root3)
            ...
            self.__shadow_root4 = self.__expand_shadow_element(self, self.__root4)
            time.sleep(1)
            self.__root5 = self.__shadow_root4.find_element_by_css_selector('settings-clear-browsing-data-dialog')
            self.__shadow_root5 = self.__expand_shadow_element(self, self.__root5)
            ...
            self.__shadow_root6 = self.__expand_shadow_element(self, self.__webelement2)
           ...
           # click button
            ...
            try:
                ...
                if self.current_url == 'chrome://settings/':
                    pass
                else:
                    raise Sair
    ...
    def __init__(self, hide=True, clean=True, **kwargs):
        ...
        if kill_chromedrivers:
            ...
            for linha in tabela:
                if linha.find('chromedriver.exe') != -1:
                    try:
                        # para evitar mandar mensagem de erro ou de sucesso para a tela
                        subprocess.Popen("taskkill /f /im  chromedriver.exe",stdout = subprocess.PIPE,stderr = subprocess.PIPE)
                    except:
                        pass
        ...
        # self.current_window_handlee = Browser.current_window_handle
