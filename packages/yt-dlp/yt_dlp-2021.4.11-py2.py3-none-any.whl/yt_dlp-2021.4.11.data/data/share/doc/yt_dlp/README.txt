...
[Release version] [License: Unlicense] [Doc Status] [Discord]
...
-   MERGED WITH YOUTUBE-DL V2021.04.07: You get all the latest features
...
Python versions 2.6, 2.7, or 3.2+ are currently supported. However, 3.2+
is strongly recommended and python2 support will be deprecated in the
future.

Although there are no required dependancies, ffmpeg and ffprobe are
highly recommended. Other optional dependancies are sponskrub,
AtomicParsley, mutagen, pycryptodome and any of the supported external
downloaders. Note that the windows releases are already built with the
python interpreter, mutagen and pycryptodome included.
...
formatting by specifying it inside the parantheses separated from the
...
-f worstvideo. It is however recomended not to use worst and related
...
suffix a prefered value for the fields, separated by a :. Eg: res:720
...
You can also use this to change only the metadata that is embedded in
the media file. To do this, set the value of the corresponding field
with a meta_ prefix. For example, any value you set to meta_description
field will be added to the description field in the file. You can use
this to set a different "description" and "synopsis", for example.
