...
def extractor_registry():
    return {
        ...
        'deep_dive': (DeepDive, DeepdiveExtractor),
...
class DeepdiveExtractor(Extractor):

    def extract_model_data(self, raw_data):
        object_title = self.get_object_title(raw_data)
        if not object_title:
            focus_title = raw_data.get('focus_title', '')
            if not focus_title:
                return None
            else:
                raw_data['title'] = focus_title

        return super(DeepdiveExtractor, self).extract_model_data(raw_data)
