...
from encodings import ascii  # necessary import for file open to work in some environments
...
class BioLib:
    ...
    def attempt_login(exit_on_failure=False):
        ...
        response = requests.post(f'{BioLib.host}/api/user/token/',
        ...
        if not response.ok:
            ...
            BioLib.auth_refresh_token = json_response['refresh']
            BioLib.auth_access_token = json_response['access']
    ...
    def push(author_and_app_name, app_path):
        ...
        if Path(zip_filename).is_file():
            biolib_logger.error(
                f'Please make sure you do not have a file called {zip_filename} in the application directory')
            sys.exit(1)
        ...
        if docker_tags:
            ...
            # the final step of docker push can take a long time, so set a long timeout for docker
            # 15 min (900s) is the maximum supported by AWS load balancers
            docker_client = docker.from_env(timeout=900)
    ...
    def start_compute_node(port, host, eif_path=None):
        if eif_path:
            ...
            subprocess.run(
                ['sudo', 'nitro-cli', 'run-enclave', '--cpu-count', '2', '--memory', '8192', '--eif-path', eif_path],
                check=True
            )
            enclave_json_string = subprocess.run(['nitro-cli', 'describe-enclaves'],
                                                 check=True,
                                                 capture_output=True).stdout
            enclave_id = json.loads(enclave_json_string)[0]['EnclaveID']
            subprocess.run(['sudo', 'nitro-cli', 'terminate-enclave', '--enclave-id', f'{enclave_id}'], check=True)
