...
def clear_cache():
    get_masked.cache_clear()  # type: ignore
    get_unmasked.cache_clear()  # type: ignore
    yield
    get_masked.cache_clear()  # type: ignore
    get_unmasked.cache_clear()  # type: ignore
...
def test_mask(setup):
    ...
    with open(
        os.path.join(env.prefix().PORTMOD_CONFIG_DIR, "package.mask"), "w"
    ) as file:
...
def test_unmask(setup):
    ...
    with open(
        os.path.join(env.prefix().PORTMOD_CONFIG_DIR, "package.unmask"), "w"
    ) as file:
