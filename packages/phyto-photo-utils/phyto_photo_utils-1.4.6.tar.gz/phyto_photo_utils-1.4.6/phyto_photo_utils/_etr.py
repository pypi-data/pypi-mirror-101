...
def calculate_amplitude_etr(fo, fm, sigma, par, alpha_phase=True, light_independent=True, dark_sigma=False, etrmax_fitting=True, serodio_sigma=False, light_step_size=None, last_steps_average=False, skip_first_light_step=False, outlier_multiplier=3, return_data=False, bounds=True, alpha_lims=[0,4], etrmax_lims=[0,2000], method='trf', loss='soft_l1', f_scale=0.1, max_nfev=None, xtol=1e-9):
	...
	skip_first_light_step : bool, default=False,
		If True, will skip the first light step in cases of large changes in F\ :sub:`v`/F\ :sub:`m` between first and second light step for performing light independent calculations.
	...
	if light_step_size == 1:
		if light_independent:
			if skip_first_light_step:
				etr = fvfm / fvfm[1]

				print('Skipping first light step')
