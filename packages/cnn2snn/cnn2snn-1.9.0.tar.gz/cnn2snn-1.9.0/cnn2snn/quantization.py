...
def quantize(model,
    ...
    # Raise warning
    _check_inconsistent_fold_BN(model_t, fold_BN)
    if fold_BN:
...
def _check_inconsistent_fold_BN(model_inverted_mp_bn, fold_BN):
    """Raises a warning message if 'fold_BN' argument is not consistent with the
    model.

    Warning is raised in two configurations:
      - a quantized neural layer with a BN layer is present and fold_BN = True.
      - a float neural layer with a BN layer is present and fold_BN = False.

    In both cases, we advise to retrain on a few batches to align BN parameters
    with the quantized weights.

    Args:
        model_inverted_mp_bn (:obj:`tf.keras.Model`): a Keras model to check.
            This model must have BN layers directly after neural layers. This is
            done by first applying the "invert_batchnorm_pooling" transformation
        fold_BN (bool): whether BN layers must be folded or not
    """
    warnings.simplefilter('always', UserWarning)

    neural_layers_to_fold_bn, _ = _find_batchnorms_to_fold(model_inverted_mp_bn)
    for neural_layer in neural_layers_to_fold_bn:
        if fold_BN and type(neural_layer) in cnn2snn_neural_layers:
            warnings.warn(
                "The model contains the quantized neural layer (layer "
                f"{neural_layer.name} of type {neural_layer.__class__.__name__}"
                f") followed by a BatchNormalization layer. This configuration"
                " is not consistent with fold_BN=True. This might require "
                "retraining on a few batches to align BatchNormalization "
                "parameters with the quantized weights.")
        elif not fold_BN and type(neural_layer) in keras_neural_layers:
            warnings.warn(
                "The model contains the float neural layer (layer "
                f"{neural_layer.name} of type {neural_layer.__class__.__name__}"
                f") followed by a BatchNormalization layer. This configuration"
                " is not consistent with fold_BN=False. This might require "
                "retraining on a few batches to align BatchNormalization "
                "parameters with the new quantized weights.")
