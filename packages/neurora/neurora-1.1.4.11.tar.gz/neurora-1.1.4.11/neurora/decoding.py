...
def decoding(data, labels, n=2, navg=5, time_win=5, time_step=5, iter=10, test_size=0.3, smooth=True):
    ...
    if smooth == True:
        ...
        for t1 in range(newnts):
            for t2 in range(newnts):
                ...
                if t1 < 2 and t2 < 2:
                    smooth_acc[:, t] = np.average(acc[:, :t1 + 3, :t2 + 3], axis=(1, 2))
                elif t1 < 2 and t2 >= 2 and t2 < (newnts - 2):
                    smooth_acc[:, t] = np.average(acc[:, :t1 + 3, t2 - 2:t2 + 3], axis=(1, 2))
                elif t1 < 2 and t2 >= (newnts - 2):
                    smooth_acc[:, t] = np.average(acc[:, :t1 + 3, t2 - 2:], axis=(1, 2))
                elif t1 >= 2 and t1 < (newnts - 2) and t2 < 2:
                    smooth_acc[:, t] = np.average(acc[:, t1 - 2:t1 + 3, :t2 + 3], axis=(1, 2))
                elif t1 >= 2 and t1 < (newnts - 2) and t2 >= 2 and t2 < (newnts - 2):
                    smooth_acc[:, t] = np.average(acc[:, t1 - 2:t1 + 3, t2 - 2:t2 + 3], axis=(1, 2))
                elif t1 >= 2 and t1 < (newnts - 2) and t2 >= (newnts - 2):
                    smooth_acc[:, t] = np.average(acc[:, t1 - 2:t1 + 3, t2 - 2:], axis=(1, 2))
                elif t1 >= (newnts - 2) and t2 >= (newnts - 2):
                    smooth_acc[:, t] = np.average(acc[:, t1 - 2:, t2 - 2:], axis=(1, 2))
                elif t1 >= (newnts - 2) and t2 >= 2 and t2 < (newnts - 2):
                    smooth_acc[:, t] = np.average(acc[:, t1 - 2:, t2 - 2:t2 + 3], axis=(1, 2))
                elif t1 >= (newnts - 2) and t2 <= 2:
                    smooth_acc[:, t] = np.average(acc[:, t1 - 2:, :t2 + 3], axis=(1, 2))
...
def unidirectional_transfer_decoding(data1, labels1, data2, labels2, n=2, navg=5, time_win=5, time_step=5, iter=10,
    ...
    for sub in range(nsubs):
        ...
        for i in range(iter):
            ...
            for t in range(newnts1):
                ...
                state = np.random.randint(0, 100)
                xt1 = x1[:, :, t]
                x_train, x_test, y_train, y_test = train_test_split(xt1, y1, test_size=test_size, random_state=state)
                svm = SVC(kernel='linear')
                svm.fit(x_train, y_train)
                for tt in range(newnts2):
                    xt2 = x2[:, :, tt]
                    x_train, x_testt, y_train, y_test = train_test_split(xt2, y2, test_size=test_size, random_state=state)
                    subacc[i, t, tt] = svm.score(x_testt, y_test)
