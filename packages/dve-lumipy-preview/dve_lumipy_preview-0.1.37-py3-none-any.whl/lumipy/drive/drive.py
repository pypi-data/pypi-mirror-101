from lumipy.drive.read import DriveRead
...
class Drive(Lockable):
    ...
    def __init__(self, atlas: Atlas, drive_secrets: Optional[str] = None):
        """__init__ method of the DriveDirectory class

        Args:
            atlas (Atlas): luminesce data providers atlas. Must contain Drive.File.
            drive_secrets (Optional[str]): path to secrets file. If not supplied auth information will be drawn from
            env vars.
        """
        self._client = atlas.get_client()
        self._atlas = atlas
        self._drive_client = DriveClient(drive_secrets)
    ...
    def _list_content(self, path: str, content_type=None, depth=0):
