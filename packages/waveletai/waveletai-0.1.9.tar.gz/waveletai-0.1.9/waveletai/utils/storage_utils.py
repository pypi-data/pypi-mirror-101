...
import six
import time
from waveletai.utils.datastream import compress_to_tar_gz_in_memory, FileChunkStream
        ...
        if entry.is_stream() or not os.path.isdir(entry.source_path):
            ...
            for root, _, files in os.walk(entry.source_path):
                ...
                for filename in files:
                    walked_entries.add(UploadEntry(os.path.join(root, filename), filename))
...
class SilentProgressIndicator(ProgressIndicator):
    ...
    def complete(self):
        _logger.warning('资源文件 [%s] 任务完成 %.2f MB (100%%)', self.filename, self.total / (1024 * 1024))


class LoggingDownloadProgressIndicator(ProgressIndicator):
    def __init__(self, total, filename, frequency=10):
        self.current = 0
        self.filename = filename
        self.total = total
        self.last_warning = time.time()
        self.frequency = frequency
        _logger.warning('资源文件 [%s] 大小为 %.2fMB 任务耗时较长，请耐心等待。', self.filename, self.total / (1024 * 1024))

    def progress(self, steps):
        self.current += steps
        if time.time() - self.last_warning > self.frequency:
            _logger.warning('资源文件 [%s] 任务完成进度 %.2f MB / %.2f MB (%.2f%%)', self.filename, self.current / (1024 * 1024),
