...
class ElasticDownloader:
    ...
    def download(self, uri, local_path=None):
        if not re.search(r'\w+:(\/?\/?)[^\s]+', uri):
            raise Exception('[error] Invalid URI "{}"'.format(uri))

        self.__parsed = urlparse(uri)
        scheme = self.__parsed.scheme
        payload = self.prepare_uri(uri)
        if scheme == 'http' or scheme == 'https':
            from downloader.helper.http_download import download

            return download(uri, local_path)
        elif scheme == 'ftp':
            from downloader.elastic.ftp import FTP

            if 'port' not in payload:
                payload['port'] = 21

            ftp = FTP(payload['host'], payload['user'], payload['password'], payload['port'])
            res = ftp.download(payload['path'], local_path)
            if res != False:
            ...
            from downloader.elastic.sftp import SFTP

            if 'port' not in payload:
                payload['port'] = 22

            sftp = SFTP(payload['host'], payload['user'], payload['password'], payload['port'])
            res = sftp.download(payload['path'], local_path)
            if res is not False:
    ...
    def retrieve(self, uri):
        ...
        payload = self.prepare_uri(uri)
        if scheme == 'http' or scheme == 'https':
            from downloader.helper.http_download import retrieve
    ...
    def prepare_uri(self, uri):
        ...
        payload['path'] = self.__parsed.path
