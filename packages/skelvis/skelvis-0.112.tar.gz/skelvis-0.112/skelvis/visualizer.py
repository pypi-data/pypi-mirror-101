...
class ColorChanger:
    ...
    def update_left_objects(self, current_color):
        for obj in self.skeleton.get_left_objects():
            obj.color = int(current_color.new.replace('#', '0x'), 16)
    ...
    def __display_color_changer(self):
        ...
        for skeleton in self.skeletons:
            left_color_picker: ColorPicker = ColorPicker(
                description='Left side color:', value='{0:#0{1}x}'.format(skeleton.get_left_objects()[0].color, 8).replace('0x', '#'))
            right_color_picker: ColorPicker = ColorPicker(
                description='Right side color:', value='{0:#0{1}x}'.format(skeleton.get_right_objects()[0].color, 8).replace('0x', '#'))
            center_color_picker: ColorPicker = ColorPicker(
                description='Center color:', value='{0:#0{1}x}'.format(skeleton.get_center_objects()[0].color, 8).replace('0x', '#'))
            all_color_picker: ColorPicker = ColorPicker(description='All color:', value='#ffffff')
            color_changer: ColorChanger = ColorChanger(skeleton, left_color_picker,
                                                       right_color_picker, center_color_picker)
        ...
        for i in range(len(self.skeletons)):
            color_changer_widget.set_title(i, 'Skeleton {:d} colors'.format(i))
