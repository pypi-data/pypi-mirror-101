    ...
    def __display_color_changer(self):
        ...
        for skeleton in self.skeletons:
            left_color_picker: ColorPicker = ColorPicker(
                description='Left side color:', value=hex(skeleton.get_left_objects()[0].color).replace('0x', '#'))
            right_color_picker: ColorPicker = ColorPicker(
                description='Right side color:', value=hex(skeleton.get_right_objects()[0].color).replace('0x', '#'))
            center_color_picker: ColorPicker = ColorPicker(
                description='Center color:', value=hex(skeleton.get_center_objects()[0].color).replace('0x', '#'))
