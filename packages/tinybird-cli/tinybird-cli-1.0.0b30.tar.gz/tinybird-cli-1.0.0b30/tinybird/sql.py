...
def schema_to_sql_columns(schema):
    ...
    for x in schema:
        ...
        parts = [col_name(x['name'], backquotes=True), _type, x['default_value'], x['codec']]
...
def parse_table_structure(table_structure):  # noqa: C901
    ...
    Traceback (most recent call last):
    ...
    ValueError: DEFAULT|MATERIALIZED|ALIAS|CODEC expected
    c Float32 b String
    ...
    for i, c in enumerate(table_structure):
        ...
        if mode == LSTRIP and c not in ' \n':
            ...
            if (c in ',' or eof(i)) and parenthesis == 0:
                ...
                if default_value == '':
                    if c not in 'cCmMdDaA \n':
                        raise ValueError(format_parse_error(
                            table_structure, i, "DEFAULT|MATERIALIZED|ALIAS|CODEC expected"))
                    if codec != '':
                        raise ValueError(format_parse_error(table_structure, i, "Unexpected DEFAULT|MATERIALIZED|ALIAS after CODEC"))
                if c in ' \n' and default_value == '':
                    continue
                default_value += c
            ...
            if (c in ',' or eof(i)) and parenthesis == 0:
                ...
                raise ValueError(format_parse_error(table_structure, i, "wrong value"))
            else:
                codec += c
    ...
    for column in columns:
        ...
        column['default_value'] = column['default_value'] if column['default_value'] else None
