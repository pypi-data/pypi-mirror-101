...
"""
    ...
    OpenAPI spec version: 51.0.6-SNAPSHOT
...
class ItemsApi(object):
    ...
    def search_workbench(self, **kwargs):
        ...
        :param str filter: If set to 'mine', only workbench items owned by the current user (or the userId specified) are returned. If set to 'shared' only workbench items where the current user (or the userId specified) has been given access are returned. If set to 'corporate', only workbench items in the corporate folder are returned. If set to 'users', only user folders are returned (can only be used by admins). If set to 'all' (or not specified), all workbench items the current user has access to will be returned.
        :param bool is_archived: Whether to filter workbench items to only folders that have been archived.
        :param bool is_pinned: When true, returns only pinned workbench items. Special values of 'mine', 'corporate', and 'users' (admins only) search the authenticated user's home folder, the corporate folder, and the folder of all home folders, respectively.
    ...
    def search_workbench_with_http_info(self, **kwargs):
        ...
        all_params = ['ids', 'types', 'folder_id', 'filter', 'is_archived', 'is_pinned', 'text_search', 'sort_order', 'limit', 'offset', 'is_exact', 'root']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
