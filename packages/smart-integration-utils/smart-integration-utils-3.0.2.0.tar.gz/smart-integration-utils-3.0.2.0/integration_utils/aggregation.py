...
class BaseAggregationManager(object):
    ...
    def _get_calc_step(self) -> str:
        ...
        calc_step = integration_data['calculation']
        cost = calc_step['cost'] if calc_step else None
        if cost:
            return cost.replace('{{', '').replace('}}', '').replace('cost', 'cost__sum')
        return ''

    def _calculate(self, calc_step: str, cost: dict, field: str):
        if calc_step:
            calc_value = eval(
                calc_step.replace(f'{field}__sum', str(cost[f'{field}__sum']))
            )
            return {f'{field}__sum': round(calc_value, 2)}
        return {f'{field}__sum': round(cost[f'{field}__sum'], 2)}
