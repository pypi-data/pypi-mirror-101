...
from azureml.contrib.automl.dnn.vision.common.constants import ArtifactLiterals, MetricsLiterals, \
...
from .constants import PredictionLiterals, ValidationMetricType
...
def compute_metrics(eval_bounding_boxes, val_metric_type,
    ...
    if val_metric_type in ValidationMetricType.ALL_VOC:
        ...
        computed_metrics[VocMap.PRECISION] = round(precision, 3)
        computed_metrics[VocMap.RECALL] = round(recall, 3)

        # update per_label_metrics
        if per_label_metrics is not None:
            per_label_metrics_current_value = {}
            ...
            for label_index, label_metric in vocmap_result[VocMap.PER_LABEL_METRICS].items():
                ...
                for metric_name in [VocMap.PRECISION, VocMap.RECALL, VocMap.AVERAGE_PRECISION]:
                    ...
                    if metric_name not in per_label_metrics_current_value[label_index]:
                        per_label_metrics_current_value[label_index][metric_name] = []

                    metric_score = round(label_metric[metric_name].item(), 3)
