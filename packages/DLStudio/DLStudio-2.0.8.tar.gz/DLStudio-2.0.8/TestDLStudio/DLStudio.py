...
__date__      = '2021-April-12'   
__url__       = 'https://engineering.purdue.edu/kak/distDLS/DLStudio-2.0.8.html'
...
#                self.word_vectors = gen_api.load("word2vec-google-news-300")
                ...
                if path_to_saved_embeddings is not None:
                    ...
                    if os.path.exists(path_to_saved_embeddings + 'vectors.kv'):
                        ...
                        ##  save to a disk-based hash table for faster reloading subsequently. 'kv' stands for
                        ##  "KeyedVectors", a special datatype used by gensim because it has a smaller footprint than dict
...
#                cell = torch.tanh(self.linear_for_cell(hidden_clone))
        ...
        class GRUnetWithEmbeddings(nn.Module):
            ...
            routed through LogSoftmax activation. 
            """
            def __init__(self, hidden_size, output_size, n_layers, input_size=300, drop_prob=0.2):
        ...
        def run_code_for_training_with_TEXTnet_word2vec(self, net, hidden_size):        
            filename_for_out = "performance_numbers_" + str(self.dl_studio.epochs) + ".txt"
            FILE = open(filename_for_out, 'w')
            net = copy.deepcopy(net)
            net = net.to(self.dl_studio.device)
        ...
        def run_code_for_training_with_TEXTnetOrder2_word2vec(self, net, hidden_size):        
            filename_for_out = "performance_numbers_" + str(self.dl_studio.epochs) + ".txt"
            FILE = open(filename_for_out, 'w')
            net = copy.deepcopy(net)
            net = net.to(self.dl_studio.device)
        ...
        def run_code_for_training_for_text_classification_with_GRU_word2vec(self, net, hidden_size): 
            filename_for_out = "performance_numbers_" + str(self.dl_studio.epochs) + ".txt"
            FILE = open(filename_for_out, 'w')
            net = copy.deepcopy(net)
            net = net.to(self.dl_studio.device)
        ...
        def run_code_for_testing_with_TEXTnet_word2vec(self, net, hidden_size):
            net.load_state_dict(torch.load(self.dl_studio.path_saved_model))
            classification_accuracy = 0.0
            negative_total = 0
            positive_total = 0
        ...
        def run_code_for_testing_with_TEXTnetOrder2_word2vec(self, net, hidden_size):
            net.load_state_dict(torch.load(self.dl_studio.path_saved_model))
            classification_accuracy = 0.0
            negative_total = 0
            positive_total = 0
        ...
        def run_code_for_testing_text_classification_with_GRU_word2vec(self, net, hidden_size):
            net.load_state_dict(torch.load(self.dl_studio.path_saved_model))
            classification_accuracy = 0.0
            negative_total = 0
            positive_total = 0
