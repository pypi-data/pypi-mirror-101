...
class JPEGBase(Distortioner):
    def __init__(self):
        ...
        self.conv_kernel = torch.nn.Parameter(torch.zeros(8, 2, 15, 15), requires_grad=False)
    ...
    def _convdot(self, x, k: torch.FloatTensor, dot_x_k: bool) -> torch.FloatTensor:
        ...
        for i in range(8):
            ...
            self.conv_kernel[i, 0, pos[0], pos[1]] = basis

            stride = (1, 8) if dot_x_k else (8, 1)
            pad = (7-i, i, 7, 7) if dot_x_k else (7, 7, 7-i, i)
            y = F.conv2d(F.pad(x, pad), self._expand(self.conv_kernel[i, 0]), stride=stride, groups=3)

            self.conv_kernel[i, 1, 7, 7] = 1
            y = F.conv_transpose2d(y, self._expand(self.conv_kernel[i, 1]), stride=stride, padding=(7, 7), groups=3)
