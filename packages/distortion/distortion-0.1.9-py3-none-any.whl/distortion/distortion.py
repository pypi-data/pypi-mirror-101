...
class JPEGBase(Distortioner):
    def __init__(self):
        super().__init__()
        self.dct_kernel = torch.nn.Parameter(self._create_dct_basis_matrix())
        self.idct_kernel = torch.nn.Parameter(self._create_idct_basis_matrix())
        self.conv_kernel = torch.nn.Parameter(torch.empty(15, 15))
...
class JPEGCompression(JPEGBase):
    def __init__(self, qf=50):
        super().__init__()
        self.qf = qf
        self.kernel = torch.nn.Parameter(self._create_quantization_table())
