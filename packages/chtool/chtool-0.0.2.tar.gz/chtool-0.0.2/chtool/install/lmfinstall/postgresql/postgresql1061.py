...
def install(conp,spath,**krg):
    ...
    con.run("%s/postgresql-10.6-1-linux-x64.run"%tdir,pty=True,watchers=[pgasw1,pgasw2,pgasw3,pgasw4,
        pgasw5,pgasw6,pgasw7,pgasw8,pgasw9,pgasw10,pgasw11,pgasw12])#,pgasw12
...
def master_slave(conp1,conp2,master_data,slave_data):
    c=Connection(conp2[0],connect_kwargs={"password":conp2[1]})
    c1=Connection(conp1[0],connect_kwargs={"password":conp1[1]})
    c1.run("sed -i '/host replication all all md5/d' %s/pg_hba.conf "%master_data,pty=True)
    c1.run("echo 'host replication all all md5' >>%s/pg_hba.conf "%master_data,pty=True)
...
#install(["root@172.16.32.6:22",'Since2015!'],"D:\\webroot\\common\\postgresql-10.6-1-linux-x64.run",pgdata="/data/postgresql")
# conp1=["root@172.16.0.11:22",'Since2015!']
# conp2=["root@172.16.32.6:22",'Since2015!']
# master_data="/data/postgresql"
# slave_data="/data/postgresql"
# master_slave(conp1,conp2,master_data,slave_data)