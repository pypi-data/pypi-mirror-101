...
def task1():
    ...
    m=gp(file="local:greenplum-db-6.5.0-rhel7-x86_64.rpm",pin=pin)
    m.segs_pernode=2
    m.mirror=False
    m.pyver='3.7'
    python_file="local:Python-3.7.4.tgz"
    m.total_new(java_file="local:jdk-8u151-linux-x64.rpm",python_file="local:Python-3.7.4.tgz")
    #m.extension('pg_trgm')
    #python_file1=m.download(python_file)
    #m.python(python_file1)
    ed=time.time()
    cost=int(ed-bg)
...
def task3():
    ...
    f=lambda conp:install(conp,python_file)
    mythread(pin,f).run(num=min(5,len(pin)))
