...
class jd_airflow():
    ...
    def __init__(self,local_file_download=True):
        ...
        self.ip_pool_pin = ["root@10.0.64.19:22", "BST@2020610", "airflow_worker7"]  ## 代理ip服务器主机配置 ， 默认 为 airflow worker7

        self.ip_pool_conp = ['postgres', 'since2015', '10.0.64.22', 'postgres', 'public']  ## 代理 ip 存放数据库配置 , 默认 为 爬虫中间库 db2
   ...
   def init_cfg_host(self):
    ...
    def init_airflow3(self, conp):
        ...
        c.run('/opt/python35/bin/python3 -m pip install zlproxy zlsrc lmfscrap lmf zltask zlgp zlapp zlgp3 zlzz zlgg3 py3Fdfs  -i https://jacky:Jacky666.@www.zhulong.com.cn/pypi/simple')
        if conp == self.airflow_master:
            c.put(dags,tdir)
