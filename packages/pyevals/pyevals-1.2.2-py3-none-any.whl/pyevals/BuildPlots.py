...
def MakePlots(df,cat_features,con_features):
	...
	def CalcuateRows(TotalPlots):
			if TotalPlots==3:
			    ...
			    remainder = TotalPlots%3
			    return(TotalPlots//3 + remainder)
	...
	while(row<TotalBarPlots):
	    for i in con_features:
	    	for j in cat_features:
	        	if row<2 and column<3:
	        		axes[row, column].set_title('BarPlot '+ i + ' '+j)
	...
	while(row<TotalBoxPlots):
	    for i in con_features:
	    	for j in cat_features:
	        	if row<2 and column<3:
	        		axes[row, column].set_title('BoxPlot '+ i + ' '+j)
	...
	while(row<TotalViolinPlots):
	    for i in con_features:
	    	for j in cat_features:
	        	if row<2 and column<3:
	        		axes[row, column].set_title('ViolinPlot '+ i + ' '+j)
