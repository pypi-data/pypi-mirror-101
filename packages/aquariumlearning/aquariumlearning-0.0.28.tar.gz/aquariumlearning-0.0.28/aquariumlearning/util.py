...
TYPE_PRIMITIVE_TO_STRING_MAP: Dict[
    Type[USER_METADATA_PRIMITIVE], USER_METADATA_TYPES
] = {
    str: "str",
    int: "int",
    float: "float",
...
class InferenceFrameSummary(TypedDict):
    frame_id: str
    label_counts: Dict[LabelType, int]
    custom_metrics_names: List[str]


class GtLabelFrameSummary(TypedDict):
    frame_id: str
    label_counts: Dict[LabelType, int]


class FrameWithEmbeddings(Protocol):
