            ...
            for src in paths_in[:-1]:
                if isinstance(src, Path):
                    if src.is_dir():
                        ...
                        val.append((src, image.path(src.name.encode(self.encoding))))
                elif isinstance(src, Drive):
                    if expand_drive:
                        src_image = self.images[src]
                        val += self.src_dest_pairs(list(src_image.expand(b''))+[dest])
    ...
    def do_attach(self, args):
        ...
        for path in self.expand_paths(args.image):
            ...
            if DiskImage.is_valid_image(path):
                self.images[next_drive] = DiskImage(path, mode, next_drive)
            else:
                self.images[next_drive] = TapImage(path, mode, next_drive)
    ...
    def do_directory(self, args):
        ...
        for d in args.drive:
            ...
            if kwargs['drive'] in self.images:
                ...
                for l in image.directory(**kwargs):
                    self.poutput(l)
            else:
