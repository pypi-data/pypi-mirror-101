...
class QueuingUDPRequestHandler(socketserver.BaseRequestHandler):
    def handle(self):
...
class QueuingUDPListener(socketserver.ThreadingUDPServer):
    ...
    def __init__(self, interface, queue):
        ...
        super().__init__(interface, QueuingUDPRequestHandler)


class QueuingTCPRequestHandler(socketserver.StreamRequestHandler):
    def handle(self):
...
class QueuingTCPListener(socketserver.ThreadingTCPServer):
    def __init__(self, interface, queue):
        self.queue = queue
        if ":" in interface[0]:
            self.address_family = socket.AF_INET6
        super().__init__(interface, QueuingTCPRequestHandler)
...
class ThreadedListener(threading.Thread):
    ...
    >>> listener = ThreadedListener('0.0.0.0', 2055)
    ...
    def __init__(self, host: str, port: int, use_tcp: bool = False):
        logger.info("Starting NetFlow/IPFIX {} listener on {}:{}".format("TCP" if use_tcp else "UDP", host, port))
        self.output = queue.Queue()
        self.input = queue.Queue()
        if use_tcp:
            self.server = QueuingTCPListener((host, port), self.input)
        else:
...
def get_export_packets(host: str, port: int, use_tcp: bool = False) -> ParsedPacket:
    """A threaded generator that will yield ExportPacket objects until it receives interrupt or terminate signal.
    """
    ...
    listener = ThreadedListener(host, port, use_tcp)
    listener.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tcp", action="store_true", dest="use_tcp", help="Use TCP instead of UDP (IPFIX)")
    ...
    try:
        ...
        for ts, client, export in get_export_packets(args.host, args.port, args.use_tcp):
            entry = {ts: {
                "client": client,
