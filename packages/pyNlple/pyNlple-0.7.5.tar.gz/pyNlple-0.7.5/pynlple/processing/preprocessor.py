...
class MatchPostprocessor(object):

    def __call__(self, match: Match) -> str:
        raise NotImplementedError()


class PreserveHighlight(MatchPostprocessor):
    ...
    def __init__(
        ...
        self.__stripped_replace_with = self.replace_with.strip()
        self.__start_index = self.replace_with.index(self.__stripped_replace_with)
        self.__end_index = self.__start_index + len(self.__stripped_replace_with)
        self.left_bound = ''.join([
            self.replace_with[:self.__start_index],
            '<b>',
            self.replace_with[self.__start_index:]
        ])
        self.right_bound = ''.join([
            self.replace_with[:self.__end_index],
            '</b>',
            self.replace_with[self.__end_index:]
        ])
        self.left_right_bound = ''.join([
            self.replace_with[:self.__start_index],
            '<b>',
            self.replace_with[self.__start_index:self.__end_index],
            '</b>',
            self.replace_with[self.__end_index:]
        ])
    ...
    def __call__(self, match: Match) -> str:
        ...
        if '<b>' in match_text or '</b>' in match_text:
            ...
            if cnt > 0:
                ...
                return self.left_right_bound
        else:
            return self.replace_with


class PreserveHighlightedEntity(MatchPostprocessor):
    ...
    def __init__(
        ...
        self.replace_single_with = replace_with
        self.replace_start_with = start_with
        self.replace_end_with = end_with
        ...
        if "<b>" in match_text or "</b>" in match_text:
            ...
            for match in BOLD_TAG_PATTERN.finditer(match_text):
                if match.group(0) == "<b>":
                    ...
                    element = element + match_text[s:match.start()] + "</b>"
                    elements.append(element)
                    s = match.end()
                    element = ""
            if element != "":
                element = element + match_text[s:] + "</b>"
                elements.append(element)
            return self.replace_start_with + " ".join(elements) + self.replace_end_with
        else:
            return self.replace_single_with


class PostprocessedMatchRegexReplacerAdapter(RegexReplacerAdapter):
    def __init__(
            self,
            regex_query,
            match_postprocessor: MatchPostprocessor,
            case_sensitive = False,
            use_locale = False,
            restrict_to_ascii = False,
    ):
        super().__init__(regex_query, match_postprocessor, case_sensitive, use_locale, restrict_to_ascii)
...
class BoldPreservingRegexReplacerAdapter(RegexReplacerAdapter):
    ...
    def __init__(self, regex_query, replace_tag_with=None, case_sensitive=False, use_locale=False,
        ...
        if not preserve_bold:
            ...
            target = PreserveHighlight(replace_tag_with)
...
class URLReplacerPreservingHighlightedEntity(PostprocessedMatchRegexReplacerAdapter):
    """Research https://mathiasbynens.be/demo/url-regex and https://gist.github.com/dperini/729294
    Since we need to increase recall of link-a-like entities in text, we adopt the algorithm with
    some possible erroneous cases.
    I used regexp from https://mathiasbynens.be/demo/url-regex by @stephenhay
    ...
    def __init__(
        self,
        replace_with: str = "<url>",
        start_with: str = "<url>",
        end_with: str = "</url>",
    ) -> None:
        match_postprocessor = PreserveHighlightedEntity(
            replace_with=replace_with,
            start_with=start_with,
            end_with=end_with
        )
        super().__init__(URL_REGEX, match_postprocessor, False, False, False)


class EmailReplacerPreservingHighlightedEntity(PostprocessedMatchRegexReplacerAdapter):
    """Replaces all e-mail-like entities, non-latin including. Includes length check for each entity part."""

    def __init__(
        self,
        replace_with: str = "<email>",
        start_with: str = "<email>",
        end_with: str = "</email>",
    ) -> None:
        match_postprocessor = PreserveHighlightedEntity(
            replace_with=replace_with,
        ...
        super().__init__(regex_query, match_postprocessor, False, False, False)
...
class MultiNonLetterReplacer(RegexReplacerAdapter):

    def __init__(self, include_digits: bool = False, max_allowed: int = 2, reduce_to: int = 2):
        ...
        if self.include_digits:
            ...
            regex_query = r'([\W_])\1{' + str(self.max) + ',}'

        super().__init__(regex_query, '\\1' * self.reduce_to, False, False, False)
...
class MultiAllReplacer(RegexReplacerAdapter):

    def __init__(self, include_digits: bool = False, max_allowed: int = 3, reduce_to: int = 3):
        self.max = max_allowed
        self.reduce_to = reduce_to
        self.include_digits = include_digits
        if self.include_digits:
            regex_query = r'(.)\1{' + str(self.max) + ',}'
        else:
            regex_query = r'(\D)\1{' + str(self.max) + ',}'
