...
:LastEditTime: 2021-04-08 17:41:45
...
from handlers.base.wechat_base import *
...
class WeiXinPayRequest(object):
    ...
    def query_order(self, out_trade_no="", transaction_id=""):
        ...
        try:
            ...
            msg = respone.text.encode('ISO-8859-1').decode('utf-8')
            xmlresp = xmltodict.parse(msg)
            # self.logger_error.error(f"【微信查询订单】"+str(xmlresp))
...
class WeiXinPayReponse(object):
    ...
    def check_sign(self):
        ...
        # self.logger_error.error("【微信本地签名】" + sign)
...
class WeiXinRefundReponse(object):
    ...
    def decode_req_info(self, req_info):
        """解密退款通知加密参数req_info"""
        base64_str = CryptoHelper.base64_decode(req_info)
        detail_info = self.aes_decrypt(base64_str, CryptoHelper.md5_encrypt(self.APIKEY))
    ...
    def aes_decrypt(self, source, password, encoding="utf-8"):
        ...
        result = aes.decrypt(source).decode(encoding)
        return unpad(result)
