...
@LastEditTime: 2021-01-26 18:29:59
...
from models.db_models.behavior.behavior_log_model import *
from models.db_models.behavior.behavior_orm_model import *
from models.db_models.behavior.behavior_report_model import *
    ...
    def process_behavior(self, act_id, user_id, behavior_key, behavior_value):
        ...
        if orm.is_repeat == 1:
            if orm.repeat_type == 2:
                ...
                behavior_log = behavior_log_model.get_entity("orm_id=%s and act_id=%s and user_id=%s and create_day=%s", params=[orm.id, act_id, user_id, SevenHelper.get_now_day_int()])
            if not behavior_log:
                behavior_report = behavior_report_model.get_entity("act_id=%s and key_name=%s and create_day=%s", params=[act_id, behavior_key, SevenHelper.get_now_day_int()])
                if not behavior_report:
                    ...
                    behavior_report.create_year = (datetime.datetime.now() + datetime.timedelta(hours=0)).year
                    behavior_report.create_month = SevenHelper.get_now_month_int(0)
                    behavior_report.create_day = SevenHelper.get_now_day_int(0)
                    behavior_report.create_date = TimeHelper.add_hours_by_format_time(hour=0)
        ...
        new_behavior_log.create_date = TimeHelper.add_hours_by_format_time(hour=0)
        new_behavior_log.create_day = SevenHelper.get_now_day_int(0)
