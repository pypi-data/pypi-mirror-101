...
:LastEditTime: 2021-04-08 17:42:04
...
class ClientBaseHandler(BaseApiHandler):
    ...
    def prepare_ext(self):
        try:
            ...
            self.logger_error.error(str(ex) + "【获取头部参数出现异常】")
    ...
    def get_device_info(self, device_string):
        ...
        device_dict["height"] = 0 if "Height" not in info_model.keys() else int(info_model["Height"][0])
        device_dict["width"] = 0 if "Width" not in info_model.keys() else int(info_model["Height"][0])
    ...
    def get_request_param(self, param_name, default="", filter_sql=True, strip=True):
        ...
        if param_name != "avatar" and isinstance(param_ret, str):
            param_ret = self.sql_filter(self.html_clean(param_ret)) if filter_sql else param_ret
        return param_ret
    ...
    def client_reponse_json_error(self, error_code="", error_message="", data=None):
        """
        :description: 头部校验
        :param device_info:头部信息
    ...
    def emoji_base64_to_emoji(self, text_string):
        ...
        if results:
            for item in results:
                text_string = text_string.replace("[em_{0}]".format(item), self.base64_encode(item))
        return text_string
    ...
    def emoji_to_emoji_base64(self, text_string):
        ...
        for location, item_emoji in enumerate(text_string):
            if item_emoji in unicode_codes.UNICODE_EMOJI:
                ...
                emoji_base64 = self.base64_decode(bytes_item_emoji)
...
def client_filter_check_head(is_check=True):
    """
                ...
                try:
                    ...
                    if clientheaderinfo_string:
                        ...
                        if now_time - device_dict["signature_stamp"] > int(1000 * 60 * 10):
                            return self.client_reponse_json_error("SignatureStamp", "登录超时,请重新进入小程序")  #10分钟过期
                        # 验证是否有头部信息签名
                        if not device_dict.__contains__("signature_md5"):
                            return self.client_reponse_json_error("NoSignatureMD5", "缺少参数SignatureMD5")
                        # 验证头部签名是否成功
                        client_head_dict = dict([(k, v[0]) for k, v in parse_qs(clientheaderinfo_string, True).items() if k != "SignatureMD5"])
                    ...
                    self.logger_error.error(str(ex) + "【头部校验】")
