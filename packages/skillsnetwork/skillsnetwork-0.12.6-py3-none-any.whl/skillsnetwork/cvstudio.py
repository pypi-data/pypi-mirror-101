...
def isValidFolder():
    return bool(re.match(r"^/resources/labs/cvstudio/[0-9a-z-]+/(runs|apps)/[0-9a-z-]+$", os.getcwd()))
...
class CVStudio(object):
    ...
    def interactive(token=None, ibm_api_key_id=None, base_url=None):
        if base_url == None:
            base_url = os.environ.get('CV_STUDIO_BASE_URL', 'https://vision.skills.network')

        if token is None:
            token = os.environ.get('CV_STUDIO_TOKEN')
        if token is None:
            # raise Exception('need to pass valid token or set CV_STUDIO_TOKEN environment variable')
            print('Need to validate your identity as no CV Studio token set in environment')
            print('Please visit the following url in your browser and copy the token: ' + base_url + '/api/token')
    ...
    def getProjects(token, base_url):
        ...
        x = requests.post(base_url + '/api/interactive', headers=tokenToHeaders(token), data=json.dumps(data, default=default))
        if not x.ok:
    ...
    def __init__(self, interactive=False, token=None, ibm_api_key_id=None, base_url=None, public=False):
        self.public = public
    ...
    def getToken(self):
        ...
        if not isValidFolder():
            raise Exception('Need to run from a valid folder. e.g. /resources/labs/cvstudio/<project-name>/runs/<run-name>')
        ...
        x = requests.post(self.base_url + '/api/token', headers=headers, data=json.dumps(data, default=default))
    ...
    def download_file_cos(self, key, folder=os.getcwd()):
        try:
            local_file_name=os.path.join(folder, key)
    ...
    def getDataset(self, transform=None,train_test=None,percentage_train=0.8,random_state=0,degrees=5):
        dataset=getDataset(annotations=self.get_annotations(), bucket_name=self.bucket, transform=transform,train_test=train_test,percentage_train=percentage_train,random_state=random_state,degrees=degrees)
        return dataset
...
def getDataset(annotations, bucket_name, transform,train_test=None,percentage_train=0.8,random_state=0,degrees=5):
    ...
    class Dataset(Dataset):
        ...
        def __init__(self, annotations, bucket_name, transform,train_test=None,percentage_train=percentage_train,random_state=random_state,degrees=degrees):
            ...
            if os.path.exists(labels_filename):
                ...
                for key,label_dict in annotations['annotations'].items():                
                    ...
                    data_['file_name'].append(os.path.join(os.getcwd(),'images',key))

                self.data=pd.DataFrame(data_)
                #train set
            if self.train_test=='train':
                print("this is the training set")
                self.data=self.data.sample(frac=percentage_train,random_state=random_state).reset_index()
                labels_filename=os.path.join(os.getcwd(),bucket_name+"_train_lables.csv")
                #self.data.to_csv(labels_filename)
            #test set    
            if self.train_test=='test':
                print("this is the test set")
                labels_filename=os.path.join(os.getcwd(),bucket_name+"_test_lables.csv")    
                temp=self.data.sample(frac=percentage_train,random_state=random_state) 
    ...
    dataset=Dataset(annotations=annotations, bucket_name=bucket_name, transform=transform,train_test=train_test,percentage_train=percentage_train,random_state=random_state,degrees=degrees)
    return dataset