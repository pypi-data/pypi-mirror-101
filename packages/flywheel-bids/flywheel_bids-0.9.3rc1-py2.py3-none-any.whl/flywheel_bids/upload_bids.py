...
from .supporting_files.templates import DEFAULT_TEMPLATE as template
...
def handle_project_label(
    ...
    if subject_label and subject_label in bids_hierarchy[project_label_cli]:
        if (
            ...
            logger.error(
                "Could not find Session {} in BIDS hierarchy!".format(session_label)
            )
            sys.exit(2)
        else:
            bids_hierarchy = {
        ...
        logger.error(
            "Could not find Subject {} in BIDS hierarchy!".format(subject_label)
        )
        sys.exit(2)
...
def handle_project(fw, group_id, project_label, assume_yes):
    ...
    for ep in existing_projs:
        if (ep["label"] == project_label) and (ep["group"] == group_id):
            logger.info(
                "Project (%s) was found. Adding data to existing project."
                % project_label
            )
            # project exists
            project = ep
...
def handle_subject(fw, project_id, subject_code):
    ...
    for es in existing_subjects:
        if es["code"] == subject_code:
            logger.info(
                "Subject (%s) was found. Adding data to existing subject."
                % subject_code
            )
            # Session exists
            subject = es
        ...
        for foldername in folders:
            ...
            for fname in subject[session_label][foldername].get("files"):
                ...
                context["acquisition"] = handle_acquisition(
                    fw, context["session"]["id"], acq_label
                )
                ### Upload file
                # define full filename
                #   NOTE: If session_label equals 'ses-', session label is not
...
def upload_bids_dir(
    fw, bids_hierarchy, group_id, rootdir, hierarchy_type, local_properties, assume_yes
):
    """
...
def main():
    ...
    parser.add_argument(
        "--session",
        default=None,
        help="Only upload data from single session fodler (i.e. ses-01)",
    )
    parser.add_argument(
        "--type",
        dest="hierarchy_type",
