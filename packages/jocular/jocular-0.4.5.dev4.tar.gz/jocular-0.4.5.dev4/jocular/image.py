...
class Image:
    ...
    def __init__(self, path=None, verbose=False, check_image_data=False):
        ...
        # is it a master? check FITs subtype or name or nsubs
        master1 = fits_props.get('sub_type', '').lower().startswith('master')
        master2 = name.startswith('master')
        master3 = fits_props.get('nsubs', 0) > 1
        self.is_master = master1 or master2 or master3
    ...
    def describe(self):
        ...
        Logger.info('Image:    {:9s} {:14s} ({:} days) {:5s} {:}s {:} {:} {:} {:}'.format(
            ...
            self.age,
            self.sub_type,
            self.exposure if self.exposure is not None else '?',
        ...
        Logger.info('Image:    min {:.4f} max {:.4f} mean {:.4f}'.format(
    ...
    #     ''' Changed in v0.4.5: use same rescaling as for subs
    #     '''
    #     if self.image is None:
    #         try:
    #             with fits.open(self.path) as hdu:
...
if __name__ == '__main__':
    ...
    fitsfiles = fits_in_dir('.')
    print('found {:} FITs'.format(len(fitsfiles)))
    for f in fitsfiles:
        print('-------------')
        try:
            s = Image(os.path.join('.', f), verbose=True)
            s.describe()
        except Exception as e:
