...
def test_train_fails_if_github_repo_not_accessible(monkeypatch):
    ...
    assert launch_url == os.getenv('GRID_URL', env.DEFAULT_GRID_URL).replace(
        "/graphql", "") + '/#/settings'
...
def test_train_with_nickname(monkeypatch):
    """grid train returns 0 exit code"""
    monkeypatch.setattr(grid, 'gql', lambda x: x)
    monkeypatch.setattr(grid.Grid, '_init_client', monkey_patch_client)
    ...
    description = 199 * '-'  # 200 characters is the limit.
    result = RUNNER.invoke(cli.train, [
        '--grid_credential', CREDENTIAL_ID, 
        '--grid_description', description,
        '--grid_instance_type', '2_cpu_4gb',
        '--grid_strategy', 'grid_search', '--grid_trials', "1",
        '--ignore_warnings', 'tests/data/script.py'
    ])
    assert result.exit_code == 0
