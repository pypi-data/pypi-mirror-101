...
class TestDelete:
    ...
    def test_delete_deletes_runs_and_experiments(self):
        ...
        result = RUNNER.invoke(cli.delete, ["experiment", 'test-run-exp0'],
                               input='y\n')
        assert result.exit_code == 0
        assert not result.exception
        assert 'Are you sure you want to do this?' in result.output
    ...
    def test_delete_experiment_fails_when_experiment_not_found(
            self, monkeypatch):
        def mp_get_experiment_id(*args, **kwargs):
            return {"success": False}

        monkeypatch.setattr(resolvers, 'get_experiment_id',
                            mp_get_experiment_id)
        result = RUNNER.invoke(cli.delete, ["experiment", 'test-run-exp0'],
                               input='y\n')
        assert result.exit_code == 1
        assert result.exception
    ...
    def test_delete_experiment_fails_when_experiment_not_found_exception(
            self, monkeypatch):
        def mp_get_experiment_id(*args, **kwargs):
            raise Exception()
