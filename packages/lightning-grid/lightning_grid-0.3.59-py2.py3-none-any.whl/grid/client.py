...
"""
    ...
    def _init_client(self, websocket: bool = False) -> None:
        ...
        try:
            ...
            if not isinstance(
                    _graphql_client,
                    Client) or _graphql_client.transport != self.transport:
                _graphql_client = Client(transport=self.transport,
        ...
        try:
            if experiment_name:
                ...
                spinner = yaspin.yaspin(
                    text=f'Cancelling experiment {experiment_name}...',
                    color="yellow")
                spinner.start()
                ...
                click.echo(
                    f'Experiment {styled_name} was cancelled successfully.')
            else:
                spinner = yaspin.yaspin(text=f'Cancelling run {run_name}...',
                                        color="yellow")
                spinner.start()
    ...
    def get_experiment_id(self, experiment_name: str) -> str:
        ...
        """
        try:
            spinner = yaspin.yaspin(text="Getting experiment info...",
                                    color="yellow")
            spinner.start()
            ...
            if not result['getExperimentId']['success']:
                raise click.ClickException(
                    f"{result['getExperimentId']['message']}")
    ...
    def delete(self,
               ...
               run_name: Optional[str] = None) -> bool:
        """
        ...
        try:
            if experiment_name:
                ...
                spinner = yaspin.yaspin(
                    text=f'Deleting experiment {experiment_name}...',
                    color="yellow")
                spinner.start()
                ...
                styled_name = click.style(experiment_name, fg='blue')
                click.echo(
                    f'Experiment {styled_name} was deleted successfully.')
            else:
                spinner = yaspin.yaspin(text=f'Deleting run {run_name}...',
                                        color="yellow")
                spinner.start()
    ...
    def list_datastores(self):
        ...
        table = BaseObservable.create_table(columns=table_cols)
        sorted_datastores = sorted(result["getDatastores"],
                                   key=lambda k: (k['name'], k['version']))
