...
class TestInteractive:
    ...
    def test_interactive_create_succeeds(self, monkeypatch):
        ...
        monkeypatch.setattr(grid.Grid, 'status', self.mp_status)
        monkeypatch.setattr(resolvers, 'create_interactive_node', mp_create_interactive_node)
    ...
    def test_interactive_create_fails_multiple_creds(self, monkeypatch):
        ...
        monkeypatch.setattr(grid.Grid, 'get_credentials', mp_get_credentials_multiple_no_default)
        result = RUNNER.invoke(cli.interactive, ["create"])
    ...
    def test_interactive_create_fails_with_wrong_creds(self, monkeypatch):
        ...
        result = RUNNER.invoke(cli.interactive, ["create", '--grid_credential', self.CREDENTIAL_ID])
        assert result.exit_code == 1
        assert result.exception
    ...
    def test_interactive_create_succeeds_with_default_creds(self, monkeypatch):
        ...
        def mp_default_creds(self):
            return {'getUserCredentials': [{'credentialId': creds, 'provider': 'aws', 'defaultCredential': True}]}
    ...
    def test_interactive_create_fails_if_config_not_found():
        ...
        result = RUNNER.invoke(cli.interactive, ["create", "--grid_config", "no-file"])

        assert result.exit_code == 2
        assert result.exception
        assert 'Could not open file' in result.output

    def test_interactive_create_succeeds_with_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(grid.Grid, 'status', self.mp_status)
        result = RUNNER.invoke(cli.interactive, ["create", "--grid_config", config.as_posix()])
    ...
    def test_interactive_delete_succeeds(monkeypatch):
        ...
        monkeypatch.setattr(resolvers, 'delete_interactive_node', mp_delete_interactive_node)
    ...
    def test_interactive_create_description(self, monkeypatch):
        ...
        result = RUNNER.invoke(cli.interactive, ["create", "--grid_description", "test-description"])
        assert result.exit_code == 0
        assert not result.exception
        assert 'Interactive node created!' in result.output

    @patch("grid.client.Grid.sync_ssh_config",
           autospec=True,
           return_value=[
               {
                   "name": "test-pending",
    ...
    def test_update_ssh_config_and_check_ixsession_status_callback(self, ixsessions):
        result = RUNNER.invoke(cli.interactive, ["ssh", "not-exist-test"])
        assert result.exit_code == 2
        assert result.output
        assert "does not exist" in result.output

        result = RUNNER.invoke(cli.interactive, ["ssh", "test-pending"])
        assert result.exit_code == 2
        assert result.output
