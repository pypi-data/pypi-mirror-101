...
class SessionManager:
    ...
    def startAWSSession(self, sessionInfo: SessionInfo):
        ...
        stsToken = {}
        stsToken["AccessKeyId"]     = sessionInfo.credentials['accessKey']
        stsToken["SecretAccessKey"] = sessionInfo.credentials['secretKey']
        stsToken["SessionToken"]    = ""
        stsToken["Expiration"]      = ""

        sessionInfo.stsToken = stsToken
                ...
                if not terminalCommand or terminalCommand.lstrip().rstrip() == "":
                    ...
                    if "git-bash.exe" in terminalCommand or "sh.exe" in terminalCommand:
                        ...
                        if not Utils.isRunningOnGitBash():
                            outputWin, _ = self.createEnvironment(sessionInfo)
                            output = outputWin
...
rm {self.SCRIPTS_PREFIX_NAME_FILE}*
    ...
    def createEnvironment(self, sessionInfo: SessionInfo):
        ...
        if AwsSeeState.NEW_TERMINAL:
            ...
            with open(f"{completePathFile}",'w') as fout:
    ...
    def setCreateSessionAtLocalOS(self, sessionInfo: SessionInfo, myPidFile: str=None):
        ...
        command = "SET" if Utils.isWindows() else "export"
