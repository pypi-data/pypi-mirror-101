...
class Functions:
    ...
    FUNCTIONS = {
        LIST_PROFILES:{
            ...
            "examples-interactive": [
                ...
                {"command":f"{Style.GREEN}awsee -lp {Style.MAGENTA}# Same as -l full / --list full{Style.RESET}"},
            ]
        },
        SYNC:{
        ...
        MFA_TOKEN:{
            "id": MFA_TOKEN,
            "identifiers": [],
            "name": "Get Session using MFA Token",
            "group": MFA,
            "description": f"Retrieve an AWS Session using your MFA Token of 6 numbers. Optionally, it can be informed the profile chosen, if not informe the default will be used.",
            "arguments": [
    ...
    def showUsage(self, short=True):
        ...
        output += "   " + Emoticons.tool() + "   " + Style.CYAN + "U S A G E S" + Style.RESET + "" + "\n"
        output += "-".ljust(SIZE_SEPARATOR,"-") + "\n"
        output += f"""
        {Style.GREEN}awsee {Style.IBLUE}token-MFA                  {Style.IMAGENTA}Open AWS Session with default profile using your six-digit MFA Token 
        {Style.GREEN}awsee {Style.IBLUE}token-MFA{Style.RESET} {Style.IBLUE}[-p profile]     {Style.IMAGENTA}Open AWS Session with the passed profile using your six-digit MFA Token 
        {Style.GREEN}awsee {Style.IBLUE}-r role                    {Style.IMAGENTA}Assume a Role
        {Style.GREEN}awsee {Style.IBLUE}function {Style.IMAGENTA}                  {Style.IMAGENTA}Execute a function, see in the section below all available options
        
        {Style.GREEN}-h {Style.RESET}or {Style.GREEN}--help{Style.IMAGENTA}                     Show all functions (--help show more information){Style.RESET}
        """
        print(output)
