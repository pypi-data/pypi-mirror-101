...
class AwsSeePy:
    ...
    Please enter a MFA Token.......:{Style.IBLUE}"
           ...
           while True:
               mfaToken = input()
               if self.checkMFATokenValid(mfaToken):
                  break
    ...
    def showInfo(self):
        ...
        if not account or account == "" and profile and profile != "":
        ...
        if expiration and expiration != "":
            ...
            if (expDate < datetime.datetime.now(timezone.utc)):
                ...
                Messages.showWarning(f"Your AWS Session expired {Style.IBLUE}{diff:02.0f} {Style.IGREEN}minutes ago\n     At {Style.IMAGENTA}{expiration}")
        ...
        if expirationLabel:
            output += f"\n{Emoticons.pointRight()} {Style.GREEN}{Style.IBLUE}Your AWS Session will expires in {Style.YELLOW}{diff:02.0f} {Style.IBLUE}minutes\n{Emoticons.pointRight()} At {Style.IMAGENTA}{expiration}{Style.RESET}\n"
        output = Utils.removeCharsColors(output) if self.preferences.noColor else output
        printPy( output )
