...
class MFAManager:
    ...
    def addMfa(self):
        ...
        for c in self.credentialsRepository.all():
            ...
            for idx, r in enumerate(self.mfaRepository.searchByQuery(where('profile') == c['profile'])):
                ...
                mfaDevices += f"{Style.BLUE}{idx+1:02d}{Style.GREEN}-{r['mfa']}"
            if len(mfaDevices) > 1:
                mfaDevices = f"{Style.IBLUE}{labelMfa}{Style.GREEN}{mfaDevices}{Style.GREEN}"
            output += line + f" {mfaDevices}\n"
        output += "-".ljust(sizeSeparator,"-")
        print(output)
        result = input(f"{Style.GREEN} {Emoticons.pin()} Choose a profile....: {Style.BLUE}")
        ...
        mfaDevice = input(f"{Style.GREEN} {Emoticons.pin()} MFA Device...........: {Style.BLUE}")
        ...
        if spacesFound:
            ...
            return None
        try:
            mfaDeviceParse = arnparse(mfaDevice)
        except:
        ...
        confirm = input(f"{Style.GREEN} {Emoticons.pin()} Confirm [{Style.IGREEN}y/{Style.IGREEN}n{Style.RESET}]: {Style.BLUE}")
        if confirm.lower() == "y":
            record = {
                "id": str(uuid.uuid4()),
                "profile": profile,
    ...
    def removeMFADevice(self, numberInList):
        ...
        if not mfaToRemove:
            ...
            confirm = input(f"{Style.GREEN} {Emoticons.pin()} Remove [{Style.IGREEN}y/{Style.IGREEN}n{Style.RESET}]: {Style.BLUE}")
            if confirm.lower() == "y":
