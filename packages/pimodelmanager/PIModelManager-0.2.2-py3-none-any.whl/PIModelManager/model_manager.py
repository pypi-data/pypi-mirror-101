...
class ModelManager:
    ...
    def __new__(cls):
        ...
        if ModelManager.__instance is None:
            logger.info("__new__: instance is none")
    ...
    def upload_model(
        ...
        if path.exists():
            filename = "_".join(map(cls.__clean_str, [language, name, client, version])) + "_"
            if zip:
