                ...
                with open(self.file_path, mode='r') as f:
                    ...
                    for next_record in data:
                        ...
                        if self.check_token_exists(user.get_email(), token, next_record):
                            ...
                            token.set_refresh_token(next_record[4])
                            return token
            ...
            with open(self.file_path, mode='r') as f:
                ...
                for next_record in data:
                    if len(next_record) == 0:
                        continue
                    grant_token = None
                    if next_record[6] is not None and len(next_record[6]) > 0:
                        grant_token = next_record[6]

                    token = OAuthToken(client_id=next_record[2], client_secret=None, grant_token=grant_token, redirect_url=next_record[8], refresh_token=next_record[4])
                    token.set_id(next_record[0])
                ...
                with open(self.file_path, mode='r') as f:
                    ...
                    for next_record in data:
                        ...
                        if next_record[0] == id:
                            ...
                            token = OAuthToken(client_id=next_record[2], client_secret=next_record[3], grant_token=grant_token,
