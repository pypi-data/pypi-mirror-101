...
class OMFITkeqdsk(OMFITnamelist):
    ...
    def from_omas(self, ods, time_index=0, time=None):
        ...
        for k, (name, case) in enumerate(mappings.items()):
            if case['omas'] in ods:
                self['IN1'][case['kfile'][0]] = ods[case['omas']] * case.get('norm', 1.0)
                if not isinstance(self['IN1'][case['kfile'][0]], float):
                    self['IN1'][case['kfile'][2]] = ~np.isnan(self['IN1'][case['kfile'][0]])
                    self['IN1'][case['kfile'][0]][np.isnan(self['IN1'][case['kfile'][0]])] = 0.0
            else:
    ...
    def compare_omas_constraints(self, ods, time_index=0, time=None):
        """
        Plots comparing constraints in the kEQDSK with respect to what are in an ODS

        :param ods: ods to use for comparison

        :param time_index: time_index of the constraint to compare
        ...
        checks = {
            'pf_active': {'omas': f'equilibrium.time_slice.{time_index}.constraints.pf_current.:.measured', 'kfile': 'BRSP'},
            'bpol_probe': {'omas': f'equilibrium.time_slice.{time_index}..constraints.bpol_probe.:.measured', 'kfile': 'EXPMP2'},
            'flux_loop': {'omas': f'equilibrium.time_slice.{time_index}..constraints.flux_loop.:.measured', 'kfile': 'COILS'},
        }

        fig = figure(num='kfile - omas comparison')
        fig.clf()

        for k, (name, case) in enumerate(checks.items()):
            try:
                odata = ods[case['omas']]
                n = len(odata)
                kdata = self['IN1'][case['kfile']][:n]
                kdata[kdata == 0] = np.nan  # ignore data that in keqdsk has is exaclty zero
                ...
                # error histograms
                ax = subplot(2, len(checks), len(checks) + k + 1)
                ax.hist((odata - kdata) / abs(kdata) * 100, linspace(-15, 15, 31))
                ax.set_aspect(1.0 / ax.get_data_ratio(), adjustable='box')
        ...
        try:
            print(f" * omas  = {ods[f'equilibrium.time_slice.{time_index}.constraints.ip.measured']:8.8} [A]")
            print(f" * kfile = {self['IN1']['PLASMA']:8.8} [A]")
        except Exception as _excp:
        ...
        try:
            print(f" * omas  = {ods[f'equilibrium.time_slice.{time_index}.constraints.diamagnetic_flux.measured']:8.8} [Wb]")
            print(f" * kfile = {self['IN1']['DFLUX'] / 1E3:8.8} [Wb]")
        except Exception as _excp:
            print("error in comparison: " + repr(_excp))
        ...
        try:
            print(f" * omas  = {ods[f'equilibrium.time_slice.{time_index}.constraints.b_field_tor_vacuum_r.measured']:8.8} [T*m]")
            print(f" * kfile = {self['IN1']['BTOR'] * self['IN1']['RCENTR']:8.8} [T*m]")
        except Exception as _excp:
            print("error in comparison: " + repr(_excp))
    ...
    def plot(self, plottype=None, fig=None, ax=None, label='', color=None, no_extra_info_in_legend=False, no_legend=False):
        ...
        new_plot_opened = (
            False
        )  # This is for a specific case where a plot that needs several subplots doesn't have enough so it has to make a new window. Assume False unless set True later
