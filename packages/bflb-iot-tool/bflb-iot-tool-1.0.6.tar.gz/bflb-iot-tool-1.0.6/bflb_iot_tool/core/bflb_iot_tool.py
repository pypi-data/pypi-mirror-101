...
class BflbIotTool(object):
    ...
    def __init__(self, chipname="bl60x", chiptype="bl60x"):
        ...
        self.bh_cfg_file = os.path.join(chip_path, chipname, "efuse_bootheader/efuse_bootheader_cfg.ini")
...
#             bflb_utils.printf(e)
    ...
    def bl60x_mfg_ota_xz_gen(self, chipname="bl60x", chiptype="bl60x", cpu_type=None):
        ...
        if cpu_type is None:
            fw_ota_path = os.path.join(chip_path, chipname, "ota/FW_OTA.bin")
        else:
            fw_ota_path = os.path.join(chip_path, chipname, "ota/" + cpu_type + "_OTA.bin")
        with open(fw_ota_path, mode="rb") as bin_f:
            file_bytes = bin_f.read()
            for b in file_bytes:
                fw_ota_bin.append(b)
        if cpu_type is None:
            fw_ota_path = os.path.join(chip_path, chipname, "ota/FW_OTA.bin.xz")
        else:
            fw_ota_path = os.path.join(chip_path, chipname, "ota/" + cpu_type + "_OTA.bin.xz")
        ...
        if cpu_type is None:
            fw_ota_path = os.path.join(chip_path, chipname, "ota/FW_OTA.bin.xz.ota")
        else:
            fw_ota_path = os.path.join(chip_path, chipname, "ota/" + cpu_type + "_OTA.bin.xz.ota")
    ...
    def bl60x_mfg_ota_bin_gen(self, chipname="bl60x", chiptype="bl60x", cpu_type=None):
        ...
        if cpu_type is None:
            fw_ota_path = os.path.join(chip_path, chipname, "ota/FW_OTA.bin.ota")
        else:
            fw_ota_path = os.path.join(chip_path, chipname, "ota/" + cpu_type + "_OTA.bin.ota")
        with open(fw_ota_path, mode="wb") as f:
...
#                               bl_factory_params_file_prefix + suffix)
        ...
        if config["check_box"]['media_download'] is True:
            ...
            if media_bin is not None and len(media_bin) > 0:
                try:
                    shutil.copy(media_bin, os.path.join(chip_path, chipname, "img_create_iot", "media.bin"))
                except Exception as e:
                    bflb_utils.printf(e)
            media_bin_d = True
    ...
    def flasher_download_thread(self, chipname, chiptype, act, config, callback=None):
        ...
        cpu_type = None   
        ota_path = os.path.join(chip_path, chipname, "ota")
        imgcreate2_path = os.path.join(chip_path, chipname, "img_create_mcu")
...
def iot_download_cmd(args):
    ...
    if chiptype == "bl702":
        ...
        if not args.xtal: 
            ...
            config["param"]["bl60x_xtal"] = args.xtal 
        if not args.dts: 
            config["input_path"]["dts_input"] = os.path.join(chip_path, chipname, "device_tree/bl_factory_params_IoTKitA_32M.dts")
        else:   
            config["input_path"]["dts_input"] = args.dts
    elif chiptype == "bl602":
        ...
        if not args.xtal: 
            ...
            config["param"]["bl60x_xtal"] = args.xtal 
        if not args.dts: 
            config["input_path"]["dts_input"] = os.path.join(chip_path, chipname, "device_tree/bl_factory_params_IoTKitA_40M.dts")
        else:   
            config["input_path"]["dts_input"] = args.dts
    elif chiptype == "bl60x":
        config["check_box"]["boot2_download"] = True
        ...
        if not args.xtal: 
            ...
            config["param"]["bl60x_xtal"] = args.xtal 
        if not args.dts: 
            config["input_path"]["dts_input"] = os.path.join(chip_path, chipname, "device_tree/bl_factory_params_avb_38.4M.dts")
        else:   
            config["input_path"]["dts_input"] = args.dts
    else:
        bflb_utils.printf("Chip type is not in bl60x/bl602/bl702")
