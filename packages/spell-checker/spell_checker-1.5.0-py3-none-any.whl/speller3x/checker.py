...
class Checker:
    ...
    def check(self, print=False):
        ...
        statistics = {
            ...
            "load_time": float, # this is the time spent on loading the words in a dictionary to the hashtable
            "clean_time": float # time spent on removing useless characters for the analysis
            statistics["runtime"] = float # time spent on looking up all of the words
            statistics["misspelled_num"] = # total number of misspelled words
        }
        ```
        """
        if not self.check_lang(self.language):
