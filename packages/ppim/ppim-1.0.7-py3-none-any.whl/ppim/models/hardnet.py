...
class HarDNet(nn.Layer):
    def __init__(self, depth_wise=False, arch=85, pretrained=True,
                 weight_path='', class_dim=1000, with_pool=True):
        super().__init__()
        first_ch = [32, 64]
        second_kernel = 3
        max_pool = True
...
def hardnet_39_ds(pretrained=False, **kwargs):
...
def hardnet_68(pretrained=False, **kwargs):
...
def hardnet_68_ds(pretrained=False, **kwargs):
...
def hardnet_85(pretrained=False, **kwargs):
