...
def check_success_wait(path):
    return check_success(path)
...
# 文件复制到本地读写：更快更方便
def read_hdfs(pattern, reader=pd.read_csv, max_workers=1, cache_dir='read_cache'):
    """支持多文件读取"""
...
def to_hdfs(
    ...
    if_not_exist_makdir(cache_dir)

    name = f"{cache_dir}/{name}"
    writer(df, name)
    cp(name, dir)
...
if __name__ == '__main__':
    print(check_success("/Users/yuanjie/Desktop/Projects/Python/MeUtils/meutils/data/_SUCCESS"))
