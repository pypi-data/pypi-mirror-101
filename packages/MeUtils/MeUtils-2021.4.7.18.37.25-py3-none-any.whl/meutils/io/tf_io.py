...
def cp(pattern, output_dir=DATA, with_success=True, filter_fn=None):
    ...
    # 过滤文件夹等
    files = tf.io.gfile.glob(pattern) | xfilter(lambda p: not tf.io.gfile.isdir(p)) | xlist
    ...
    # new_files = map(func, tqdm(files)) | xlist
    new_files = files | xThreadPoolExecutor(func, 16) | xlist  # 多线程
