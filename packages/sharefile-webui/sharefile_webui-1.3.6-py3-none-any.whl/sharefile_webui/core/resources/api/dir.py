...
class Dir(BaseResource):
    def __init__(self):
        super().__init__()
        self.add_argument("rename", str, "New filename for directory rename")
        ...
        for dir_item in dir_list:
            ...
            if os.path.isfile(full_path):
                result_list_files.append({
                    ...
                    "token": self.config.FILE_TOKENS.data.get(context_item_path, None)
                })
    ...
    def delete(self, path: str) -> dict:
        ...
        try:
            ...
            self.abort_error(f"Dir.delete: {ex}")
        return {
            "status": True
        }
    ...
    def put(self, path: str) -> dict:
        ...
        if rename:
            ...
            if not self._check_path_perms(full_path_rename):
                self.abort_error(f"Permission denied to rename file '{full_path}' to '{full_path_rename}'")
            try:
                ...
                self.abort_error(f"Dir.update: {ex}")
        return {
            "status": True,
