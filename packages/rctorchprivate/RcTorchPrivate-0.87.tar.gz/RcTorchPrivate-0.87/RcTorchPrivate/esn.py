...
class EchoStateNetwork(nn.Module):
    ...
    def train_state(self, t, X, state, y, output = False):
        ...
        # + self.PyESNnoise * (self.external_noise - 0.5)
        if self.noise != None:
        ...
        next_state = self.leaking_rate[0] * update + self.leaking_rate[1] * state #+  noise_vec*self.noise
    ...
    def train(self, y, X=None, burn_in=0, input_weight=None, verbose = False , 
        ...
        self.unscaled_X = check_x(X , y, self.dev).to(self.device)
        
        self.unscaled_X = Parameter(self.unscaled_X, requires_grad = self.track_in_grad)
        ...
        if not self.backprop:
            ...
            #####################################################3
            extended_states = hstack((self.X, self.states))
            ...
            if out_weights:
                ...
                for e in range(self.epochs):
                    ...
                    if ODE:
                        assert self.yfit.shape == self.dy_dx.shape, f'{self.yfit.shape} != {self.dy_dx.shape}'
                        loss = self.criterion(self.yfit, self.dy_dx) #torch.square(self.yfit - self.dy_dx).mean()
                    else:
    ...
    def error(self, predicted, target, method='nmse', alpha=1.):
        ...
        if type(method) != type("custom"):
            ...
            method(target = target, predicted = predicted)
            """
            try:
                error = 
            except:
