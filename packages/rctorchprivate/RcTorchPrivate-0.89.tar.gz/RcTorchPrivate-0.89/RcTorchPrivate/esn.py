...
class Recurrence(Function):
    ...
    @staticmethod
    def forward(ctx, input,  states, LinIn, LinOut, LinRes, bias_, n_nodes, activation_function, leaking_rate, noise, feedback, y,  tensor_args, dh_dx, LinFeedback = None):
        for t in range(1, input.shape[0]):
            ...
            next_state = leaking_rate[0] * update + leaking_rate[1] * state
    ...
    def backward(ctx, grad_output):
        ...
        return ctx.dh_dx, None, None, None, None, None, None, None, None, None, None, None, None, None, None
...
def dfx(x,f, retain_graph = True, create_graph = True, requires_grad = True, grad_outputs = None):
    try:
        ...
        return grad([f],[x], grad_outputs=torch.ones_like(f), 
                    create_graph = create_graph, retain_graph = retain_graph, only_inputs = False)[0]
    except:
        return grad([f],[x], grad_outputs=torch.ones_like(f), create_graph = create_graph, 
                             retain_graph = retain_graph, only_inputs = False)[0]
...
class EchoStateNetwork(nn.Module):
    ...
    def train_state(self, t, X, state, y, output = False):
        """
        Arguments:
    ...
    def train(self, y, X=None, burn_in=0, input_weight=None, verbose = False , 
        ...
        out_weights = None, save_after_n_epochs = 1000, ODE = True):
        """
        NLLLoss(),
        Train the network.
        ...
        if self.unscaled_X.std() != 0:
            self.X = self.scale(inputs=self.unscaled_X, keep=True).clone()
        else:
        ...
        if not self.backprop:
            ...
            X_detached = self.X.clone().detach().requires_grad_(True)
            

            for t in range(1, X_detached.shape[0]):
                input_t = X_detached[t, :].T
                state_t, output = self.train_state(t, X = input_t,
                                          state = states[t-1,:], 
                                          y = y[t-1], output = True)
                #with no_grad():
                #    dh_dx_t = dfx(input_t, state_t)
                #self.dh_dx = torch.cat((self.dh_dx, dh_dx_t))


                states = cat([states, state_t.view(-1, self.n_nodes)], axis = 0)
            #####################################################3
            extended_states = hstack((X_detached, states))

            if ODE:
                yfit_detached = self.LinOut(extended_states)

                self.dy_dx_orig = dfx(X_detached, yfit_detached) 
                self.dh_dx = self.dy_dx_orig / (self.LinOut.weight * self.LinOut.weight.shape[1])
                self.dy_dx = self.LinOut(self.dh_dx) - self.LinOut.bias
            ...
            extended_states = train_states_class.apply(self.X, 
                                                       self.states, 
                                                       self.LinIn, 
                                                       self.LinOut, 
                                                       self.LinRes, 
    ...
    def calculate_n_grads(self, X, y,  n = 2, scale = False):
        ...
        assert self.yfit.requires_grad, "entended doesn't require grad, but you want to track_in_grad"
        for i in range(n):
    ...
    def test(self, y, X=None, y_start=None, steps_ahead=None, scoring_method='nmse', alpha=1.):
        ...
        if self.ODE:
            ...
            score = self.criterion(self.X_val, y_predicted, dy_dx_val)
        else:
