...
class ServerObjectList(collections.abc.Sequence):
    ...
    def __init__(self, data: Any, get_method: Optional[Callable], item_class_name: str):
        self._data = data
        self._display_headers = ["", "Type", "Key", "Name"]
        self._display_table = [
            [
                i,
                item_class_name,
    ...
    def __repr__(self) -> str:
        return tabulate.tabulate(
            self._display_table, headers=self._display_headers, tablefmt="presto"
        )
...
def is_server_version_less_than(client: "_core.Client", version: str) -> bool:
    ...
    for i, x in enumerate(server_version_as_list):
        if int(x) < int(target_version_as_list[i]):
            return True
    return False
