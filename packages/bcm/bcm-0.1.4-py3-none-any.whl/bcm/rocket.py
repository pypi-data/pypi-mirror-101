...
class Map(arcade.Sprite):
    def __init__(self):
        super().__init__(filename=f"{SYS_RESOURCES_DIR}{os.sep}bottom_image.png", scale=1.2)
        self.position = SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2
...
class Window(arcade.Window):
    ...
    def on_draw(self):
        ...
        if self.bg_list:
            ...
            self.top_picture.draw()

            if self.rocket:
                self.rocket.rocket_list.draw()
                arcade.draw_text(str(int(self.rocket.height)), self.width / 2, self.top_picture.center_y,
                                 arcade.color.WHITE,
    ...
    def set_smoke_photo(self):
        ...
        self.smoke.index = 0
        self.smoke.position = self.width / 2 - 15, 237
        for j in range(2):
    ...
    def bg_move(self):
        ...
        for bg in [self.bg_list[0], self.bg_list[1], self.bg_list[2], self.bg_list[3]]:
            if bg.top < 0:
                bg.bottom = 3 * self.height
