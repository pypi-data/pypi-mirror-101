...
from sklearn.metrics import cohen_kappa_score, roc_curve, precision_recall_curve
...
from AISC.utils.feature import augment_features, remove_samples, get_classification_scores, balance_classes
...
from AISC.utils.vector import scale, translate, get_mutual_vectors
...
from AISC.FeatureExtractor.FeatureExtractor import SleepSpectralFeatureExtractor, SleepSpectralFeatureExtractor_trial
from AISC.FeatureExtractor.SpectralFeatures import mean_bands, mean_frequency, relative_bands
...
class SleepClassifierWrapper:
    ...
    def train(self, X, df):
        ...
        model0 = KDEBayesianModel_trial(fs=fs_downsample, fbands=fbands, bands_to_erase=[], Selector2=False)
        ...
        model2 = KDEBayesianModel_trial(fs=fs_downsample, fbands=fbands, bands_to_erase=bands_to_erase_2, Selector2=False)
        ...
        model7 = KDEBayesianModel_trial(fs=fs_downsample, fbands=fbands, bands_to_erase=bands_to_erase_7, Selector2=False)
        ...
        #### Train 72.5 ####
        X725 = X[(df.freq == 72.5) | (df.freq == 0)]
        df725 = df.loc[(df.freq == 72.5) | (df.freq == 0)].reset_index(drop=True)
        model725 = KDEBayesianModel_trial(fs=fs_downsample, fbands=fbands, bands_to_erase=[], Selector2=False)
...
class KDEBayesianModel_trial:
    ...
    def __init__(self, fbands=[[0.5, 5], # delta
                               ...
                               [11, 16], # spindle
                               [14, 20],
                               [20, 30]], segm_size=30, fs=250, attenuate_filters=7, repeat_attenuate=2, bands_to_erase=[],
        ...
        self.FeatureExtractor_MeanBand = SleepSpectralFeatureExtractor_trial(
            ...
            fbands=self.fbands,
            filters_attenuate= attenuate_filters,
            repeat_attenuate=repeat_attenuate,
        ...
        self.FeatureExtractor = SleepSpectralFeatureExtractor_trial(
            fs=self.fs,
            segm_size=self.segm_size,
            fbands=self.fbands,
            filters_attenuate=attenuate_filters,
    ...
    def extract_features_bulk(self, list_of_signals, return_names=False):
...
from hypnogram.analysis import get_transition_matrix_dataset

"""
class SleepDynamicMarkovModel:
    ...
    def create_model(self, hypnograms):
        self.tmat = get_transition_matrix_dataset(hypnograms, self.STATES)
        self.change_prob = copy(1 - self.tmat.diagonal())
        self.prob_change_to = copy(self.tmat / (self.tmat.sum(axis=1) - self.tmat.diagonal()).reshape(-1, 1))
        self.prob_change_to[range(self.prob_change_to.shape[0]), range(self.prob_change_to.shape[1])] = 0
