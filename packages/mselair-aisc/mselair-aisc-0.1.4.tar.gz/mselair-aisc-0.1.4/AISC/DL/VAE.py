...
class GRUVariantialSleepEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        dropout_rate = cfg.MODEL.TRAIN.DROPOUT
        ...
        self.gru = nn.GRU(int(67*filters_n), hidden_size=gru_n, num_layers=1, dropout=dropout_rate, bidirectional=False)
        ...
        self.up_gru = nn.GRU(gru_n, hidden_size=gru_n, num_layers=1, dropout=dropout_rate, bidirectional=False)
    ...
    def format_single(self, x):
        ...
        ySxx = torch.tensor(ySxx).long().to(device)
        x['Sxx'] = Sxx
        x['ySxx'] = ySxx
        return x

    def normalize(self, x, ax):
        q99 = torch.quantile(x, 0.99, axis=ax, keepdims=True)
        q01 = torch.quantile(x, 0.01, axis=ax, keepdims=True)
        den = (q99-q01)
        den[den == 0] = 1
        x = (x - q01) / den
        x[x<0] = 0
    ...
    def loss_crossentropy(self, x):
        x_ = x['scores']
        y = x['ySxx'].to(x_.device)

        loss = []
        for x__, y_ in zip(x_, y):
            x__ = x__[y_ >= 0].squeeze()
            y_ = y_[y_ >= 0].squeeze()
...
class Trainer:
    ...
    def do_epoch(self):
        ...
        if self.current_epoch % int(self.cfg.MODEL.TRAIN.VALIDATE_EPOCH) == 0:
            self.validate()
