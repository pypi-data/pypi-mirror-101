...
def create_tasks(task_type, entities):
    ...
    for entity in entities:
        ...
        if existing_task is None:
            ...
            genesis_service.create_task_file(entity=entity, task=task, task_type=task_type, project_id=entity["project_id"])
            tasks.append(task)
    Task.commit()
...
def clear_assignation(task_id):
    ...
    """
    # TODO clear assignation of task
...
def assign_task(task_id, person_id):
    ...
    genesis_service.grant_file_access(task, person, project_id)
    task.save()
    task_dict = task.serialize()
    events.emit(
        "task:assign",
