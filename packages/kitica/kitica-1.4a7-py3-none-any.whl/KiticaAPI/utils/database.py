...
class Database(Constants):
    ...
    def _set_device_status(self,
        ...
        tc_update = ""
        if deviceReturn:
            tc_update = ',currentTestCase=\"\", ' \
                + 'previousTestCase=\"' + str(device["currentTestCase"]) \
                + '\",' \
                + f'previousTestStatus=\"{testResult}\"'
        if device is not None:
    ...
    def _set_borrower_info(self, key,
        ...
        query = f'UPDATE device SET \
            borrowerIp=\"{ip}\", \
                borrowerHostname=\"{hostname}\", \
    ...
    def return_device(self, deviceId, testResult):
        device = self.get_device_by_id(deviceId)
        if device["status"] not in ['UNPLUGGED', 'FREE', 'DISABLED']:
            self._set_device_status(f'deviceId={deviceId}', 'FREE', testResult, True)
