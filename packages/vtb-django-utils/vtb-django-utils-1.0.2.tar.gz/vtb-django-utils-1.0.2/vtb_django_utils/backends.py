    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        try:
            ...
            user, _ = UserModel.objects.get_or_create(
                **{UserModel.USERNAME_FIELD: keycloak_id}
            )
...
def token_admin_info(token_info: Optional[Dict[str, Any]]) -> Tuple[bool, bool]:
    ...
    if realm_access is not None:
        ...
        if roles is not None:
            ...
            expected_super_admin = set(settings.KEY_CLOAK_SUPER_ADMIN_ROLES)
            expected_svc_admin = set(settings.KEY_CLOAK_SVC_ADMIN_ROLES)
