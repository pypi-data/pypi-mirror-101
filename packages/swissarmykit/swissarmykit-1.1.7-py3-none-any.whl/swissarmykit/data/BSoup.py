...
class BItem(BaseObject):
    ...
    def find_by_text(self, text='', selector=None):
        ...
        if self.is_not_null():
            pattern = '.*%s.*' % text
            item = self.item.findAll(tag, text=re.compile(pattern), attrs=attrs)
    ...
    def text(self, new_line=False, recursive=True, replace_white_space=False, remove_empty_line=False, callback=None):
        value = ''
        if self.item:
            if new_line:
    ...
    def find_contains(self, selector='div', attr_name='id', contains_str='text_'): # type: (str, str, str) -> BItem
        ''' #todo: [class*="test"]  https://www.w3schools.com/cssref/sel_attr_contain.asp'''
        return BItem(item=self.item.select_one('%s[%s*=%s]' % (selector, attr_name, contains_str)))

    def find_all_contains(self, selector='div', attr_name='id', contains_str='text_'): # type: (str, str, str) -> list
        ''' #todo: [class*="test"]  https://www.w3schools.com/cssref/sel_attr_contain.asp'''
        lst = []
        for item in self.item.select('%s[%s*=%s]' % (selector, attr_name, contains_str)):
            lst.append(BItem(item=item))
        return lst
    ...
    def find_all(self, selector, recursive=True):  # type: (str|list, bool) -> [BItem]

        if isinstance(selector, list):
            lst = []
            for s in selector:
                lst.extend(self.find_all(s))
            return lst
    ...
    def split_by(self, selector=None, text=None, in_text=None, split_all=False, maxsplit=1):
        if self.item:
            val = None
            ...
            if val:
                ...
                return [BItem(i) for i in html.split(sub, maxsplit)]
    ...
    def to_temp_html(self):
        print('INFO: URL: ', self.item.url)
        FileUtils.to_html_file(path=FileUtils.conf.DIST_PATH + '/tmp.html', data=str(self.item))
        return True
    ...
    def dump(self, path=None):
        if not path: path = self.get_cls_name() + '_tmp'
        FileUtils.dump_object_to_file(str(self.item.html), path=path)

    @classmethod
    def load(cls, path=None):
        if not path: path = cls.get_cls_name() + '_tmp'
        return BItem(FileUtils.load_object_from_file(path=path))
