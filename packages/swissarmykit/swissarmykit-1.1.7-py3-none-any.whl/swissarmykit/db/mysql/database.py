...
from swissarmykit.db.mysql.schema import SchemaObj

from swissarmykit.conf import *

class RecordModel(Model):
    '''
        Deprecate: Use MySQL and sqlite3 to generate dynamic table.
        Use MongoDB now
    '''

    @classmethod
    def get_db(cls, database=swissarmykit_conf.DATABASE_NAME):
        ...
        if not swissarmykit_conf:
            raise Exception('Must import definitions.py which contains: swissarmykit_conf: swissarmykit_conf = swissarmykit_conf.instance()')

        if swissarmykit_conf.is_use_mysql():
            config = swissarmykit_conf.config.get('mysql')
            ...
            return SqliteDatabase(swissarmykit_conf.DATABASE_PATH + '/' + database)
    ...
    def dump_mysql_of_this_table(cls):
        ...
        file0 = swissarmykit_conf.HTML_PATH + os.sep + table
        file1 = cls.dump_on_query_by_id()

        files = [file1, file0]

        file_name = swissarmykit_conf.get_desktop_path() + '/' + table + '_sql.zip'
    ...
    def dump_mysql_to_sqlite(cls):
        ...
        table = cls.get_table_name()
        file1 = cls.dump_on_query_by_id()
        program = swissarmykit_conf.BIN_PATH + os.sep + 'mysql2sqlite'

        command = '%s %s > %s/%s_lite.sql' % (program, file1, swissarmykit_conf.USER_DESKTOP, table)
    ...
    def dump_on_query_by_id(cls, ids: list = None, path=None, file_name=None, table=None):
        ...
        if not path:
            path = swissarmykit_conf.get_desktop_path()
    ...
    def create_table_if_not_exists(cls, unique_lst=None):
        ...
        if not swissarmykit_conf.is_use_mysql() and 'url' in sql:
            table = cls.get_table_name()
...
class %s(%s):\n%s    
    ...
    def get_schema(cls, unique_lst=None, create_if_not_exist=False):
        table = cls.get_table_name()
        is_use_mysql = swissarmykit_conf.is_use_mysql()
    ...
    def export_data_to_csv(cls, lst=None, headers=None, file_name=None):
        file = swissarmykit_conf.EXCEL_PATH + '/' + (file_name if file_name else cls.get_table_name()) + '.csv'
    ...
    def export_data_to_excel(cls, lst=None, headers_order=None, col_name='data', limit=-1, offset=0, format='', headers=None,
        ...
        if not file_name:
            file_name = swissarmykit_conf.EXCEL_PATH + '/' + cls.get_table_name() + '.xlsx'
    ...
    def output_html(cls, id=None, path=None):
        ...
        path = path if path else swissarmykit_conf.USER_DESKTOP + '/_test_html.html'
