...
class ExtendedModel(object):
    ...
    @staticmethod
    def get_workspace(fname:str, ws_name:str):
        if not os.path.exists(fname):
            raise FileNotFoundError('workspace file {} does not exist'.format(fname))
        file = ROOT.TFile(fname) 
        if (not file):
    ...
    @staticmethod
    def get_model_config(fname:str, ws_name:str,  model_config_name:str):
    ...
    @staticmethod       
    def get_nuisance_parameters(fname:str, ws_name:str, model_config_name:str, as_str=True):
    ...
    @staticmethod       
    def get_global_observables(fname:str, ws_name:str, model_config_name:str, as_str=True):
        model_config = ExtendedModel.get_model_config(fname, ws_name, model_config_name)
        global_observables = model_config.GetGlobalObservables()
        if as_str:
    ...
    @staticmethod       
    def get_parameters_of_interest(fname:str, ws_name:str, model_config_name:str, as_str=True):
        model_config = ExtendedModel.get_model_config(fname, ws_name, model_config_name)
        pois = model_config.GetParametersOfInterest()
        if as_str:
    ...
    @staticmethod
    def get_datasets(fname:str, ws_name:str, as_str=True):
        ws = ExtendedModel.get_workspace(fname, ws_name)
        if as_str:
