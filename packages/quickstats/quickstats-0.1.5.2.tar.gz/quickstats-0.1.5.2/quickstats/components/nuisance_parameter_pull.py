...
class NuisanceParameterPull(object):
    ...
    _CLI_ARG_OPTIONS_    = \
        {
            ...
            'no_cache'  : {
                'abbr'        : None,
                'description' : 'Cache existing result', 
                'required'    : False,
                'type'        : bool,
                'default'     : False
            },
            'exclude'  : {
                'abbr'        : None,
                'description' : 'Exclude NPs (wildcard is accepted)', 
                'required'    : False,
                'type'        : str,
                'default'     : ''
            }        
    ...
    def _run_pulls(input_file:str, workspace:str, model_config:str, data:str,
                   ...
                   max_iters:int=-1, outdir:str='output', no_cache=False, **kwargs):
        outname_root = os.path.join(outdir, nuis_name + '.root')
        outname_json = os.path.join(outdir, nuis_name + '.json')
        #checkpointing:
        if (os.path.exists(outname_root) and os.path.exists(outname_json)) and (not no_cache):
    ...
    def run_pulls(self, input_file='workspace.root', workspace='combWS', model_config='ModelConfig',
                 ...
                 offset=1, optimize=2, parameter="", max_calls=-1, max_iters=-1, parallel=0, no_cache=False, **kwargs):
        ...
        nuis_names = list(set(nuis_to_include) - set(nuis_to_exclude))
