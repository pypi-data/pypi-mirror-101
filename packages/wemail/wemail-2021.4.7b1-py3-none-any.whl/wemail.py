...
class Looper(Cmd):
    ...
    def do_cd(self, line):
        ...
        """
        os.chdir(line or "..")
        self.config["curdir"] = Path()

    def complete_cd(self, text, stuff, begidx, endidx):
        return [file.name for file in Path().iterdir() if file.name.startswith(text)]
    ...
    def do_list(self, line):
        ...
        """
        list_messages(config=self.config)

    def do_check(self, line):
        """
        Check email.
        """
        check_email(config=self.config)

    def do_read(self, line):
        """
        Syntax: 
          read <n> - read the email with the provided number
          read <text> - read an email by search, with tab autocomplete
        """
        line = line.strip()
        # TODO: add text search option -W. Werner, 2021-04-02
        if not line:
            mailnumber = 1
        elif line.isdigit():
            mailnumber = int(line)
        read(config=self.config, mailnumber=mailnumber)

    def do_raw(self, line):
        """
        View the raw email with full headers.
        """
        mailnumber = 1
        if line.isdigit():
            mailnumber = int(line)
        raw(config=self.config, mailnumber=mailnumber)

    def do_rm(self, line):
        """
        Syntax:
          rm <n> - Move to trash the message  referenced by n, or the oldest
          message.
        """
        mailnumber = 1
        if line.isdigit():
            mailnumber = int(line)
        remove(config=self.config, mailnumber=mailnumber, maildir=self.config["curdir"])
        list_messages(config=self.config, limit=1)
    ...
    def do_save(self, line):
        ...
        Save a message to a folder. If message number is missing, use message
        #1. If folder is missing, use default (saved-messages).
        """
        num, _, folder = line.partition(" ")
        num = num.strip()
        folder = folder.strip()
        if not num.isdigit():
            if folder:
                print(f"ERROR: Unable to save message number {num!r} to {folder!r}")
            else:
                folder = num
                num = 1
        else:
            num = int(num)
        # TODO: Need a better default option here... -W. Werner, 2021-04-02
        save(
            config=self.config,
            maildir=self.config["curdir"],
            mailnumber=num,
            target_folder=folder or "saved-messages",
        )
        list_messages(config=self.config, limit=1)
    ...
    def complete_save(self, text, line, begidx, endidx):
        return [
            file.name
            for file in self.config["maildir"].iterdir()
    ...
    def do_reply(self, line):
        """
        Syntax:
          reply <n> - Reply to the (optional) message numbered <n>.
        """
        # TODO this hax is silly. Can we just fix the reply function?
        class C:
            ...

        o = C()
        o.name = line or "1"
        reply(config=self.config, mailfile=o)

    def do_html(self, line):
        """
        View the HTML attachment in your browser.

        Synax:
          html <n> <browser>
        """
        num, _, browser = line.partition(" ")
        if not browser:
            if num.isdigit():
                num = int(num)
            else:
            ...
            if num.isdigit():
                num = int(num)
        view_html(config=self.config, mailnumber=num, browser=browser or None)

    # aliases
    do_ls = do_list
    do_r = do_read
    do_s = do_save
    complete_s = complete_save
...
def make_parser():
    ...
    interactive_parser = subparsers.add_parser(
        "interactive", help="Start a wemail shell"
    )
    interactive_parser.set_defaults(action="interactive")
...
def create_draft(*, template, config):
    ...
    msg = _parser.parsebytes(template)

    f = draft_dir / _make_draftname(subject=(msg["subject"] or ""))
    f.write_bytes(template)
    return f
...
def reply(*, config, mailfile, reply_all=False, keep_attachments=False):
    ...
    draft = create_draft(template=msg.as_bytes(), config=config)
...
def iter_headers(*, maildir, limit=None):
    i = 0
...
def list_messages(*, config, limit=None):
    ...
    for i, msg in enumerate(iter_headers(maildir=maildir, limit=limit), start=1):
        ...
        status = ""
        print(f"{status:<1} {i:>2}. {date_str} - {sender} - {subject}")
    if not any_seen:
        print("No mail!")
...
def view_html(*, config, mailnumber, browser=None):
    mailfile = sorted_mailfiles(maildir=config["curdir"])[mailnumber - 1]
    with tempfile.NamedTemporaryFile(suffix=".html") as temphtml:
        msg = _parser.parsebytes(mailfile.read_bytes())
        temphtml.write(msg.get_body(("html", "plain")).get_payload(decode=True))
        temphtml.flush()
        # browser = webbrowser.get(browser)
        # TODO: Let specify browser -W. Werner, 2021-04-02
        webbrowser.open("file:///" + temphtml.name)
...
def do_it_two_it(args):  # Shia LeBeouf!
    ...
    try:
        ...
        if args.action == "new":
            ...
            return Looper(config=config).cmdloop()
