...
from PowerbotBacktesting.utils import _order_matching
...
def flexpos_algo(orderbooks: Dict[str, pd.DataFrame],
    ...
    start_from = [datetime.strptime(i.replace(f"{str(i).split(' - ')[1]}", "").replace(" - ", ":00"), DATE_YMD_TIME_HMS) for
                  i in [*orderbooks]]
    ...
    for nr, (timestamp, contract) in enumerate(orderbooks.items()):
        for key, orderbook in contract.items():
            if datetime.strptime(key, DATE_YMD_TIME_HMS) >= start_from[nr]:
...
def flex_algo(orderbooks: Dict[str, pd.DataFrame],
    ...
    for nr, (key, contract) in enumerate(orderbooks.items()):
        ...
        contract_time = (datetime.strptime(contract_time[3], TIME_HM) - datetime.strptime(contract_time[1],
                                                                                          TIME_HM)).seconds / 60

        for timestamp, orderbook in contract.items():
            # Stopping Loop on closing Timestamp
            if closing_from and datetime.strptime(timestamp, DATE_YMD_TIME_HMS) >= closing_timestamps[nr]:
                break
            # Buy
