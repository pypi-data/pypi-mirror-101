...
def generar_kpis_traslado(df,ANIO,T_ANIOS,modalidad):
...
def generar_kpis_traslado_a_publico(df,ANIO,T_ANIOS,df_servicios,modalidad="EBR"):
...
def generar_kpis_desercion(df,ANIO,T_ANIOS,modalidad="EBR"):
    
    if(T_ANIOS<=1):
        df['TOTAL_DESERCIONES'] = np.nan
        df['MEAN_DESERCIONES'] = np.nan
        df['STD_DESERCIONES'] = np.nan
        return df
...
def generar_kpis_historicos(df,anio_df=None,anio_h=None,cls_json=None,t_anios=0,modalidad="EBR"):
    ...
    if(t_anios < 1):            
        print("ERROR: El numero t_anios debe ser mayor o igual a  ",1)        
        return False
    
    ultimo_anio =anio_h- t_anios
    ultimo_anio_data = ultimo_anio + 1
    if(ultimo_anio_data<2014):            
        print("ERROR: Se pretende consultar hasta el anio ",ultimo_anio_data,", solo se tiene data hasta el 2014")        
        return False    
...
df_h = generar_kpis_historicos(df,anio_df=2017,anio_h=2016,cls_json=cls_json,t_anios=1 )
'''
