...
def get_df_procesado(anio,col_name_y,grupo_grados):
    ...
    #df_reg = hsk.generar_kpis_historicos(df_reg,anio_df=anio,anio_h=anio-2,cls_json=cls_json,t_anios=2)
    #df_reg = htk.agregar_sisfoh(df_reg)

    #print("df_reg.SISFOH_CSE.value_counts()")
    #print(df_reg.SISFOH_CSE.value_counts())
...
for grupo_grado in grupo_grado_list:
    ...
    model , predicted_probas  , kpis = hcm.modelar_clasificacion_binaria("neg_bagging_fraction__lgb_model", X_train,y_train,X_test,y_test)
