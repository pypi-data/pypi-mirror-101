...
class Application(object):
    ...
    def _init_pipeline(self, pipeline: Pipeline) -> Set[str]:
        desired_worker_ids = self._init_pipeline_workers(pipeline)
        self._init_pipeline_mod_views(pipeline)
        return desired_worker_ids

    def _init_pipeline_workers(self, pipeline: Pipeline) -> Set[str]:
        # Reconcile workers
    ...
    def _init_pipeline_mod_views(self, pipeline: Pipeline):
        def generate_mod_view_projections(to_state: PipelineModViewToState) -> NamedModViewColumn:
            ...
            return NamedModViewColumn(
                ...
                column=UpdateOneButton(
                    text=to_state.name,
                    callback_id=f'pipeline.{pipeline.name}.{mod_view.from_state}.{to_state.name}.'
