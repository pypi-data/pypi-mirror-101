...
from broccoli_server.pipeline import Pipeline, PipelineState, PipelineModView, PipelineWorker, PipelineWorkerInstance, \
    StateUpdate, PipelineModViewToState
...
class TestPipelineAssertDag(TestPipeline):
    def test_assert_dag_straight_line_pipeline(self):
        ...
        pipeline.add_worker_instance(self.create_demo_worker_instance("state1", {"state2"}))
        pipeline.add_mod_view(PipelineModView("state2", [PipelineModViewToState("state3")], [], ''))
    ...
    def test_assert_dag_simple_pipeline(self):
        ...
        pipeline.add_worker_instance(self.create_demo_worker_instance("state1", {"state2"}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state2", {"state3"}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state2", {"state4"}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state3", {"state4"}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state4", {"state5"}))
        pipeline.assert_pipeline_is_dag()
    ...
    def test_assert_not_dag(self):
        ...
        pipeline.add_state(PipelineState("state5", {}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state1", {"state2"}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state3", {"state2"}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state2", {"state4"}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state4", {"state3"}))
        pipeline.add_worker_instance(self.create_demo_worker_instance("state4", {"state5"}))
...
class TestPipelineAssertValidMigration(TestPipeline):
    ...
    def test_assert_tricky_missing_and_duplicate_doc_invalid(self):
        ...
        self.assertRaisesRegex(
            RuntimeError, "is duplicated across state migrations",
            pipeline.assert_valid_state_migrations,
            self.content_store, 2
        )
