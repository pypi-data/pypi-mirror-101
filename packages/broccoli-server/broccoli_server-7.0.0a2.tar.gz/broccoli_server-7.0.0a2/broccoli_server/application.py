...
class Application(object):
    ...
    def __init__(self, pipeline: Pipeline = None):
        ...
        self.pipelines = []  # type: List[Pipeline]
        self._init_pipelines([pipeline])
    ...
    def _init_pipelines(self, pipelines: List[Pipeline] = None):
        if not pipelines:
            return

        # Assert all pipelines are DAGs
        for pipeline in pipelines:
            pipeline.assert_pipeline_is_dag()
        self.pipelines = pipelines

        # Also preemptively add workers and mod views
        # If state migration fails (either pre-assertion or actual execution), the reconciler would not start
        # Hence the added workers won't take affect
        desired_all_pipeline_worker_ids = set()
        for pipeline in pipelines:
            desired_pipeline_worker_ids = self._init_pipeline(pipeline)
            desired_all_pipeline_worker_ids |= desired_pipeline_worker_ids

        # Remove workers for an entire pipeline if it's removed
        ...
        for worker_id in actual_all_pipeline_workers.keys() - desired_all_pipeline_worker_ids:
            print(f"Removing pipeline worker {worker_id} (pipeline gone)")
            self.worker_config_store.remove(worker_id)

    def _init_pipeline(self, pipeline: Pipeline) -> Set[str]:
        # Reconcile workers
        ...
        for worker_instance in pipeline.worker_instances:
            # Register worker module
            module_name = f"{pipeline_namespace}.{worker_instance.from_state}.{'|'.join(sorted(worker_instance.to_states))}"
            self.register_worker_module(
                module_name=module_name,
                constructor=worker_instance.clazz
            )

            # Get worker ID
            pipeline_worker_args = {
                'pipeline_name': pipeline.name,
                'from_state': worker_instance.from_state,
                'to_states': worker_instance.to_states,
                'limit': worker_instance.limit,
                'sort': worker_instance.sort
            }
            # putting pipeline_worker_args after so that it has the final say on those args
            args = {**worker_instance.extra_args, **pipeline_worker_args}
            status, worker_or_message = self.worker_cache.load_module(module_name, args)
            if not status:
                raise RuntimeError("Fails to load worker")
            ...
            if worker_id not in actual_workers:
                # Add this worker if not already present
                print(f"Preemptively adding pipeline worker {worker_id}")
                ...
                print(f"Pipeline worker {worker_id} exists, no need to add preemptively")

        for worker_id in actual_workers.keys() - desired_worker_ids:
            print(f"Removing pipeline worker {worker_id}")
            self.worker_config_store.remove(worker_id)

        def generate_mod_view_projections(_to_state: PipelineModViewToState) -> NamedModViewColumn:
            if not _to_state.update_set_doc:
                button_text = _to_state.to_state
            else:
                button_text = f"{str(_to_state.update_set_doc)} -> {_to_state.to_state}"

            update_set_doc = _to_state.update_set_doc if _to_state.update_set_doc else {}
            # putting StateKey: ... after so that it has the final say on StateKey
            new_update_set_doc = {**update_set_doc, **{
                StateKey: _to_state.to_state
            }}

            return NamedModViewColumn(
                name=_to_state.to_state,
                column=UpdateOneButton(
                    text=button_text,
                    callback_id=f'pipeline.{pipeline.name}.{mod_view.from_state}.{button_text}',
        ...
        for mod_view in pipeline.mod_views:
            self.add_mod_view(
                name=f"pipeline.{pipeline.name}.{mod_view.from_state}->{'|'.join(sorted(mod_view.to_state_names()))}",
    ...
    def start_database_migration(self):
        ...
        for pipeline in self.pipelines:
            print("---------")
            pipeline.assert_valid_state_migrations(self.content_store)
        for pipeline in self.pipelines:
            print("---------")
