...
from broccoli_server.pipeline import Pipeline, StateKey
...
class Application(object):
    ...
    def _init_pipeline(self, pipeline: Pipeline) -> Set[str]:
        ...
        for worker_instance in pipeline.worker_instances:
            ...
            args = {**pipeline_worker_args, **worker_instance.extra_args}
        ...
        for mod_view in pipeline.mod_views:
            self.add_mod_view(
                name=f"pipeline.{pipeline.name}.{mod_view.from_state}->{'|'.join(sorted(mod_view.to_states))}",
                mod_view=ModViewQuery(
                    query={
                        StateKey: mod_view.from_state
                    },
                    projections=mod_view.projections + list(map(
                        lambda to_state: NamedModViewColumn(
                            name=to_state,
                            column=UpdateOneButton(
                                text=to_state,
                                callback_id=f'pipeline.{pipeline.name}.{mod_view.from_state}.{to_state}',
