...
from openmodule.config import validate_config_module, settings
...
class OpenModuleCore(threading.Thread):
    ...
    def init_database(self):
        if self.config.TESTING:
            self.database = Database(settings.DATABASE_FOLDER, self.config.NAME, alembic_path="../src/database")
        else:
            self.database = Database(settings.DATABASE_FOLDER, self.config.NAME)
