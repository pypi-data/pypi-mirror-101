...
def process_message(callable_function):
    def process(queue):
        while True:
            message = queue.get()
            if message:
                callable_function(message)
...
def multiprocess_kafka_consumer(
    ...
    consumer = create_consumer(bootstrap_servers, input_topic, partition)
    message_queue = [mp.Manager().Queue() for i in range(n_worker)]
    worker_queue = create_worker_queue(n_worker)

    for i in range(n_worker):
        worker = mp.Process(
            target=process_message(callable_function),
