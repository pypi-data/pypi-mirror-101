...
def register_localstack_plugins():
 ...
 if SilHb([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if 'No module named' not in SilHQ(e):
   print('ERROR: %s'%e)
...
def api_key_configured():
 return SilHo(os.environ.get('LOCALSTACK_API_KEY'))
