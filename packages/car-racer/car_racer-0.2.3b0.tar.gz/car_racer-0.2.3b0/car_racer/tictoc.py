...
def timed_function_statistics(function_name: str,
    ...
    if function_name in times:
        try:
            ...
            raise SyntaxError(f"calling {func.__name__} on result raises an exception: {e}\n")
    else:
        raise ValueError(f"no execution times saved for {function_name}")


def consolidate(function_name: str,
                func: Callable = statistics.mean,
                *args,
                delta: bool = False,
                **kwargs):
