...
class AccountService(BaseRdbmsService):
    ...
    DEFAULT_CDN_BUCKET_NAME_PICTURE = 'photo'
    ...
    REGEX_PASSWORD_REQUIREMENTS = re.compile(r'^[a-zA-Z0-9@#$%\^&*+._<>=-]{6,18}$')
    ...
    def __delete_password_reset_request(
            self,
            request_id,
            connection=None):
        """
    ...
    @staticmethod
    def __generate_password(length=8, is_punctuation_allowed=False):
        """
        Generate a password
        ...
        :param is_punctuation_allowed: Indicate whether the password can
    ...
    def assert_password_reset_request_nonce_valid(
        ...
        with self.acquire_rdbms_connection(auto_commit=False, connection=connection) as connection:
            ...
            # function is valid.
            if (nonce and request.nonce != nonce) \
               or (request_id and request.request_id != request_id):
            ...
            has_request_expired = request.object_status != ObjectStatus.enabled or ISO8601DateTime.now() > request.expiration_time
            if has_request_expired and request.object_status == ObjectStatus.enabled:
                self.__delete_password_reset_request(request.request_id, connection=connection)
    ...
    def request_password_reset(
        ...
        with self.acquire_rdbms_connection(auto_commit=True, connection=connection) as connection:
            ...
            # Delete a previous request that exists but has expired.
    ...
    def reset_password(
        ...
        with self.acquire_rdbms_connection(auto_commit=True, connection=connection) as connection:
            ...
            # fulfilled.
            self.__delete_password_reset_request(
                request.request_id,
                connection=connection)
    ...
    def sign_up(
        ...
        if first_name and last_name:
            full_name = prosoponym.format_full_name(first_name, last_name, locale, full_name=full_name)
    ...
    def upload_picture(
        ...
        with self.acquire_rdbms_connection(auto_commit=True, connection=connection) as connection:
            ...
            # Network File System (NFS).  This file will be read by background task
            # for additional processing.
            picture_file_path_name = self.__store_picture_image_file(
                picture.picture_id,
                image,
