...
class SMWinservice(win32serviceutil.ServiceFramework):
    ...
    def write_to_db(self,status = 'start', options=[''],retry=0):
        db_conn = self.connect_to_db()
        if db_conn:
            try:
                cursor = db_conn.cursor()
                if self._daily_:
                    ...
                    temp_daily = 'NULL'
                if status == 'start':
                    cursor.execute(self.start_query.format(table=self._svc_name_,type=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily))
                if status == 'finish':
                    cursor.execute(self.finish_query.format(table=self._svc_name_,status=options[0]))
                cursor.commit()
                cursor.close()
                db_conn.close()
                return True
                ...
                if datetime.datetime.now() > checker:
                    ...
                    if datetime.datetime.now().hour == self._daily_:
                        ...
                        try:
                            ...
                            self.write_to_db(status='finish',options=[result])
