...
class axidma:
    ...
    def mm2s_is_idle(self):
        isIdle = getbit(self.axil.read32(0x4),0) or getbit(self.axil.read32(0x4),1) #halt or idle
        return isIdle
    def s2mm_is_idle(self)
        isIdle = getbit(self.axil.read32(0x34),0) or getbit(self.axil.read32(0x34),1) #halt or idle
        return isIdle
...
class axicdma:
    def __init__(self,axil_offset=0):
        self.axil = axilite(axil_offset)
        self.dma_bufsize = 2**23
    def is_idle(self):
        isIdle = getbit(self.axil.read32(0x4),1) #idle
        return isIdle
