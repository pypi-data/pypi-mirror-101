...
#        print(["nohup", fs_locations["runtime_exec"] + mname])
    ...
    while not is_over:
        ...
        for job_id, task_list in tasks_per_job:
            ...
            if protocol == 'remote':
                manager_cmd = ["ssh", remote_machine, "stat", macname]
                chk_time_cmd = "ssh " + remote_machine + " echo $(date +%H:%M)"
            elif protocol == 'remote-sharedfs':
                manager_cmd = ["stat", macname_sf]
                chk_time_cmd = "echo $(date +%H:%M)"
            elif protocol == 'local':
                manager_cmd = ["stat", macname]
                chk_time_cmd = "echo $(date +%H:%M)"
            if DEBUG:
            ...
            if touch_time_txt.strip():
                ...
                touch_time = -9999
                for line in touch_time_txt.split('\n'):
                    fields = line.split()
                    if fields[0] == "Modify:":
                        ff = fields[2].split(":")
                        touch_time = int(ff[0])*60 + int(ff[1])
                        break
                if touch_time == -9999:
                    print("ERROR reading {0}\n{1}".format(manager_cmd, touch_time_txt))
                    exit(1)
