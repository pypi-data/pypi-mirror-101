...
@cmdln.option('--build-suffix', dest='build_suffix',
              help='build suffix to use instead of automatically incremented one')
@cmdln.option('--preserve-metadata', dest='keep_metadata', action='store_true',
              help='do not modify the project and repository references in the metadata')
@cmdln.option('--copy-packages-only', dest='copy_only', action='store_true',
              help='do not copy the metadata or the project configuration')
@cmdln.option('--no-verify', dest='no_verify', action='store_true',
