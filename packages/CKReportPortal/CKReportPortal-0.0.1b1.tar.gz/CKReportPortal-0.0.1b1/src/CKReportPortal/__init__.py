...
class CKReportPortal(object):
    ...
    @keyword('RP Config')
    def RPConfig(rp_endpoint, rp_project, rp_uuid):
        """
        Examples
    ...
    def RPStartBulid(rp_launch_name):
        """
        Examples
    ...
    def RPStartTest(rp_testname, rp_description=None, rp_parameters={'':''}):
        """
        Examples
        ...
        """
        item_id = service.start_test_item(name=rp_testname, description=rp_description, start_time=str(int(time()*1000)), item_type="STEP", parameters=rp_parameters)
        return item_id

    @keyword('RP Start Test Step')
    def RPStartLog(item_id, rp_message, rp_attachment=''):
        """
        Examples
        | ${ReturnID}           |  RP Start Test  |  TEST  |                |       
    ...
    def RPFinishTest(item_id, rp_status='PASSED'):
        """
        Examples
        | ${ReturnID}         |  RP Start Test  |  TEST  |
        | RP Start Test Step  |  ${ReturnID}    |  TEST  |                | 
