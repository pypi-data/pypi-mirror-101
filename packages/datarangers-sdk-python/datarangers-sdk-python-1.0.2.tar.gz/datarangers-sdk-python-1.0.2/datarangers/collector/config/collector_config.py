...
from datarangers.collector.model.message import Message, Messages
...
class DataRangersSdkConfig:
    ...
    def set_domain(self, domain: str):
        ...
        self.__applog = "{}{}".format(self.__domain, "/sdk/list")
    ...
    def init(self, config: dict):
        if not self.is_ready():
            ...
            if self.is_send():
                ...
                self.sdk_queue = queue.Queue(maxsize=self.get_queue_length())
                self.__sdk_executor = ThreadPoolExecutor(max_workers=2)
                self.__sdk_executor.submit(DataRangersSdkConfig.consumer_thread,
                                           "datarangers-consumer-{}".format("writer"),
                                           self.sdk_queue)
            self.ready = True

    @staticmethod
    def consumer_thread(name, t_queue: queue.Queue):
        if dataRangersSdkConfig.is_send():
        ...
        while True:
            try:
                ...
                for i in range(20):
                    try:
                        msg = t_queue.get(block=True, timeout=0.05)
                        if msg:
                            messages.push(msg)
                    except:
                        continue
                if not messages.is_empty():
                    if dataRangersSdkConfig.is_send():
                        logging.debug(messages.get_json())
                        try:
                            res = session.post(dataRangersSdkConfig.get_applog(),
                                               headers=dataRangersSdkConfig.get_http_header(),
                                               data=messages.get_json(),
                                               timeout=dataRangersSdkConfig.get_http_time_out())
                            result = json.loads(res.content)
                            logging.info(result)
                            if "message" not in result or result["message"] != "success":
                                logging.error("Send Error")
                                dataRangersSdkConfig.sdk_error_logger.debug(messages.get_json())
                        except Exception as e:
                            logging.error(e)
                            dataRangersSdkConfig.sdk_error_logger.debug(messages.get_json())
                    else:
                        dataRangersSdkConfig.sdk_logger.info(messages.get_json())
            except Exception as e:
                print(e)
                break
