...
class OWLearningCurveA(OWWidget):
    ...
    def _update_table(self):
        ...
        self.table.setRowCount(len(self.opt().keys()))
        self.table.setColumnCount(4)

        self.table.setHorizontalHeaderLabels(
            tuple([x[0] for x in self.scoring])
        )
        l=list(self.opt().keys())
        l=tuple([str(i)for i in l])
        self.table.setVerticalHeaderLabels(
            l
        )

        for row in range(1,len(self.opt().keys())+1):
            for column in range(0, 4):
                self.table.setItem(
                    row-1, column, QTableWidgetItem(str(self.opt()[list(self.opt().keys())[row-1]][column]))
                )
    ...
    def opt(self):
        ...
        optimizer.maximize(init_points=10, n_iter=50, acq='ei', xi=0.6)
