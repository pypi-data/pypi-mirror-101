...
class OWLearningCurveA(OWWidget):
    ...
    def _update_table(self):
        ...
        #테이블 열 라벨(acc, auc, precision, recall)
        self.table.setHorizontalHeaderLabels(
            tuple("n_estimator",[x[0] for x in self.scoring])
        )
        ...
        for row in range(0,len(keys)+1): #행 : 0 ~ 찾은 최적의 n_estimator의 개수까지
            for column in range(0, 4): #열 : acc, auc, precision, recall
                self.table.setItem(
                    row, column, QTableWidgetItem(str('%.3f' % self.score[list(keys)[row]][column]))
                )
