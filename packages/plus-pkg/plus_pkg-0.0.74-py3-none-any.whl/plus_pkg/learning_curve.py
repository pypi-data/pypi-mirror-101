...
class OWLearningCurveA(OWWidget):
    ...
    def _update_table(self):
        ...
        sdict = sorted(self.score.items(), key=operator.itemgetter(1), reverse=True)
        self.score = dict(sdict)
    ...
    def black_box_function(self, n_estimators):
        ...
        if n_estimators not in self.score.keys(): #초기값
            ...
            if self.score[n_estimators][0] < s[n_estimators]: #같은 n_est값을 찾았을 때(int변환 때문) acc가 더 높은 값 대입
                self.score[n_estimators] = [s[n_estimators], Orange.evaluation.scoring.AUC(res)[0],
                                       Orange.evaluation.scoring.Precision(res, average='macro')[0],
                                       Orange.evaluation.scoring.Recall(res, average='macro')[0]]

        return s[n_estimators]
    ...
    def opt(self):
        ...
        optimizer.maximize(init_points=50, n_iter=300, acq='ei', xi=0.6)
