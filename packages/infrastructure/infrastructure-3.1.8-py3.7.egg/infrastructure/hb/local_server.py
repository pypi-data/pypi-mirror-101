...
# @Last Modified time: 2021-04-12 21:43:25
...
class LocalServer(object):
	...
	def cpFilePreprocess(self,alreadyRecord):
		...
		mergeFile = JAVA_COV["mergeFile"].format(week=alreadyRecord.week_num,
			service_name=alreadyRecord.service_name,recordID=alreadyRecord.id)
		mdestfile = JAVA_COV["mdestfile"].format(week=alreadyRecord.week_num,
				service_name=alreadyRecord.service_name,recordID=alreadyRecord.id)
		if not os.path.exists(mergeFile):
			os.makedirs(mergeFile)

		if not os.path.exists("{jacocoIt}".format(self.jacocoIt.format(
		...
		os.makedirs(mdestfile) if not os.path.exists(mdestfile) else os.popen("rm -rf {}/*".format(mdestfile)).read()
		command = "cp {jacocoIt} {mergeFile}/{index}.exec".format(jacocoIt=self.jacocoIt.format(
				week=self.alreadyRecord.week_num,service_name=self.alreadyRecord.service_name,recordID=self.alreadyRecord.id,index=self.alreadyRecord.id),mergeFile=mergeFile,
		...
		if not os.path.exists("{mergeFile}/{index}.exec".format(mergeFile=mergeFile,
				index=self.alreadyRecord.id)):
	...
	def mergeRecordFile(self):
		# self.cpFile()
		if not os.path.exists(self.mergeFile):
			os.makedirs(self.mergeFile) 
	...
	def mergePostProcess(self):
		...
		"""
		os.popen("rm -rf {}/*".format(self.mergeFile)).read()
		command = "mv {mdestfile}/all.exec {mergeFile}/".format(mdestfile=self.mdestfile,mergeFile=self.mergeFile)
	...
	def parseReport(self):
		...
		if os.path.exists(index_path):
			...
			report_html_url = ['http://{}:8001/report/{}/{}/{}/{}/index.html'.format(
					env,getWeek(onlyWeek=True),reportName,self.alreadyRecord.service_name,self.alreadyRecord.id),
	...
	def deleteImportantRes(self):
		...
		# with os.popen("rm -rf {jacocoIt}".format(
		# 	jacocoIt=(self.jacocoIt.format(week=self.alreadyRecord.week_num,service_name=self.alreadyRecord.service_name,
		# 			recordID=self.alreadyRecord.id,index=self.alreadyRecord.id)))) as temp:
		# 	temp.read()
