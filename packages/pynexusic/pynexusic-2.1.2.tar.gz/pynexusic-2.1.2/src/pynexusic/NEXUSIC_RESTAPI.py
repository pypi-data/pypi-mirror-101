...
class NEXUSIC_REST():
    ...
    ######################################################################################
    def getTable(self, tableDBName, xFilter=None, pageSize=None, current_attempt=1,
                 verbose=False):
        if self.verbose or verbose:
        ...
        if pageSize == None:
            uri = baseURI + '/bo/' + tableDBName + '/' + '?hash=' + self.hash
        else:
            uri = baseURI + '/bo/' + tableDBName + '/' + '?pageSize=' + str(pageSize) + '&hash=' + self.hash
    ...
    ######################################################################################
    def getAssetByFullLocation(self, fullLocation, assetView=None, current_attempt=1, pageSize=None, verbose=False):
        """
            ...
            :param pageSize: (``int`` - optional) - Page size for response (default value None). When default is used the default NEXUS IC page size will be used (1000).
            :param verbose: (``bool`` - optional) - Print internal messages if True (default value False).
    ...
    ######################################################################################
    def getTableDefInfo(self, tableName, tableType, current_attempt=1,
                        pageSize=None, verbose=False):
        x_filter = {"operator": "and",
                    "where": [{"field": "Name", "value": tableName},
                              {"field": "Def_Type.Name", "value": tableType}]
