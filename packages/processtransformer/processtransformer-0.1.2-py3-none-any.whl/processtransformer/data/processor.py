...
class LogsDataProcessor:
    def __init__(self, name, filepath, columns, dir_path = "./datasets/processed"):
    ...
    def _process_next_time(self, df, train_list, test_list):
        ...
        train_df = processed_df_time[processed_df["case_id"].isin(train_list)]
        test_df = processed_df_time[processed_df["case_id"].isin(test_list)]
    ...
    def _process_remaining_time(self, df, train_list, test_list):
        ...
        train_remaining_time = processed_df_remaining_time[processed_df["case_id"].isin(train_list)]
        test_remaining_time = processed_df_remaining_time[processed_df["case_id"].isin(test_list)]
    ...
    def _process_all(self, df, train_list, test_list):
        case_id = "case:concept:name"
        event_name = "concept:name"
        event_time = "time:timestamp"
        processed_df = pd.DataFrame(columns = ["case_id", "prefix", "k", "time_passed", 
                "recent_time", "latest_time", "next_act", "next_time", "remaining_time_days"])
        idx = 0
        unique_cases = df[case_id].unique()
        for _, case in enumerate(tqdm(unique_cases)):
            act = df[df[case_id] == case][event_name].to_list()
        ...
        processed_df_na = processed_df[["case_id", "prefix", "k", "next_act"]]
        processed_df_nt = processed_df[["case_id", "prefix", "k", "time_passed", 
            "recent_time", "latest_time", "next_time"]]
        processed_df_rmt = processed_df[["case_id", "prefix", "k", "time_passed", 
            "recent_time", "latest_time","remaining_time_days"]]
        
        k_max = processed_df_na.groupby("case_id")["k"].transform("max")
        processed_df_na = processed_df_na[~(processed_df_na["k"] == k_max)]
        train_na = processed_df_na[processed_df_na["case_id"].isin(train_list)]
        test_na = processed_df_na[processed_df_na["case_id"].isin(test_list)]
        train_na.to_csv(f"{self._dir_path}/{self._name}/{Task.NEXT_ACTIVITY.value}_train.csv", index = False)
        test_na.to_csv(f"{self._dir_path}/{self._name}/{Task.NEXT_ACTIVITY.value}_test.csv", index = False)

        train_nt = processed_df_nt[processed_df["case_id"].isin(train_list)]
        test_nt = processed_df_nt[processed_df["case_id"].isin(test_list)]
        train_nt.to_csv(f"{self._dir_path}/{self._name}/{Task.NEXT_TIME.value}_train.csv", index = False)
        test_nt.to_csv(f"{self._dir_path}/{self._name}/{Task.NEXT_TIME.value}_test.csv", index = False)
        
        train_rmt = processed_df_rmt[processed_df["case_id"].isin(train_list)]
        test_rmt = processed_df_rmt[processed_df["case_id"].isin(test_list)]
        train_rmt.to_csv(f"{self._dir_path}/{self._name}/{Task.REMAINING_TIME.value}_train.csv", index = False)
        test_rmt.to_csv(f"{self._dir_path}/{self._name}/{Task.REMAINING_TIME.value}_test.csv", index = False)
    ...
    def process_logs(self, task, 
        ...
        if task == Task.NEXT_ACTIVITY:
            ...
            self._process_all(df, train_list, test_list)
        else:
