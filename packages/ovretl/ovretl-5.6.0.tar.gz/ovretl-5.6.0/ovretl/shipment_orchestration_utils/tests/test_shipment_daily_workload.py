...
def test_shipments_timelines_with_transit_time():
    ...
    tasks_df = pd.DataFrame(data={"shipment_id": []})
    result_should_be = pd.DataFrame(
        data={"shipment_id": ["1", "1", "2", "2", "2"], "dates": [day_3, day_4, day_3, day_4, day_5]}
    )
...
def test_estimated_remaining_daily_workload():
    ...
    result_should_be = pd.DataFrame(
        data={
            ...
            "remaining_days": [0, 11],
            "total_tasks_weight": [1, 16],
            "estimated_remaining_daily_workload": [0.0, 4.0],
        }
    )
...
def test_shipments_daily_workload_timelines():
    ...
    shipment_2_workload = [0.0, 1.0, 6.0, 16.0, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0]
    result_should_be = pd.DataFrame(
