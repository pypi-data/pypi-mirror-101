...
class PymunkDemo(RelativeLayout):
    def small_ball(self, space):
        for x in range(10):
            mass = 3
            radius = 8
            moment = easymunk.moment_for_circle(mass, radius, 0)
    ...
    def car(self, space):
        ...
        mass = 100
        radius = 25
        moment = easymunk.moment_for_circle(mass, radius, 20)
        ...
        space.add(
            easymunk.DistanceJoint(
                wheel1_b, chassi_b, anchor_a=(0, 0), anchor_b=(-25, -15)
            ),
            easymunk.DistanceJoint(
                wheel1_b, chassi_b, anchor_a=(0, 0), anchor_b=(-25, 15)
            ),
            easymunk.DistanceJoint(
                wheel2_b, chassi_b, anchor_a=(0, 0), anchor_b=(25, -15)
            ),
            easymunk.DistanceJoint(
                wheel2_b, chassi_b, anchor_a=(0, 0), anchor_b=(25, 15)
            ),
        ...
        speed = -4
        space.add(
            easymunk.AngularVelocityJoint(wheel1_b, chassi_b, speed),
            easymunk.AngularVelocityJoint(wheel2_b, chassi_b, speed),
        )
        with self.canvas:
            Color(*wheel_color)
    ...
    def create_logo_lines(self, logo_img):
        ...
        for line in line_set:
            line = easymunk.simplify_curves(line, 0.7)
    ...
    def start(self):
        ...
        floor = easymunk.Segment((-100, 0), (900, 62), 5, space.static_body)
        floor.friction = 1.0
        space.add(floor)
        with self.canvas:
            Color(0.2, 0.2, 0.2)
        ...
        for shape in self.space.shapes:
            if hasattr(shape, "ky") and not shape.body.is_sleeping:
                ...
                if isinstance(shape, easymunk.Segment):
                    body = shape.body
                    p1 = body.position + shape.a.complex_rotation(body.rotation_vector)
                    p2 = body.position + shape.b.complex_rotation(body.rotation_vector)
