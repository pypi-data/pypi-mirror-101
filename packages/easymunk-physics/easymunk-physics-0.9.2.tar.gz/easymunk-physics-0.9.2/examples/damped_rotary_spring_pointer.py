...
def main():
    ...
    pointer_body = mk.Body(body_type=mk.Body.KINEMATIC)

    ps = [(80, 0), (0, 20), (0, -20)]
    moment = mk.moment_for_poly(1, ps)
    gun_body = mk.Body(1, moment)
    gun_body.position = Vec2d(300, 300)
    gun_shape = mk.Poly(ps, body=gun_body)

    rest_angle = 0
    stiffness = 125000.0
    damping = 6000.0

    rotary_spring = mk.DampedRotarySpring(
        pointer_body, gun_body, stiffness, damping, rest_angle
    )
