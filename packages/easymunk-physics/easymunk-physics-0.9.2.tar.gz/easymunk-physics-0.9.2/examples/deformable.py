...
def generate_geometry(surface, space):
    ...
    line_set = easymunk.march_soft(BB(0, 0, 599, 599), 60, 60, 90, sample_func)

    for polyline in line_set:
        line = easymunk.simplify_curves(polyline, 1.0)
...
def main():
    ...
    space.collision_handler(0, 1).pre_solve = pre_solve
    ...
    for x in range(25):
        mass = 1
        moment = easymunk.moment_for_circle(mass, 10, 0)
