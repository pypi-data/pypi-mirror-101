...
import easymunk
from easymunk import Vec2d, geometry
...
class PhysicsDemo:
    ...
    def create_ball(self, point, mass=1.0, radius=15.0):
        ...
        moment = pm.moment_for_circle(mass, radius, 0.0)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if len(self.poly_points) > 0:
                    self.poly_points = easymunk.reduce_poly(
                        self.poly_points, tolerance=5
                    )
                if len(self.poly_points) > 2:
                    self.poly_points = easymunk.convex_hull(self.poly_points)
                    if not geometry.is_clockwise(self.poly_points):
                        self.poly_points.reverse()

                    center = easymunk.centroid(self.poly_points)
                    self.poly_points = easymunk.poly_vectors_around_center(
                        self.poly_points
                    )
