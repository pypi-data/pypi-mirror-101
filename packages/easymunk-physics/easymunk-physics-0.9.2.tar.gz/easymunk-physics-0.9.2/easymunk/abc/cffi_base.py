...
class CpCFFIBase(ABC):
    ...
    _ipython_renderer: IPythonRenderer = IPythonRenderer()

    # Keep references before adding objects to space
    _nursery: List["CpCFFIBase"]
    _nursery = lazy(lambda self: [])  # type: ignore
    ...
    def _describe_head(self, indent: str, name: str, description: str) -> Iterator[str]:
    ...
    def _describe_body(self, indent: str, memo) -> Iterator[str]:
        yield from self._describe_collection(self._describe_objects(memo), indent, memo)

    def _describe_element(self, name: str, obj, indent, memo) -> Iterator[str]:
    ...
    def _describe_collection(self, objs: Iterable, indent: str, memo) -> Iterator[str]:
