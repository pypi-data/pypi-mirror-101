...
def moment_for_circle(
    mass: float, radius: float, inner_radius: float = 0.0, offset: VecLike = (0, 0)
) -> float:
    """
    Calculate the moment of inertia for a possibly hollow circle
    """
    return lib.cpMomentForCircle(mass, inner_radius, radius, offset)
...
def area_for_circle(radius: float, inner_radius: float = 0.0) -> float:
    """
    Area of a possibly hollow circle.
    """
    return lib.cpAreaForCircle(inner_radius, radius)
...
def is_clockwise(points: Sequence[VecLike]) -> bool:
    """
    Return True if the points given form a clockwise polygon.

    This function assumes standard cartesian axes (i.e., y-axis points up).
    """
    pts = iter(points)
    (x, y) = next(pts)
    ...
    for u, v in pts:
        cross_prods += x * v - y * u
    return cross_prods <= 0.0


def is_left(p0: VecLike, p1: VecLike, p2: VecLike) -> int:
...
# TODO: remove? we already have cpPolylineToConvexHull
...
def centroid(points: Sequence[VecLike]) -> Vec2d:
    """
...
def poly_vectors_around_center(points: Sequence[VecLike]) -> List[Vec2d]:
    """
    ...
    cm = centroid(points)
    return [pt - cm for pt in points]


# TODO: do we need this and lib.cpAreaForPoly?
...
# TODO: change naming conventions to perimeter_for_poly? implement other shapes?
...
# FIXME: do we need this and lib.cpPolylineConvexDecomposition?
