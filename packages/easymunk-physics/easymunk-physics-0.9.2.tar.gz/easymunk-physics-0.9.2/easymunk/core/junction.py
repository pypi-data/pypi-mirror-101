...
from sidekick.properties import alias
...
class Junction(GameObjectInterface, Sequence[Constraint]):
    ...
    a: "Body" = alias("_a")
    b: "Body" = alias("_b")
    space: Optional["Space"] = alias("_space")
    ...
    def flipped(self) -> "Junction":
        ...
        try:
            ...
            self._flipped: weakref.ReferenceType[Junction] = weakref.ref(flipped)
        return flipped
    ...
    def __init__(self, a: "Body", b: "Body", space: Optional["Space"] = None):
        self._a = a
        self._b = b
    ...
    def fix_distance(
        self,
        *args: RangeLimit,
        tol: Union[None, float, Tuple[float, float]] = None,
    ...
    def fix_radians(
        self: JunctionT, *args, **kwargs: ConstraintKwargs
    ) -> LimitAngleJoint:
        """
        Constrains the relative rotations of two bodies.
    ...
    def fix_omega(
        self: JunctionT, rate: float = 0.0, **kwargs: ConstraintKwargs
    ) -> AngularVelocityJoint:
        """
        Force a fixed relative angular velocity between two bodies.
    ...
    def _create_constraint(
        self, cls: Callable[..., CT], args, kwargs: Dict[str, Any]
    ) -> CT:
        # Todo: type kwargs: ConstraintOpts as soon as mypy support this.
        cons = cls(self.a, self.b, *args, **kwargs)
        self.add_constraint(cons)
        return cons

    def detach(self: JunctionT) -> JunctionT:
        """
    ...
    def destroy(self: JunctionT) -> JunctionT:
        """
        Destroy joint, making it unusable.
        """
        self.detach()
        del self._a, self._b, self._constraints  # type: ignore
        return self

    def clear(self: JunctionT) -> JunctionT:
        """
    ...
    def add_constraint(self: JunctionT, constraint: Constraint) -> JunctionT:
        """
        ...
        if (space := self._space) is None:
            raise RuntimeError("cannot add constraint to detached joint")

        self._constraints.append(constraint)
        space.run_safe(self._add_constraint_to_space, constraint, space)
        return self

    def _add_constraint_to_space(self, constraint: Constraint, space: "Space") -> None:
    ...
    def remove_constraint(self: JunctionT, constraint: Constraint) -> JunctionT:
        """
        ...
        if (space := self._space) is not None:
            space.run_safe(self._remove_constraint_from_space, constraint, space)
        return self

    def _remove_constraint_from_space(self, constraint, space):
