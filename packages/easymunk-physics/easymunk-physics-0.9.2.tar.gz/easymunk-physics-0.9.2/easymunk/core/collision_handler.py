...
def always_collide(arb: Arbiter):
    return True


def do_nothing(arb: Arbiter):
    return None
...
class CollisionHandler:
    ...
    def as_dict(self) -> Dict[str, Union[NullCB, BoolCB, None]]:
        """
    ...
    def _bool_cb(self, func: BoolCB, ptr: CData, _space: CData, _data: CData) -> bool:
    ...
    def _null_cb(self, func: NullCB, ptr: CData, _space: CData, _data: CData) -> None:
        arb = Arbiter(ptr, self._space)
        func(arb)
        arb.close()  # it is not safe to access arbiter anymore

    def _locked_cb(self, func: NullCB, ptr: CData, _space: CData, _data: CData) -> None:
