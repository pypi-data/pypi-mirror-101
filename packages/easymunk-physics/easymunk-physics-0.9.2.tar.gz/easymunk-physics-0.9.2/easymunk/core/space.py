...
from ..cp import lib, ffi, property_
...
# Types
ColType = TypeVar("ColType", bound="int")
...
NO_STEP_EVENTS: Container[str] = frozenset()
ALL_STEP_EVENTS: Container[str] = frozenset(
    {"before-step", "before-sub-step", "after-sub-step", "after-step"}
)
SHAPE_TO_BODY = {Circle: CircleBody, Segment: SegmentBody, Poly: PolyBody}
NO_KWARGS: Mapping[str, Any] = MappingProxyType({})
...
class Space(
    ...
    shapes: Shapes = property_(
        fget=lambda s: Shapes(s, s._iter_shapes),
        doc="""A list of all the shapes added to this space

        (includes both static and non-static)
        """,
    )
    bodies: Bodies = property_(
        fget=lambda s: Bodies(s, s._bodies.__iter__),
        doc="A list of the bodies added to this space",
    )
    constraints: Constraints = property_(
        fget=lambda s: Constraints(s, s._iter_constraints),
    ...
    _named_bodies = lazy(lambda self: {})  # type: ignore
    ...
    def __init__(self, threaded: bool = False, **kwargs) -> None:
        ...
        self._cffi_ref: Any = ffi.gc(cp_space, partial(cffi_free_space, freefunc))
    ...
    def create_path(
        ...
        **kwargs,
        ...
        Keyword Args:
            Accepts all arguments for :class:`SegmentBody`.
        """
        if not vertices:
    ...
    def create_margin(self, bb: BBLike, **kwargs) -> Tuple["Body", List["Segment"]]:
        """
    ...
    def add(self: ST, *args: AddableObjects, add_children=True) -> ST:
    ...
    def step(
        ...
        skipped_events: Container[str]
        if skip_events is False:
            skipped_events = NO_STEP_EVENTS
        elif skip_events is True:
            skipped_events = ALL_STEP_EVENTS
        else:
            skipped_events = skip_events  # type: ignore

        self._execute_step_handlers("before-step", skipped_events)
        for _ in range(sub_steps):
            self._execute_step_handlers("before-sub-step", skipped_events)
            try:
                self._locked = True
                if self.threaded:
            ...
            self._execute_step_handlers("after-sub-step", skipped_events)
        self._execute_step_handlers("after-step", skipped_events)
        return self
    ...
    def _step_decorator(self, event, opt, args, kwargs):
        def decorator(fn):
            if args or kwargs:
                cb = partial(fn, *(args or ()), **(kwargs or {}))
            else:
                cb = fn
    ...
    def draw(self: ST, options: "DrawOptions" = None) -> ST:
        ...
        for body in self.bodies:
            body.draw(options, shapes=shapes)
        return self
