...
"""
import logging
from typing import TYPE_CHECKING, Any, Callable, Optional, TypeVar, TypedDict, Union
...
from ..typing import VecLike, EllipsisType
from ..util import void, init_attributes, set_attrs
...
RangeLimit = Union[float, EllipsisType]
SolveFunc = Callable[["Constraint"], None]
ForceFunc = Callable[[float], float]
T = TypeVar("T")
S = TypeVar("S")
CT = TypeVar("CT", bound="Constraint")
...
DEG: "WrapF" = {"wrap": to_degrees, "prepare": to_radians}
INVDEG: "WrapF" = {"prepare": to_degrees, "wrap": to_radians}


class WrapF(TypedDict):
    wrap: Callable[[float], float]
    prepare: Callable[[float], float]
...
class Constraint(GameObjectInterface, CpCFFIBase):
    ...
    def __init__(
        ...
        self._junction: Optional["Junction"] = None
...
class LimitDistanceJoint(AbstractDistanceJoint):
    ...
    anchor_a = cpvec_property_("cpSlideJoint(Get|Set)AnchorA", doc=ANCHOR_A)
    anchor_b = cpvec_property_("cpSlideJoint(Get|Set)AnchorB", doc=ANCHOR_B)
    min_distance: float = cp_property_(
        "cpSlideJoint(Get|Set)Min", doc="Minimum distance between anchor points."
    )
    max_distance: float = cp_property_(
        "cpSlideJoint(Get|Set)Max", doc="Maximum distance between anchor points."
    )
    distance_vector: Vec2d = property_(
        fget=lambda c: c._a.local_to_local(c.anchor_a, c._b) - c.anchor_b,
        doc="""
        Distance vector between two anchor points.
        """,
    )
    distance: float = property_(
        fget=lambda c: max(min(c.distance_vector.length, c.max), c.min),
        fset=lambda c, value: set_attrs(c, min=value, max=value),
        doc="""
...
class DistanceJoint(AbstractDistanceJoint):
    ...
    anchor_a = cpvec_property_("cpPinJoint(Get|Set)AnchorA", doc=ANCHOR_A)
    anchor_b = cpvec_property_("cpPinJoint(Get|Set)AnchorB", doc=ANCHOR_B)
    distance: float = property_(
    ...
    min_distance: float = property_(lambda self: self.distance)
    max_distance: float = property_(lambda self: self.distance)
    ...
    def __init__(
        ...
        **kwargs: ConstraintKwargs,
        ...
        ptr = lib.cpPinJointNew(a._cffi_ref, b._cffi_ref, anchor_a, anchor_b)
        super().__init__(a, b, ptr, **kwargs)
...
class PositionJoint(AbstractDistanceJoint):
    ...
    anchor_a = cpvec_property_("cpPivotJoint(Get|Set)AnchorA", doc=ANCHOR_A)
    anchor_b = cpvec_property_("cpPivotJoint(Get|Set)AnchorB", doc=ANCHOR_B)
    groove_a: Vec2d = property_(lambda self: self.anchor_a)
    groove_b: Vec2d = property_(lambda self: self.anchor_a)

    # Properties of slide joints.
    distance_vector = property_(lambda self: Vec2d.zero())
    distance = property_(lambda self: 0.0)
    min_distance = property_(lambda self: 0.0)
    max_distance = property_(lambda self: 0.0)
    ...
    def __init__(
        ...
        **kwargs: ConstraintKwargs,
        ...
        _constraint = lib.cpGrooveJointNew(
            a._cffi_ref, b._cffi_ref, segment_start, segment_end, anchor
        )
    ...
    def __init__(
        ...
        **kwargs: ConstraintKwargs,
        ...
        ptr = lib.cpDampedRotarySpringNew(
            a._cffi_ref, b._cffi_ref, angle, stiffness, damping
        )
        super().__init__(a, b, ptr, **kwargs)
    ...
    def __init__(
        ...
        **kwargs: ConstraintKwargs,
        ...
        if phase is None:
            ...
            phase = to_radians(phase)
        ptr = lib.cpGearJointNew(a._cffi_ref, b._cffi_ref, phase, ratio)
        super().__init__(a, b, ptr, **kwargs)
...
class AngularVelocityJoint(Constraint):
    ...
    def __init__(
        ...
        **kwargs: ConstraintKwargs,
        ...
        ptr = lib.cpSimpleMotorNew(a._cffi_ref, b._cffi_ref, rate)
        super().__init__(a, b, ptr, **kwargs)
