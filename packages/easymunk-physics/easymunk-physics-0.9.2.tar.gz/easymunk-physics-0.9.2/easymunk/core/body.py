...
from sidekick.properties import lazy, delegate_to
...
from ..util import set_attrs, init_attributes
...
AxisT = Union[VecLike, Literal["center", "pos"], None]
ConsT = TypeVar("ConsT", bound=Constraint)

# Constants
BODY_TYPES: Dict[BodyType, int] = {}
CREATED_BODIES: MutableMapping[int, "Body"] = WeakValueDictionary()
...
class Body(
    ...
    * Don't modify a body's position every step unless you really know what
    ...
    _connected_bodies = lazy(lambda self: set())  # type: ignore
    color: Any = None
    ...
    @property
    def velocity_func(self) -> VelocityFuncT:
        ...
        """
        return self._velocity_func_base or self.update_velocity

    @velocity_func.setter
    def velocity_func(self, func: VelocityFuncT) -> None:
    ...
    @property
    def position_func(self) -> PositionFuncT:
        ...
        used to update the body's position.
            ...
            ``func(body, dt) -> None``
        """
        return self._position_func_base or self.update_position

    @position_func.setter
    def position_func(self, func: PositionFuncT):
        @ffi.callback("cpBodyPositionFunc")
        def _impl(_: ffi.CData, dt: float):
            func(self, dt)
    ...
    def remove_shape(self: BodyT, shape: "Shape") -> BodyT:
        ...
        if (space := shape._space) is not None:
            space.run_safe(self._remove_shape_from_space, shape, space)
        return self

    def _remove_shape_from_space(self, shape: Shape, space: "Space") -> None:
        shape_ref = shape._cffi_ref
        space_ref = space._cffi_ref
    ...
    def copy(self: BodyT, **kwargs) -> BodyT:
        ...
        new: BodyT = super().copy(**kwargs)
        for shape in self._shapes:
            sp = shape.copy(body=new)
            new._nursery.append(sp)
        return new

    def step(self: BodyT, dt: float) -> BodyT:
        """
        Evolve body, if not connected to space.

        An body attached to a space does not update physics in this step, but
        rather when its space .step() method is called.
        """
        return self.step_physics(dt) if self._space is None else self

    def step_physics(
        self: BodyT, dt: float, gravity: VecLike = (0, 0), damping: float = 1.0
    ) -> BodyT:
        """
        Advance physics by dt, ignoring interaction with other elements in the
        same space.

        A custom gravity and damping can be given.
        """
        ...
        self.velocity_func(self, Vec2d.from_coords(gravity), damping, dt)
    ...
    def draw(self, camera=None, shapes=None):
        if shapes is None:
            ...
            draw_shapes = filter(lambda s: s in shapes, self.shapes)
    ...
    # Describe
    def _describe_body(self, indent, memo) -> Iterator[str]:
        if self.description:
            yield ""
...
class BodyShape(Body):
    ...
    # Properties
    radius: float = delegate_to("shape", mutable=True)
    area: float = delegate_to("shape")
    collision_type: int = delegate_to("shape", mutable=True)
    filter: "ShapeFilter" = delegate_to("shape", mutable=True)
    elasticity: float = delegate_to("shape", mutable=True)
    friction: float = delegate_to("shape", mutable=True)
    surface_velocity: Vec2d = delegate_to("shape", mutable=True)
    bb: "BB" = delegate_to("shape")
    center: Vec2d = delegate_to("shape")
    center_world: Vec2d = delegate_to("shape")

    # Methods
    point_query = delegate_to("shape")
    segment_query = delegate_to("shape")
    shapes_collide = delegate_to("shape")
    radius_of_gyration_sqr = delegate_to("shape")
...
class CircleBody(BodyShape, Body):
    ...
    offset: Vec2d = delegate_to("shape", mutable=True)
    offset_world: Vec2d = delegate_to("shape", mutable=True)
...
class SegmentBody(BodyShape, Body):
    ...
    a: Vec2d = delegate_to("shape", mutable=True)
    a_world: Vec2d = delegate_to("shape", mutable=True)
    b: Vec2d = delegate_to("shape", mutable=True)
    b_world: Vec2d = delegate_to("shape", mutable=True)
    endpoints: Tuple[Vec2d, Vec2d] = delegate_to("shape", mutable=True)
    endpoints_world: Tuple[Vec2d, Vec2d] = delegate_to("shape", mutable=True)
...
class PolyBody(BodyShape, Body):
    ...
    vertices: List[Vec2d] = delegate_to("shape", mutable=True)
    vertices_world: List[Vec2d] = delegate_to("shape", mutable=True)
    get_vertices = delegate_to("shape")
    set_vertices = delegate_to("shape")
...
def body_from_cffi(ptr: ffi.CData) -> Body:
    ...
    """
    if not bool(ptr):
        raise ValueError("cannot get body from null pointer")

    id_ = int(ffi.cast("int", lib.cpBodyGetUserData(ptr)))
    try:
        return CREATED_BODIES[id_]
    except KeyError:
        raise ValueError("Body does not exist")
