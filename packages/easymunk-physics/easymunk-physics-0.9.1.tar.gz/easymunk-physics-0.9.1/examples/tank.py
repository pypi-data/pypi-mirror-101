...
def update(space, dt, surface):
    ...
    mouse_pos = draw_options.from_pygame(Vec2d(*mpos))
    ...
    if (mouse_pos - tank_body.position).length_sqr < 30 ** 2:
        ...
        tank_control_body.velocity = tank_body.rotation_vector.complex_rotation(dv)
...
def init():
    ...
    for _ in range(50):
        ...
        pivot = easymunk.PositionJoint(static_body, body, (0, 0), (0, 0))
        space.add(pivot)
        pivot.max_bias = 0  # disable joint correction
        pivot.max_force = 1000  # emulate linear friction

        gear = easymunk.AngularRatioJoint(static_body, body, 1.0, 0.0)
        space.add(gear)
    ...
    pivot = easymunk.PositionJoint(tank_control_body, tank_body, (0, 0), (0, 0))
    space.add(pivot)
    pivot.max_bias = 0  # disable joint correction
    pivot.max_force = 10000  # emulate linear friction

    gear = easymunk.AngularRatioJoint(tank_control_body, tank_body, 1.0, 0.0)
    space.add(gear)
