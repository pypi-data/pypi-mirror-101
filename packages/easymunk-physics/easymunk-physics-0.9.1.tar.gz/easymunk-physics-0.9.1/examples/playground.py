...
class PhysicsDemo:
    ...
    def draw_ball(self, ball):
        body = ball.body
        v = body.position + ball.offset.complex_rotation(body.rotation_vector)
        p = self.flipyv(v)
        r = ball.radius
    ...
    def draw_wall(self, wall):
        body = wall.body
        pv1 = self.flipyv(body.position + wall.a.complex_rotation(body.rotation_vector))
        pv2 = self.flipyv(body.position + wall.b.complex_rotation(body.rotation_vector))
    ...
    def draw_poly(self, poly):
        ...
        if easymunk.geometry.is_clockwise(ps):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if len(self.poly_points) > 0:
                    self.poly_points = easymunk.geometry.reduce_poly(
                        self.poly_points, tolerance=5
                    )
                if len(self.poly_points) > 2:
                    self.poly_points = easymunk.geometry.convex_hull(self.poly_points)
                    if not easymunk.geometry.is_clockwise(self.poly_points):
                        self.poly_points.reverse()

                    center = easymunk.geometry.centroid(self.poly_points)
                    self.poly_points = easymunk.geometry.poly_vectors_around_center(
                        self.poly_points
                    )
